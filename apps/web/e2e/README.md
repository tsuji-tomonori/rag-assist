...
`screen-reader-semantics.spec.ts` は Chromium の accessibility tree を CDP 経由で取得し、login / chat / documents / history / profile の代表画面で landmark、form、control の role と accessible name が欠落しないことを検査します。historyでは並び順comboboxのvalueとお気に入りcheckboxのchecked state、profileでは送信キーcomboboxのvalueも検証します。各画面の検査時には、検査対象 role、name、value、checked state の JSON を Playwright report へ attach します。

...
function sseFinal(data: unknown, id = 1) {
...
function mockAppFetch(groups = ["SYSTEM_ADMIN"], initialHistory: ConversationHistoryItem[] = [], initialBenchmarkRuns: BenchmarkRun[] = []) {
  ...
  const fetchMock = vi.fn((url: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (requestUrl.endsWith("/admin/usage") && isGet(init)) {
      return Promise.resolve(response({ periodStart: "2026-05-01T00:00:00.000Z", periodEnd: "2026-05-02T00:00:00.000Z", users: [{ userId: "local-dev", email: "tester@example.com", chatMessages: 12, chatRequestCount: 12, llmCallCount: 2, inputTokens: 900, outputTokens: 300, totalTokens: 1200, estimatedCostUsd: 0.00144, actualTokenEventCount: 0, estimatedTokenEventCount: 2, missingTokenEventCount: 0, conversationCount: 3, questionCount: 1, documentCount: 2, benchmarkRunCount: 0, debugRunCount: 0, lastActivityAt: "2026-05-02T00:00:00.000Z" }], breakdowns: { byFeature: [{ key: "rag.generate_answer", label: "rag.generate_answer", inputTokens: 900, outputTokens: 300, totalTokens: 1200, estimatedCostUsd: 0.00144, actualTokenEventCount: 0, estimatedTokenEventCount: 2, missingTokenEventCount: 0 }], byModel: [{ key: "amazon.nova-lite-v1:0", label: "amazon.nova-lite-v1:0", inputTokens: 900, outputTokens: 300, totalTokens: 1200, estimatedCostUsd: 0.00144, actualTokenEventCount: 0, estimatedTokenEventCount: 2, missingTokenEventCount: 0 }], byGroup: [{ key: "SYSTEM_ADMIN", label: "SYSTEM_ADMIN", inputTokens: 900, outputTokens: 300, totalTokens: 1200, estimatedCostUsd: 0.00144, actualTokenEventCount: 0, estimatedTokenEventCount: 2, missingTokenEventCount: 0 }] }, totals: { inputTokens: 900, outputTokens: 300, totalTokens: 1200, estimatedCostUsd: 0.00144 }, dataCompleteness: { actualTokenEventCount: 0, estimatedTokenEventCount: 2, missingTokenEventCount: 0 } }))
    }
    if (requestUrl.endsWith("/admin/costs") && isGet(init)) {
      return Promise.resolve(response({ periodStart: "2026-05-01T00:00:00.000Z", periodEnd: "2026-05-02T00:00:00.000Z", currency: "USD", totalEstimatedUsd: 0.0123, pricingVersion: "bedrock-2026-06-local-v1", pricingCatalogUpdatedAt: "2026-05-02T00:00:00.000Z", users: [{ userId: "local-dev", email: "tester@example.com", estimatedCostUsd: 0.0123 }], items: [{ service: "Bedrock", category: "chat completion", usage: 1200, unit: "token", unitCostUsd: 0, estimatedCostUsd: 0.0096, confidence: "estimated_usage", pricingVersion: "bedrock-2026-06-local-v1" }], dataCompleteness: { actualTokenEventCount: 0, estimatedTokenEventCount: 2, missingTokenEventCount: 0 } }))
    }
    ...
    if (requestUrl.endsWith("/rpc/chat/startRun") && init?.method === "POST") {
      ...
      return Promise.resolve(response({ runId: "chat-run-1", status: "queued", eventsPath: "/chat-runs/chat-run-1/events" }))
    }
    if (requestUrl.endsWith("/chat-runs/chat-run-1/events") && init?.method === "GET") {
      return Promise.resolve(response(sseFinal(lastChatResponse)))
    }
    return Promise.resolve(response({}))
  })
...
describe("App chat and upload flow", () => {
  ...
  it("disables submission while a question is pending and selects the returned debug run", async () => {
    let resolveEvents: ((value: ReturnType<typeof response>) => void) | undefined
    const fetchMock = vi.fn((url: RequestInfo | URL, init?: RequestInit) => {
      const requestUrl = String(url)
      if (requestUrl === "/config.json") return Promise.resolve(response({ apiBaseUrl: "http://api.test" }))
      if (requestUrl.endsWith("/me") && isGet(init)) return Promise.resolve(response(currentUserResponse()))

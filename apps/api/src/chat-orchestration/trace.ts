...
function inferStatus(update: ChatOrchestrationUpdate): DebugStep["status"] {
...
function inferModelId(label: string, state: ChatOrchestrationState): string | undefined {
...
function summarizeUpdate(label: string, update: ChatOrchestrationUpdate): string {
  ...
  if (update.conversationState) return `conversation=${update.conversationState.conversationId ? "present" : "none"}, dependency=${update.conversationState.turnDependency}, citations=${update.conversationState.previousCitationCount}`
  if (update.sessionScopeNormalization) {
    return `temporaryScopeCount=${update.sessionScopeNormalization.acceptedTemporaryScopeCount}, denied=${update.sessionScopeNormalization.deniedTemporaryScopeCount}, reasons=${update.sessionScopeNormalization.reasonCodes.join(",") || "none"}, previousCitationAnchors=${update.sessionScopeNormalization.previousCitationAnchorCount}`
  }
...
function detailUpdate(update: ChatOrchestrationUpdate): string | undefined {
  ...
  if (update.conversationState) {
    return `activeEntities=${update.conversationState.activeEntities?.length ?? 0}, activeDocuments=${update.conversationState.activeDocuments?.length ?? 0}, activeTopics=${update.conversationState.activeTopics?.length ?? 0}, previousCitations=${update.conversationState.previousCitationCount}`
  }
  if (update.sessionScopeNormalization) return JSON.stringify(update.sessionScopeNormalization, null, 2)
...
function formatSufficientContextDetail(judgement: NonNullable<ChatOrchestrationUpdate["sufficientContext"]>): string {
  return [
...
function formatAnswerSupportDetail(judgement: NonNullable<ChatOrchestrationUpdate["answerSupport"]>): string {
  return [
...
function formatRetrievalEvaluationDetail(evaluation: NonNullable<ChatOrchestrationUpdate["retrievalEvaluation"]>): string {
  return [
...
function formatRiskSignals(signals: NonNullable<NonNullable<ChatOrchestrationUpdate["retrievalEvaluation"]>["riskSignals"]>): string[] {
...
function formatRetrievalLlmJudge(judge: NonNullable<ChatOrchestrationUpdate["retrievalEvaluation"]>["llmJudge"]): string[] {
  if (!judge) return ["なし"]
  return [
    `- label=${judge.label}`,
...
function outputUpdate(update: ChatOrchestrationUpdate): Record<string, JsonValue> | undefined {
  const output: Record<string, JsonValue> = {}
  const keys: Array<keyof ChatOrchestrationUpdate> = [
    "normalizedQuery",
...
function formatClarificationDetail(clarification: NonNullable<ChatOrchestrationUpdate["clarification"]>): string {
  return [
...
function formatAnswerabilityDetail(answerability: NonNullable<ChatOrchestrationUpdate["answerability"]>): string {
...
function formatSearchPlanDetail(searchPlan: NonNullable<ChatOrchestrationUpdate["searchPlan"]>): string {
...
function formatActionObservationDetail(actionHistory: NonNullable<ChatOrchestrationUpdate["actionHistory"]>): string {
...
function formatRetrievalDiagnostics(diagnostics: NonNullable<ChatOrchestrationUpdate["actionHistory"]>[number]["retrievalDiagnostics"]): string[] {
...
function inferHitCount(update: ChatOrchestrationUpdate): number | undefined {
  return update.memoryCards?.length ?? update.retrievedChunks?.length ?? update.selectedChunks?.length ?? update.citations?.length
}

function inferTokenCount(update: ChatOrchestrationUpdate): number | undefined {

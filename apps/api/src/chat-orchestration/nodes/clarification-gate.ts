...
import type { Clarification, ClarificationOption, ChatOrchestrationState, ChatOrchestrationUpdate } from "../state.js"
...
export async function clarificationGate(state: ChatOrchestrationState): Promise<ChatOrchestrationUpdate> {
...
function scoreAmbiguity(state: ChatOrchestrationState, query: string, optionCount: number): number {
...
function hasCandidateEvidence(state: ChatOrchestrationState): boolean {
  return state.memoryCards.length + state.retrievedChunks.length > 0
}

function hasSufficientSupportedEvidence(state: ChatOrchestrationState, query: string): boolean {
  return (
...
function hasExplicitQuestionScope(query: string, state: ChatOrchestrationState): boolean {
...
function hasSpecificTarget(query: string, state: ChatOrchestrationState): boolean {
  const queryTerms = significantTerms(query).filter((term) => !genericTerms.includes(term) && !isQuestionIntentTerm(term) && !isGenericCompoundTerm(term))
  const labels = [...state.memoryCards, ...state.retrievedChunks]
    .map((hit) => selectPublicLabel(hit, query))
    .filter((label): label is string => Boolean(label))
...
function evidencePublicTerms(state: ChatOrchestrationState): string[] {

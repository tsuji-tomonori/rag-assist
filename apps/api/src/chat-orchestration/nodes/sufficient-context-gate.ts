...
import { NO_ANSWER, isPrimaryRequiredFact, requiredFactNecessity, type ChatOrchestrationState, type ChatOrchestrationUpdate, type RequiredFact, type SufficientContextJudgement } from "../state.js"

type JudgeJson = Partial<SufficientContextJudgement>

export function createSufficientContextGateNode(deps: Dependencies) {
  return async function sufficientContextGate(state: ChatOrchestrationState): Promise<ChatOrchestrationUpdate> {
...
function hasQuestionAnchoredEvidence(state: ChatOrchestrationState): boolean {
...
function hasUnresolvedPrimaryMissingFact(state: ChatOrchestrationState, judgement: SufficientContextJudgement): boolean {
...
function hasPrimaryConflict(state: ChatOrchestrationState, judgement: SufficientContextJudgement): boolean {
...
function hasSupportedPrimaryEvidence(state: ChatOrchestrationState, judgement: SufficientContextJudgement): boolean {
  const primaryFacts = primaryRequiredFacts(state.searchPlan.requiredFacts)
  const supportedFacts = judgement.supportedFacts.map(normalize)
  if (primaryFacts.length === 0) return false
...
function hasPrimaryFactId(state: ChatOrchestrationState, factIds: string[]): boolean {
...
function primaryFactSupportedByEvidence(state: ChatOrchestrationState, fact: RequiredFact, supportedFacts: string[]): boolean {
...
function normalizeJudgement(parsed: JudgeJson | undefined, state: ChatOrchestrationState): SufficientContextJudgement {
...
function updateRequiredFactStatuses(state: ChatOrchestrationState, judgement: SufficientContextJudgement): ChatOrchestrationState["searchPlan"] {
...
function validSupportingChunkIds(ids: string[], state: ChatOrchestrationState): string[] {

...
export class SecurityMutationAuditReconciler {
  ...
  async reconcileTenant(tenantId: string, limit = 100): Promise<SecurityMutationAuditReconciliationResult> {
    ...
    return this.completeIntents(tenantId, pending, repaired, repairDeferred)
  }

  /** Explicit candidates only: never enumerate outbox or repair full history. */
  async reconcileIntents(tenantId: string, intentIds: readonly string[]): Promise<SecurityMutationAuditReconciliationResult> {
    assertCanonicalIdentifier(tenantId, "tenantId")
    if (intentIds.length < 1 || intentIds.length > 100 || new Set(intentIds).size !== intentIds.length) {
      throw new Error("Audit repair requires 1 to 100 unique intent IDs")
    }
    for (const id of intentIds) assertCanonicalIdentifier(id, "intentId")
    const pending: SecurityMutationAuditIntent[] = []
    for (const id of intentIds) {
      const intent = await this.outbox.get(tenantId, id)
      if (intent.draft.tenantId !== tenantId) throw new Error("Audit repair crossed tenant boundary")
      if (intent.status === "pending" || intent.status === "finalization_pending") pending.push(intent)
    }
    return this.completeIntents(tenantId, pending)
  }

  private async completeIntents(tenantId: string, pending: readonly SecurityMutationAuditIntent[], repaired = 0, repairDeferred = 0): Promise<SecurityMutationAuditReconciliationResult> {
    let completed = 0

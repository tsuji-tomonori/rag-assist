...
test("security audit reconciliation worker is single-tenant and rechecks authoritative domain state", async () => {
  ...
  assert.match(workerSource, /ResourceGroupMembershipAuditAuthoritativeResolver\(deps\.groupMembershipStore\)/)
  ...
  assert.match(workerSource, /FolderDeleteAuditAuthoritativeResolver\(deps\.documentGroupStore\)/)
  ...
  assert.doesNotMatch(folderDeleteResolverSource, /createWithPathLock|updateWithPathLocks|\.update\(|\.delete\(|list\(|putText|cleanup|register/)

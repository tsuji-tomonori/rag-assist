...
test("tracking separates tokenizer estimates from missing values and never invents a tenant", async () => {
  ...
  await model.embed("")
  const page = await store.query("tenant-a", { periodStart: "2020-01-01T00:00:00.000Z", periodEnd: "2030-01-01T00:00:00.000Z" })
  const generationEvent = page.events.find((event) => event.feature !== "embedding")
  const embeddingEvent = page.events.find((event) => event.feature === "embedding")
  assert.equal(generationEvent?.quantities[0]?.source, "tokenizer_estimate")
  assert.equal(generationEvent?.quantities[1]?.source, "missing")
  assert.equal(embeddingEvent?.quantities[0]?.source, "missing")

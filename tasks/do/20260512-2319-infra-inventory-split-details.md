...
- [ ] `docs/generated/infra-inventory.md` の「リソース別主要設定」以降が type 別詳細ファイルへのリンクになっている。
- [ ] `docs/generated/infra-inventory/*.md` が生成され、resource type ごとの詳細を保持する。
- [ ] 各詳細ファイルで Logical ID ごとに表が分かれ、設定値が `設定項目` / `値` の表で記載される。
- [ ] `npm run docs:infra-inventory:check` が分割ファイルも含めて drift を検出できる。
- [ ] README の生成先説明が分割ファイル構造と整合する。

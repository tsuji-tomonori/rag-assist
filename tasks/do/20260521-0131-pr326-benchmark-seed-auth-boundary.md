...
- [ ] `rag:doc:write:group` のみでは benchmark seed 形 body を `POST /documents` できない。
- [ ] `benchmark:seed_corpus` では benchmark seed body を group scope なしで `POST /documents` できる。
- [ ] 通常 writer が group scope 付きでも benchmark seed metadata を混ぜた場合は拒否される。
- [ ] `authorizeDocumentUpload` の単体テストで benchmark seed body は `benchmark:seed_corpus` 専用であることを固定する。
- [ ] 関連 API テスト、typecheck、diff check が pass する。

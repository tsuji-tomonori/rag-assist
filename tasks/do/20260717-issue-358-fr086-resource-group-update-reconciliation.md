...
- [ ] docs / coverage / access-control policy / targeted-full tests / lint / typecheck / build / final-head CIを確認する（localは成功、final-head CI待ち）。
- [x] benchmark期待語句、QA sample固有値、dataset固有分岐をproduct runtimeへ追加しない。
- [ ] post-task report、日本語draft PR、AC comment、self-review、task done lifecycleを完了する。

...
- [ ] `purpose=document` の `POST /documents` は group scope 未指定と groupIds 空配列を拒否する。
- [ ] `POST /documents/uploads/{uploadId}/ingest` は group scope 未指定と groupIds 空配列を拒否する。
- [ ] `POST /document-ingest-runs` は group scope 未指定と groupIds 空配列を拒否する。
- [ ] 通常文書 ingest は対象 groupIds に対して `assertDocumentGroupsWritable` を必ず呼ぶ。
- [ ] readOnly / nonexistent groupId は 403、full groupId は成功することをテストで固定する。
- [ ] groupIds / owner / ACL がない通常文書 manifest は一般ユーザーに可視化されない。
- [ ] Web hook は `uploadGroupId` 空のとき `uploadDocumentFile` を呼ばず、`ok:false` を返す。
- [ ] 関連 API/Web テスト、typecheck、docs check、diff check が pass する。

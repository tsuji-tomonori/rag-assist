...
保存先: `tasks/do/20260507-0844-rag-answerability-phase1.md`
...
- [ ] `RequiredFact` が `necessity: primary | secondary | inferred` を後方互換に保持できる。
- [ ] `sufficient_context_gate=PARTIAL` かつ primary fact supported の場合、secondary / inferred missing だけでは `finalize_refusal` にならない。
- [ ] 高信頼 `LLM judge == NO_CONFLICT` が conflict downgrade と回答継続に反映される。
- [ ] primary fact に触れる unresolved conflict または primary fact missing の場合は、既存どおり refusal / 追加検索に倒せる。
- [ ] benchmark 固有の expected phrase、row id、dataset 固有分岐を追加していない。
- [ ] 対象 API test と `git diff --check` が pass する、または未実施理由を明記する。

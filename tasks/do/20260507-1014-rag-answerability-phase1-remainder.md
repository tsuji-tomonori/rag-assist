...
保存先: `tasks/do/20260507-1014-rag-answerability-phase1-remainder.md`
...
- [ ] sufficient context prompt が required fact id を judge に明示している。
- [ ] supported primary fact id がある場合、追加具体化の missing 自然文だけでは hard refusal しない。
- [ ] primary fact id が missing / conflicting の場合は拒否経路を維持する。
- [ ] 固定語彙、benchmark row id、expected phrase 固有分岐を追加していない。
- [ ] 対象 API test / typecheck / `git diff --check` が pass する。

...
- [ ] `AgentRuntimeProvider`, `AgentModelSelection`, `AsyncAgentRun`, `AgentWorkspaceMount`, `AgentArtifact`, `SkillDefinition`, `AgentProfileDefinition`, `AgentExecutionPreset` の provider-neutral schema/type が追加されている。
- [ ] `agent:*`, `skill:*`, `agent_profile:*`, `agent_preset:*` permission と role preset、route metadata、`access-control-policy.test.ts` が更新されている。
- [ ] `/agents/runs` の create/list/get/cancel と read-only artifact metadata API が追加され、OpenAPI に反映されている。
- [ ] provider 実行は実装せず、disabled/not configured/unavailable では mock run や架空 artifact を作らず `not_configured` または `provider_unavailable` の failed/blocked run を返す。
- [ ] selected folder/document readOnly、writeback target full、run ownership / managed read resource condition が docs/test/route metadata に固定されている。
- [ ] worker contract は J2 の `{ runId }` 互換を維持し、`agentRunId` と `runId` が同一値または alias として明示されている。
- [ ] Web は empty / permission denied / not configured / read-only run detail を最小表示し、架空 provider/run/artifact/固定コストを表示しない。
- [ ] provider credentials / workspace execution / writeback 適用 / Claude Code / Codex / OpenCode 本実行は scope-out として残されている。
- [ ] 作業レポートを `reports/working/` に追加し、task を PR コメント後に `tasks/done/` へ移動している。

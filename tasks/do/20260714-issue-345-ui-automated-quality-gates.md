...
## 2026-07-18 current main 収束

### 対象と判断

- current main: `origin/main@8a427a24`
- 更新対象: 最上流 draft PR #361。新規 PR は追加しない。
- 後続 dependency: PR #381 は #361 branch を base とし、PR #385 は #381 branch を base とする。#361 更新後も後続 branch の再統合と再検証は別途必要であり、自動的に完了扱いしない。
- 並行累積差分: PR #429 は #408〜#427 の累積差分を含み、#361 と `Taskfile.yml`、`NFR-018`、`DES_UI_UX_001` が重複する。owner が merge 順を確定するまで本 task から #429 の差分を取り込まない。

### 受け入れ条件

- [x] published history を書き換えず、`origin/main@8a427a24` を PR #361 branch へ非破壊に統合する。
- [x] merge 後の PR #361 差分が UI quality gate、3件の contrast remediation、正本文書・task・report に限定され、main 側の upload/RAG safety、auth、permission、RAG behavior を変更しない。
- [ ] `git diff --check`、Web lint/typecheck/unit、API requirements coverage、Chromium UI quality、Firefox/WebKit cross-browser、docs check が成功する。
- [ ] PR #361 の最新 head CI と semver check を確認し、PR 本文・受け入れ条件コメント・セルフレビューを更新する。
- [ ] PR #381/#385 と #408〜#429 の未収束・重複、`OQ-UI-001`、manual screen reader / real-browser zoom / real-device を未完了として Issue #345 に記録する。

### 検証状況（2026-07-18）

- merge commit `42db3bc3` で `origin/main@8a427a24` を非破壊に統合し、内容競合はなかった。merge 時点の `origin/main...HEAD` は main 側 behind 0、PR 側 ahead 32。
- `git diff --check`: pass。
- `npm run lint`: pass。
- `npm run typecheck -w @memorag-mvp/web`: pass。
- `TZ=UTC npm test -w @memorag-mvp/web`: 61 files / 442 tests pass。初回は実行環境のローカル時差により日付表示2件が前日となったため、CI と同じ UTC で再検証した。
- API requirements coverage: `apps/api` を cwd として対象ファイルを直接実行し、1 test pass。workspace の package script は引数指定時も全 API suite を追加実行するため、最小検証では使用しなかった。
- docs check: `validate_docs.py`、OpenAPI、API code docs、UI trace、Web/infra inventory、hidden Unicode の全検査が pass。OpenAPI の npm script は sandbox の `tsx` IPC socket が `EPERM` となったため、同一モジュールを `node --import tsx` で実行した。
- local E2E: browser binary がなく、一時領域への Playwright browser download は取得元の0 byte応答で失敗した。権限拡張は行わず、Chromium / Firefox / WebKit は latest head の GitHub Actions 判定待ちとする。2026-07-16 の前 head では Chromium 9 / 9、Firefox / WebKit 2 / 2 pass 済みだが、今回の受け入れ条件をその証跡だけで完了扱いしない。
- task 全体は `OQ-UI-001`、manual screen reader / real-browser 200%・400% zoom / touch・real-device が未完了のため `do` を維持する。

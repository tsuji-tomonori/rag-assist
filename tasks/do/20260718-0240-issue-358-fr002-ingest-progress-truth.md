...
- 保存先: `tasks/do/20260718-0240-issue-358-fr002-ingest-progress-truth.md`
- 状態: in_progress
- タスク種別: 修正
...
- [ ] AC3: 同じ running response を反復 polling しても user-visible phase が時間経過だけで進まない。
- [ ] AC4: known API stage だけを source-aware mapper で presentation state へ変換する。
- [ ] AC5: missing / unknown stage は explicit `unknown` とし、既知の詳細工程または complete へ昇格しない。
- [ ] AC6: `succeeded` + manifest のみ complete、rejected / failed / cancelled は既存 error contract を維持する。
- [ ] AC7: preparing / transferring / creatingRun は実 client operation 由来として維持し、auth / scope / manifest refresh を変更しない。
- [ ] AC8: progress panel が missing optional data を架空工程で埋めず、honest unavailable state を live region 内へ表示する。
- [ ] AC9: FR-002 canonical requirement / requirements coverage / 必要な generated Web inventory を実装と同期する。
- [ ] AC10: targeted mapper / API client / hook / component tests、Web lint / typecheck / full coverage / build、docs / inventory、task verify、source audit、root CI、pre-commit / diff check を成功させる。
...
- [ ] AC12: local HEAD / upstream / remote 一致と clean worktree を確認し、actual AWS / manual E2E、backend/schema、migration、scanner、merge、deploy、release を未実施として記録する。

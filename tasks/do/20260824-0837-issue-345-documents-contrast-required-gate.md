# Issue #345 文書画面 contrast required gate
...
`SQ-016`はtext、meaningful non-text UI、focus indicatorのcontrastと、色だけに依存しない状態表現を`AC-SQ016-004`に要求している。PR #462では文書画面のkeyboard、Chromium AX tree、loading／error／permission／retry証跡がrequired gateに入っている一方、screen固有のcontrast automated statusだけが`blocked`である。

## 目的

文書画面の代表的なtext contrast、3px focus indicator contrast、permission stateの非色依存cueをChromium required E2Eで検査し、`documents → SQ-016 → AC-SQ016-004 → E2E`の追跡を正本・authored matrix・生成文書で同期する。

## スコープ

- 文書管理regionの320 / 1280 CSS pxにおけるaxe `color-contrast`検査
- フォルダ検索入力の実computed 3px focus indicator幅・色・背景と3:1以上のcontrast検査
...
- profileの`AC-SQ016-004`
- representative screen reader、手動keyboard、実browser 200% / 400% zoom、text-only zoom、OS scaling、touch / real device
- PR #461が所有するshared UI primitiveとdocuments production componentの変更

## なぜなぜ分析

### 問題

文書画面の`AC-SQ016-004`は、required E2Eに全画面axe baselineがあるにもかかわらず、screen固有のpositive evidenceへ追跡されず`automated: blocked`である。
...
- 文書画面のフォルダ検索入力には3px `:focus-visible` indicatorが実装済みだが、contrast ratioを測定しない。
- 文書API群のHTTP 403 stateは可視textと`role=alert`を持ち、private document panelを表示しないが、contrast evidenceと同じcontractに束ねられていない。
...
axeの全画面baselineとkeyboard focus幅の証跡が別々に存在し、`AC-SQ016-004`が要求するtext・focus indicator・color independenceを、文書画面固有のpositive required contractとして一意に結ぶE2E IDが欠落している。

### 全影響範囲への対応

文書画面限定の`E2E-UI-CONTRAST-003`を追加し、320 / 1280 CSS pxのtext contrast、実computed focus indicator 3:1、permission stateの複数cueを同一証跡に束ねる。trace/matrix/正本/生成物を同期し、profileとmanual evidenceは未検証のまま維持する。

## 実装計画

1. required Chromium UI quality suiteへ文書画面contrast E2Eを追加する。
2. E2E IDをauthored traceへ登録し、文書画面の`SQ-016`追跡と`AC-SQ016-004`を接続する。
3. authored quality matrixの文書画面automated statusだけをpassへ更新する。
...
- 要求の意味や閾値は変更せず、`SQ-016`の現在の自動証跡へ文書画面contrast contractを追記する。
- `DES_UI_UX_001`のdocuments screen traceへ`AC-SQ016-004`とE2E境界を追記する。
...
- [x] `E2E-UI-CONTRAST-003`が320 / 1280 CSS pxの文書画面text contrastをaxeで検査し、`color-contrast` violation 0を要求する。
- [x] 同E2Eがフォルダ検索入力の実computed 3px focus indicatorと背景比3:1以上を検査する。
- [x] 同E2Eがpermission stateの可視text、alert semantics、private content suppressionを検査し、色だけに依存しない状態cueを証明する。
- [x] `documents → SQ-016 → AC-SQ016-004 → E2E-UI-CONTRAST-003`が正本、authored trace/matrix、生成文書で一致する。
- [x] 文書画面のautomated statusだけがpassとなり、manual / overallおよびprofileの未検証statusはblockedを維持する。
...
- `npm exec -w @memorag-mvp/web -- playwright test e2e/visual-regression.spec.ts --project=chromium --grep E2E-UI-CONTRAST-003`
...
- PR #461のdocuments production pathと競合していないか。
- 正本と生成物の所在が重複していないか。
...
- head: `79ec9f2ac1c743703dc9674d5222617afdbb3c69`
...
このtaskの自動受け入れ条件は満たしたが、Issue #345全体のmanual evidence、実browser zoom、実機、profile contrast、owner判断が未完了のため、状態は`do`を維持する。

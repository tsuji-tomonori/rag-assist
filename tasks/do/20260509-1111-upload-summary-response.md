...
- [ ] ファイルアップロード完了/取り込み API が full `DocumentManifest` をレスポンスに含めず、summary または run status のみ返す。
- [ ] `POST /documents` が大容量ファイルアップロード用途では非推奨であり、署名付き upload session + ingest run を使うべきことが API docs に明記される。
- [ ] UI の通常ファイルアップロードは `/documents/uploads` と `/document-ingest-runs` を使い、`POST /documents` に base64 file payload を送らないことがテストで確認される。
- [ ] 認証・認可境界、uploadId scope、benchmark seed / chatAttachment の条件付き permission を弱めない。
- [ ] 変更範囲に見合う API / Web test が pass する。

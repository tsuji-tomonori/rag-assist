...
- [ ] model first-token latency は orchestration ingress と、最終回答を生成した attempt の最初の非空 Bedrock content delta を同一 monotonic clock lineage で測定する。
- [ ] 現行の buffered JSON / SSE final 応答では client-visible first-token を測定済みとせず、model first-token と明確に区別する。
- [ ] versioned evidence に測定値、単位、clock/source、適用可否を再検証可能な形で保持する。
- [ ] summary → run metrics → production observation の値と sample count が同じ case evidence から導出される。
- [ ] total latency、完了時刻差分、provider aggregate、推定値を first-token latency に流用しない。
- [ ] evidence 欠損・非対応・clock mismatch・invalid/non-finite/negative・分母0を unavailable とし、zero/pass に変換しない。
- [ ] retry / multi-turn / partial failure で、異なる attempt や turn の clock を混在させない。
- [ ] owner 未承認 threshold を default policy / promotion gate に追加しない。
- [ ] RAG 根拠性、tenant、認可、trace sanitization、artifact integrity 境界を後退させない。
- [ ] benchmark期待語句、QA sample、dataset 固有分岐を production runtime へ追加しない。
- [ ] canonical docs、coverage、generated docs が source と同期する。
- [ ] local validation、implementation/lifecycle final-head CI、semver、comments、clean/upstream が揃う。
- [ ] 実 streaming provider / 実 benchmark / actual AWS を未実施なら残存 gate に記録する。

...
保存先: `tasks/do/20260716-2109-issue-359-api-transport-adr.md`
...
- [ ] REST helper、oRPC、SSE、OpenAPI、認証、error normalization、type contract、testability、bundle依存、routes/workersの現状が実ソース根拠付きで記録される。
- [ ] 3候補を比較し、1案を根拠付きでADRのAccepted decisionとして選ぶ。
- [ ] `confirmed`、`inferred`、`conflict`、`open_question`を区別し、未確定事項を決定済みにしない。
- [ ] 将来migrationの境界、順序、rollback、guard、検証gateを定義する。
- [ ] 現在の挙動、公開contract、code、generated docs/historyを変更しない。
- [ ] benchmark paths/release-audit、FR-089/CORS、Web shim/UI、PR #338 changed pathsを尊重し、一括変更を行わない。
- [ ] 選定した最小docs検証が成功し、root CIの実施/省略判断と残余riskが記録される。
- [ ] 日本語draft PR、受け入れ条件コメント、セルフレビュー、task done移動、作業レポートが完了する。

## Done条件

- [ ] 全受け入れ条件を満たす。
- [ ] ADRと作業レポートのpath・参照・状態・Markdownが検証済みである。
- [ ] stage対象を確認し、日本語gitmoji commitをpushする。
- [ ] PR lifecycleとPR更新後セルフレビューを完了する。
- [ ] 未実施検証と未確定事項を明示し、未解決failureを残さない。

...
## 2026-07-26 最新Phase A再統合

### 問題文と確認済み事実

- PR #381 final headは`e23c731c`まで進み、PR #385 final head `1a0e8a15` はbehind 86 / ahead 7、merge不可となっている。
- #381はcurrent `main@bfaf8f20`、keyboard / Chromium AX tree / touch E2E、実行可能E2E IDの正本trace同期、最新task / report / completion statusを含む。
- Phase B固有差分はproduction UI / CSS、computed audit、visual snapshot、`SQ-016` / `DES_UI_UX_001`、Web生成物、task / reportの39 filesである。
- 旧baseのまま`done`を維持すると、公開PRがmerge不可であること、正本・生成物の再検証が必要なことをtask状態が表さない。

### 根本原因と対策

根本原因は、Phase B完了後もroot Phase Aへmain統合と横断E2E trace改善が継続し、stacked PRのbaseを同期するowner / gateが自動化されていないことである。Phase Bのpublished historyは書き換えず、最新#381を2-parent mergeで取り込む。競合は最新#381の正本・生成規則を優先し、Phase B固有のproduction remediationと根拠付きcandidate分類だけを再適用する。生成文書は手編集せずgeneratorから同期する。

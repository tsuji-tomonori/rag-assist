...
- [x] `AdminWorkspace` が overview / users / roles / usage-cost / audit / alias の sections または tabs として整理されている。
- [x] Dashboard card model があり、clickable action と read-only KPI が型または構造上区別されている。
- [x] 既存 permission gate と data loader 条件を弱めず、権限外 card/panel/API load を増やしていない。
- [x] Usage/cost/audit の read-only 表示が、推定・未提供・空状態を固定値なしで明示している。
- [x] Role assign UI が変更前後の差分を表示し、理由入力・保存を実装済みのように見せていない。
- [x] 本番 UI に固定ユーザー、固定グループ、固定容量、固定コスト、demo fallback を追加していない。
- [x] Web admin tests、web typecheck、`git diff --check` を実行し、結果を記録している。実行できない検証は理由を記録している。
- [x] Web inventory が変わる場合、`npm run docs:web-inventory` と `npm run docs:web-inventory:check` を実行し、生成差分を commit している。
- [x] 作業レポートを `reports/working/` に作成し、PR に受け入れ条件確認コメントとセルフレビューコメントを投稿している。

...
- [x] `reports/bugs/` に今回の CI 失敗を説明する障害レポートが作成されている。
- [x] 障害レポートまたは関連成果物に、事実と推定を分けたなぜなぜ分析が記録されている。
- [x] CDK app が `benchmarkSourceOwner` context 未指定でも例外終了しない。
- [x] 変更範囲に見合う infra/CDK 検証が実行され、結果が記録されている。

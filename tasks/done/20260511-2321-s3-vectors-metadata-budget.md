...
- confirmed: MemoRAG CI の失敗箇所は API test の coverage gate。
- confirmed: ローカルで CI 相当の API coverage command を再実行したところ、test は全件 pass したが branch coverage が `84.98%` で閾値 `85%` を下回り exit 1 になった。
- remediation: 追加済みの `service keeps rich drawing metadata out of vector filter metadata` regression test に、`expiresAt`、`domainPolicy`、`ragPolicy`、`answerPolicy` の filterable metadata 維持確認を追加した。
- validation: 同じ API coverage command を再実行し、branch coverage が `85.06%` となり pass した。
...
- PR: https://github.com/tsuji-tomonori/rag-assist/pull/267
- 受け入れ条件確認コメント: 投稿済み
- セルフレビューコメント: 投稿済み

...
- [x] deployed SPA runtime configを`/api`へ切り替える。
- [x] deployed config helperとexecute-api非漏洩assertionを追加する。
- [x] relative/absolute baseのURL join helperを追加しHTTP/oRPC/SSEへ適用する。
- [x] file config、dev/test VITE override、production `/api` fail-closed、malformed値、local fallback、safe joinをWeb unit testで固定する。
- [x] internal execute-api consumers / outputsを維持するnegative assertionを追加する。
- [x] snapshot / generated infra inventory / ADR / HLDを同期する。
- [x] selected validations、implementation-head GitHub Actions、PR comment、task/report lifecycleを完了する。

## 受け入れ条件

- [x] deployed SPA `config.json.apiBaseUrl` はexact `/api`である。
- [x] deployed SPA runtime configのJSONに`execute-api`、stage URL、旧API domainが含まれない。
- [x] file config `/api`から`/api/documents`、`/api/rpc`、`/api/chat-runs/.../events`を二重slashなしで生成できる。
- [x] dev/testのexplicit `VITE_API_BASE_URL` absolute/local overrideはfile configより優先され、末尾slashを安全に正規化する。
- [x] productionはVITE/fileのexecute-api・任意cross-origin absolute base、config fetch失敗、未設定、malformed値をcanonical `/api`へfail closedに解決し、localhostへ接続しない。
- [x] benchmark / CodeBuild internal `API_BASE_URL`とstack API outputsはexecute-api URLを維持する。
- [x] WebSocket / Hosted UI / direct origin restrictionを実装済みと扱わない。
- [x] API auth / permission / tenant / resource / RAG safetyを変更しない。
- [x] Web/infra tests、snapshot/generated docs、canonical docsが同期する。
- [x] selected local validationsとimplementation-head GitHub CIが成功する。final-head CIはtask/report commit push後の外部gateとして確認する。
...
- implementation-head GitHub Actions: run `29507042880` success、semver run `29507051978` success
- local selected validations: Web 448/448、infra 42/42、lint/typecheck/build/docs/inventory/hidden Unicode/`git diff --check` pass
- malicious execute-api VITE値を注入したproduction build後、bundleの`execute-api`検索0件
- local root `npm run ci`はsandboxのtsx IPC / loopback listen `EPERM`と明示承認なしのため未実施。full判定はGitHub Actionsを使用した。
- 実AWS previewは未実施。CloudFront経由の認証、SSE、large payload、API error status/bodyはstacked deploy後の確認事項として残す。
- final-head CIは本task/report完了commitをpush後に確認し、PR top-level commentへ記録する。

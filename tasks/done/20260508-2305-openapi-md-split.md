...
- [ ] `docs/generated/openapi.md` が上位ドキュメントとして API 一覧と個別ファイルへのリンクを持つ。
- [ ] API ごとの詳細 Markdown が `docs/generated/openapi/` 配下に生成される。
- [ ] 各 API 詳細 Markdown は headers、path parameters、query parameters、data、responses を表形式で記載する。
- [ ] `docs/generated/openapi.json` が git 管理対象から外れ、生成 script / workflow が JSON commit を前提にしない。
- [ ] `docs:openapi:check` は引き続き説明不足を検出する。

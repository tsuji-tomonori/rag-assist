...
`tasks/done/20260507-2012-api-c1-coverage-improvement.md`
...
- 決定事項: API C1 85% gate は達成済みで、package と CI の coverage command を `--branches 85` に更新した。
- リスク: C1 は 85.01% と閾値に近いため、今後 API 分岐を追加する場合は同時に分岐 test を追加する必要がある。
- リスク: coverage だけを目的に低価値な test を増やすと保守性が下がるため、今後も認可・RAG・外部依存 failure の重要分岐を優先する。

## 進捗メモ

- 2026-05-10: 専用 worktree `codex/api-c1-coverage-improvement` で着手し、task を `tasks/do/` に移動した。
- 2026-05-10: `analyze-input.ts`、`execute-computation-tools.ts`、`agent/trace.ts`、`text-extract.ts`、`mock-bedrock.ts`、`user-directory.ts`、`memorag-service.ts` の一部重要分岐に unit test を追加した。
- 2026-05-10: targeted test `npm exec -w @memorag-mvp/api -- tsx --test src/agent/nodes/node-units.test.ts src/rag/text-processing.test.ts src/adapters/user-directory.test.ts src/rag/memorag-service.test.ts` は pass。
- 2026-05-10: 最新の全体 coverage 実測は `npm run test:coverage -w @memorag-mvp/api` pass、C0 statements 91.75%、C1 branches 82.06%、functions 92.32%、lines 91.75%。C1 85% には未達のため、`test:coverage` と CI の `--branches 85` gate 化、PR 作成、task done 移動は未実施。
- 2026-05-10: 追加 test と到達不能な mock defensive branch の整理により、`npm run test:coverage -w @memorag-mvp/api` が `--branches 85` gate で pass。実測は C0 statements 92.64%、C1 branches 85.01%、functions 92.76%、lines 92.64%。
- 2026-05-10: `memorag-bedrock-mvp/apps/api/package.json` と `.github/workflows/memorag-ci.yml` の API coverage command を `--branches 85` に更新した。
- 2026-05-10: `npm run typecheck -w @memorag-mvp/api` と `git diff --check` は pass。
- 2026-05-10: PR #241 を作成し、受け入れ条件確認コメントとセルフレビューコメントを投稿した。受け入れ条件を満たしたため task を `done` へ移動する。
- 2026-05-10: PR #241 の CI 失敗を受けて `origin/main` を merge し、API typecheck/build、benchmark seed delete authorization test、production config hardening coverage、chat run event stream 分岐を追従修正した。
- 2026-05-10: 再検証として `npm exec -w @memorag-mvp/api -- tsx --test src/contract/api-hardening.test.ts src/contract/api-contract.test.ts`、`npm exec -w @memorag-mvp/api -- tsx --test src/contract/chat-run-events-stream.test.ts`、`npm run test:coverage -w @memorag-mvp/api`、`npm run typecheck -w @memorag-mvp/api`、`npm run build -w @memorag-mvp/api`、`git diff --check` を実行し、すべて pass。coverage 実測は C0 statements 92.70%、C1 branches 85.01%、functions 92.93%、lines 92.70%。

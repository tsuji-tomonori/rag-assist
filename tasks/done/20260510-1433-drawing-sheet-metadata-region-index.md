...
保存先: `tasks/done/20260510-1433-drawing-sheet-metadata-region-index.md`
...
- [x] AC1: titleblock 由来の `drawing_no`、`sheet_title`、`scale` が構造化 metadata として保存される。
- [x] AC2: legend / table / note / detail region が page + bbox 付きで検索候補になる。
- [x] AC3: 建築図面 QARAG の titleblock / legend 系 seed QA で根拠 page または bbox を評価できる。
- [x] AC4: 既存の一般文書 RAG の API 互換性を壊さない。

## 完了メモ

- `architecture-drawing-qarag` prepare で source ごとの `drawingSheetMetadata` と `drawingRegionIndex` を sidecar metadata として生成するようにした。
- benchmark seed upload whitelist と vector metadata 伝搬を拡張し、通常文書 upload の認可境界は広げていない。
- `drawingRegionIndex` の bbox は seed QA 由来の初期候補であり、実測 OCR crop ではない。精密 bbox は後続 task で扱う。
- PR: https://github.com/tsuji-tomonori/rag-assist/pull/246

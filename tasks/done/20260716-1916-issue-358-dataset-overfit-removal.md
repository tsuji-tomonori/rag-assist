...
- [x] production evidence 採用経路に SWEBOK dataset 固有語句・dataset 固有分岐が残っていない。
- [x] benchmark expected 語句、QA sample 固有値を production 実装へ追加していない。
- [x] 別 corpus、言い換え、英語の同型質問で同じ domain-neutral policy が成立する。
- [x] 無関係文書を evidence として採用しない negative test が通る。
- [x] dataset 固有語句を production 対象へ再導入すると静的検査または test が失敗する。
- [x] 根拠不足 refusal、citation、support verification、認可済み chunk の境界を弱めていない。
- [x] `FR-045`、`SQ-003`、関連設計、coverage／generated docs が実装・test evidence と同期している。
- [x] `tasks/todo/20260506-1203-requirements-classification-policy.md` が実態に合わせてクローズされる。
- [x] 本差分に直接関係する validation が pass し、broader test の sandbox failure は clean `origin/main` と比較して同一 failure set であることを確認し、理由とリスクを記録する。
- [x] main 向け PR を GitHub Apps 優先で作成し、応答停止時は承認済み fallback を使って受け入れ条件確認とセルフレビューの日本語 top-level comment を投稿する。
...
- targeted tests／関連 graph／requirements coverage／typecheck／build／docs underlying checks／pre-commit: pass
- API broader: branch 107 file 中 102 pass／5 sandbox failure。clean `origin/main` snapshot でも同じ5 fileが `tsx` IPC／listen `EPERM` で失敗し、本差分との因果なしを確認。
- benchmark broader: branch／clean main とも 22 file 中 19 pass／3 localhost listen `EPERM`。
- GitHub Apps: PR 作成 call が長時間応答停止したため、親 workflow の指示に従い `gh` fallback で PR／comment を作成。
- 作業レポート: `reports/working/20260716-1946-issue-358-dataset-overfit-removal.md`

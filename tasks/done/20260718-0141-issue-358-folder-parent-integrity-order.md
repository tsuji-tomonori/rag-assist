...
- 保存先: `tasks/done/20260718-0141-issue-358-folder-parent-integrity-order.md`
- 状態: done
- タスク種別: セキュリティ修正
...
- [x] AC3: missing parent を持つ folder の administrative principal が既存 `hasFolderCycle` 経路で `none` / `resource_integrity_unverified` となることを回帰固定する。
- [x] AC4: archived / cross-tenant parent を持つ folder の administrative principal を explicit parent integrity 経路で `none` / `resource_integrity_unverified` とする。
- [x] AC5: root folder と active same-tenant parent を持つ folder の有効な administrative principal は `full` を維持する。
- [x] AC6: cycle、identity、account、actor/resource tenant、folder lifecycle の mandatory deny を admin `full` より優先したまま維持する。
- [x] AC7: non-admin の parent inheritance / explicit policy と mutation / audit / cleanup contract を変更しない。
- [x] AC8: FR-061 / FR-077、requirements coverage、必要な generated docs を同期し、public schema / OpenAPI / UI / infra / IAM 非変更を記録する。
- [x] AC9: targeted folder permission / requirements coverage、API typecheck / full coverage、docs check / source audit、task verify、repository full CI、pre-commit / diff check を成功させる。
- [x] AC10: report、目的別 commit、Draft stacked PR、`semver:patch`、日本語 body / AC / self-reviewを完遂する。
- [ ] AC11: implementation / final-head CI、Issue #358 progress、clean / upstream / remote 一致を最終 external evidence へ記録する。
- [x] AC12: actual AWS / manual evidence、migration、scanner、merge、deploy、release を未実施として記録する。

## 実装 external evidence
...
- implementation-head CI: run `29598881190`、head `2a4f75cd7e3e45bf0b29d24cbea8e80bd2fce000`、success
- final-head CI、最終コメント、Issue #358 progress、clean / upstream / remote 一致は本 lifecycle commit 後に確認する。

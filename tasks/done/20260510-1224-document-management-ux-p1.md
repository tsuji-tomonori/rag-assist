...
- [x] ファイル名検索、種別フィルタ、状態フィルタ、所属フォルダフィルタ、ソートを文書一覧で利用できる。
- [x] フィルタ結果件数と、該当なし empty state が表示される。
- [x] 文書行クリックで詳細 drawer が開き、API / props 由来の文書 metadata を表示できる。
- [x] API が提供しない metadata は架空値で埋めず、未設定または利用不可として表示する。
- [x] documentId をコピーする操作を提供する。
- [x] 共有設定で shared groups の差分 preview と重複 / 空値 validation が表示される。
- [x] 既存の upload / delete / reindex P0 UX が破綻しない。
...
- `npm ci`: pass。npm audit は 3 件の脆弱性を報告したが、依存更新は範囲外。
...
- https://github.com/tsuji-tomonori/rag-assist/pull/238
- 受け入れ条件確認コメント: posted
- セルフレビューコメント: posted

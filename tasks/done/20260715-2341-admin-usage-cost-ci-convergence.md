...
- [x] 最新 `origin/main` を取り込み、stacked branch の差分を `main` 向けへ収束する。
- [x] 競合、生成物、認可境界、Usage / Cost の真正性を再確認する。
- [x] 変更範囲に見合う検証を実行し、失敗があれば修復して再実行する。
- [x] PR 本文、受け入れ条件コメント、セルフレビューを最新 head に同期する。

## Done 条件

- [x] PR #357 の base が `main` で、最新 `origin/main` を含む。

...
- [ ] merge conflict marker が残っていない。
- [ ] 旧 `hybrid-search` / `search-evidence` は main 側の shim 形に揃っている。
- [ ] PR #326 の ACL fail-closed / folder permission 判定が新配置 module に反映されている。
- [ ] `npm run typecheck -w @memorag-mvp/api` が pass する。
- [ ] 関連 API / RAG focused tests が pass する。
- [ ] PR に受け入れ条件確認とセルフレビューを日本語コメントで追記する。

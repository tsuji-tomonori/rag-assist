...
3. CodeBuild/benchmark 用に runner fatal を避ける修正を実装する。
...
- [x] CodeBuild 障害の概要、影響、原因仮説、是正策を含む障害レポートが `reports/bugs/` に作成されている。
- [x] Textract OCR fallback の 45 秒固定待機が、MMRAG DocQA の corpus seed で過度に早く失敗する原因に対して修正されている。
- [x] 変更範囲に対応するテストまたは型チェックが実行され、結果が記録されている。
- [x] 実施していない検証がある場合は、未実施理由がレポートと最終回答に明記されている。
- [x] 作業完了レポートが `reports/working/` に作成されている。

## 完了結果

- 障害レポート: `reports/bugs/20260507-2029-mmrag-textract-timeout.md`
- 作業完了レポート: `reports/working/20260507-2029-fix-mmrag-textract-timeout.md`
- PR: https://github.com/tsuji-tomonori/rag-assist/pull/159
- PR 受け入れ条件コメント: posted
- PR セルフレビューコメント: posted
- 未実施検証: AWS CodeBuild 上の `mmrag-docqa-v1` 全量再実行は未実施。
...
- OCR timeout になった PDF とそれを期待資料にする row は評価対象から除外されるため、全 PDF を必ず評価したい場合は非同期 ingestion job 化または OCR timeout 設計の見直しが必要。

...
- [x] `npm run docs:infra-inventory` で infra resource inventory Markdown/JSON が生成される。
- [x] `npm run docs:infra-inventory:check` で生成物の drift を検出できる。
- [x] Markdown に CloudFormation Type 別個数と resource logical id 別の主要設定値が含まれる。
- [x] npm scripts、Taskfile、README、CI の実行導線が既存 web/API 生成 docs と整合する。
- [x] 変更範囲に応じた検証が実行され、未実施項目は理由付きで記録される。

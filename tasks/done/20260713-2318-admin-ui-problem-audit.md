...
- セルフレビュー: https://github.com/tsuji-tomonori/rag-assist/pull/344#issuecomment-4964510575
- 受け入れ条件確認: https://github.com/tsuji-tomonori/rag-assist/pull/344#issuecomment-4964510488
...
- [x] R6: 適用可能な docs validator/freshness、hidden Unicode、pre-commit、`git diff --check` と最終 content head の GitHub CI が成功している。task/report のみを更新する metadata head の CI は merge gate として別途確認する。
- [x] R7: PR 本文、`semver:patch` label、日本語の受け入れ条件確認、セルフレビューが作成され、この task done 更新が同じ PR branch に含まれている。metadata head の最終コメントは merge 前に追加する。

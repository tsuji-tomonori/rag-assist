...
- [x] `npm run docs:openapi` または Taskfile から OpenAPI JSON と Markdown を生成できる。
- [x] 生成先が `memorag-bedrock-mvp/docs/generated/` に固定され、手作業編集ではなく再生成できることが分かる。
- [x] GitHub Actions で生成 docs の更新 PR を作成できる。
- [x] README または設計 docs に運用方法が記載されている。
- [x] 変更範囲に見合う検証が実行され、未実施項目があれば理由が記録されている。

...
- [x] `rag:group:create` が `canCreateDocumentGroups` として UI に伝播し、フォルダ作成のガードに使われる。
- [x] `rag:group:assign_manager` が `canShareDocumentGroups` として UI に伝播し、共有・フォルダ設定更新のガードに使われる。
- [x] 文書一覧の `+` はフォルダ作成モーダルを開き、アップロードは独立したボタンから実行できる。
- [x] 選択中フォルダでフォルダ作成を開いた場合、そのフォルダが親候補として初期設定される。
- [x] 右側の常設設定パネルをやめ、設定は必要時だけモーダルで開閉できる。
- [x] 本番 UI に固定の架空データや未実装操作を追加しない。
- [x] 変更範囲に見合う Web テスト・型チェック・差分チェックを実行し、未実施があれば理由を記録する。
- [x] 作業完了レポートを `reports/working/` に作成する。

## 完了メモ

- PR: https://github.com/tsuji-tomonori/rag-assist/pull/326
- 受け入れ条件確認コメント: 投稿済み。
- セルフレビューコメント: 投稿済み。
- ブラウザ手動確認と E2E は未実施。理由は PR コメントと作業レポートに記録済み。

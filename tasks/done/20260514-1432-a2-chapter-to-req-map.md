...
- [x] `FR-049` から `FR-055` が API の requirements coverage 静的テストで検出・登録されている。
  - 根拠: `npm exec -w @memorag-mvp/api -- tsx --test src/rag/requirements-coverage.test.ts` pass、API coverage コマンド pass。
...
- 実行コマンド:
  ...
  - `git diff --check`
  - `npm exec -w @memorag-mvp/api -- tsx --test src/rag/requirements-coverage.test.ts`
...
- 期待結果:
  ...
  - planning REQ が API の requirements coverage 静的テストで欠落扱いにならない。

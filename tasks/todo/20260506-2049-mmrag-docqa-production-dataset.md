...
- 決定済み default selection: `MMLongBench-Doc` train split 全 1,091 questions。`mmrag-docqa-v1` の通常実行では subset に絞らない。
...
- `docs/4_運用_OPS/21_監視_MONITORING/OPS_MONITORING_001.md`: corpus seed、multimodal assets、結果解釈、baseline 比較、ローカル実行手順と必要環境変数を更新する。
...
- [ ] dataset row が ground-truth answers、期待ファイル、期待 document、fact slots、multimodal 正解条件を持つ。
  - 現状: 部分充足。`benchmark/mmrag-docqa.ts` で MMLongBench-Doc 全 1,091 rows から JSONL を生成する converter は追加済み。実 CodeBuild run と baseline 保存は未完了。

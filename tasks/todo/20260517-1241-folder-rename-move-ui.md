...
- 優先度: P0。backend の rename / move 能力を製品導線へ出す最初の UI task。
- 依存関係: PR #321 の canonical path / path lock 実装、現行 folder tree / create folder UI。
- 推奨 PR 分割:
  - PR 1: `useDocuments` と API client の update payload 型同期。
  - PR 2: rename / description update UI。
  - PR 3: parent folder move UI と selected/upload destination state 更新。
- 成功指標: UI 操作後に API 補完済み `canonicalPath` が表示され、重複 path error が利用者に伝わる。

## 実装設計メモ

- `/document-groups/{groupId}/share` を使う場合でも、Web 側の関数名は `updateDocumentGroup` など意味ベースに寄せる。
- move target は既存 folder tree 由来の実データだけを候補にする。
- move dialog は target folder、変更後 path、影響する子孫数、upload destination への影響を表示する。
- API error は validation、permission、conflict、subtree too large を区別して表示する。

## 追加確認観点

- selected folder が rename / move された場合、URL state と upload destination が stale path を保持しない。
- folder search は rename 後の `name` と `canonicalPath` を検索対象にする。
- 共有更新 modal と rename / move modal の state が相互に上書きされない。

## 未確定点

- rename / move を同じ modal にまとめるか、folder action menu から別操作にするか。
- `/share` endpoint をそのまま使うか、API 整理を同時に行うか。

...
- 優先度: P2。大規模運用向けで、通常 move UI と GSI query 活用後に着手する。
- 依存関係: move API、path lock consistency、run store、operation audit。
- 推奨 PR 分割:
  - PR 1: move plan / dry-run API。
  - PR 2: async run state machine と lock acquisition。
  - PR 3: Web progress UI と recovery / verify command。
- 成功指標: transaction limit を超える subtree でも、進捗と失敗復旧を持って path 更新できる。

## 実装設計メモ

- async move は plan 作成時点の subtree snapshot と expected updatedAt を保存する。
- lock acquisition、group update、old lock cleanup は段階ごとに idempotent にする。
- 同一 subtree / ancestor / descendant の concurrent move は拒否または serialized にする。
- UI は受付完了を move 完了として表示しない。

## 追加確認観点

- retry で同じ lock item を二重作成しない。
- cancel は安全な checkpoint でのみ許可する。
- failed run の verify / repair 手順が残る。

## 未確定点

- 既存 async run store を流用するか、folder operation 専用 run store を作るか。
- 大規模 move の最大許容件数と timeout。

...
- CI の API branch coverage が 84.99% で再度閾値未達となったため、unknown provider の `provider_unavailable` blocked run と failed worker result の分岐テストを追加。
...
G1 で追加した Web/API feature の実挙動だけをテストし、production path に架空 provider/run/artifact/cost は追加していない。存在しない artifact run は正直に 404 を返すようになり、CI coverage 閾値はローカルで満たした。

## 未対応・制約・リスク

- `npm ci` 実行時に npm audit notice が表示されたが、この作業では依存更新を行っていない。
- PR #313 の CI で API branch coverage が 84.99% となったため、追加テスト後に API coverage を再実行し、origin/main rebase 後も Branches 85.28% で通過した。

...
既存`E2E-UI-LAYOUT-STRESS-001`は320×720 CSS pxを3 browserのPR必須gateで実走するが、お気に入りは確認済み0件だけである。production変更なしでtest-only fixtureを実データ極端値へ拡張し、同一journeyで0件への再取得も維持するのが、既存作業と重複しない最小の高優先sliceである。
...
1. 採用: 既存layout stressへ24件・8 target type・長い文字列・アクセス不可cueを追加し、その後の確認済み0件も同一journeyで検証する。
...
- `conflict`回避: #461が変更する`FavoritesWorkspace.tsx`を編集しない。

...
- CI の lint 指摘に対応し、テスト用 helper を component file から `documentWorkspaceUtils.ts` へ移動した。
- API contract test の `parentGroupId` 初期代入を削除し、`assert.ok(parentGroupId)` で作成結果を明示した。
- `npm run docs:web-inventory` で generated web inventory を更新した。
...
- `apps/web/src/features/documents/components/workspace/documentWorkspaceUtils.ts`

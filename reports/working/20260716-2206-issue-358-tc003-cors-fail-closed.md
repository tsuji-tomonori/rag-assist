...
- draft PR #374を作成し、`semver:patch` を付与した。GitHub Appsは1回の呼び出しが60秒でtimeoutし、head branchにPRがないことを確認してから`gh` fallbackを使用した。
- 日本語の受け入れ条件コメントとセルフレビューをtop-level commentとして投稿し、blocking指摘なしを確認した。
- implementation-head GitHub Actions run `29501173928` はmain CI success、semver label check success、promotion gate skipだった。
- taskを`tasks/done/`へ移すlifecycle commitのpush後、latest-head CIを成功まで監視し、結果をPR commentへ記録する。

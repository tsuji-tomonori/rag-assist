...
- main 取り込み後の PR CI で API branch coverage が 85% 未満になったため、async agent route/service/worker の test を補強し、存在しない run の artifact list が 404 にならず空配列 200 を返す service 実装を修正した。
- 追加 CI で API branch coverage が 84.99% となり 1 分岐だけ閾値未達だったため、service の owner list、forbidden cancel、missing execution、未登録 provider、readable document selection / duplicate normalization の coverage を追加補強した。
...
- `npm exec -w @memorag-mvp/api -- c8 --check-coverage --statements 90 --branches 85 --functions 90 --lines 90 --reporter=text-summary --reporter=json-summary tsx --test src/**/*.test.ts src/**/**/*.test.ts` (Statements 94%, Branches 85.11%, Functions 93.99%, Lines 94%)

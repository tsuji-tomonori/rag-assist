...
- The `Candidate tasks`, `Acceptance criteria`, `E2E and non-UI scenarios`, and `Operation and expectation groups` sections of the task-scoped specification analysis report
...
Create or update one canonical requirement file per requirement under `docs/1_要求_REQ/`. Preserve the repository's category and filename conventions and include the requirement-specific acceptance criteria in the same file.
...
Route each approved specification by concern: requirement and acceptance conditions to `docs/1_要求_REQ/`, architecture/ADR to `docs/2_アーキテクチャ_ARC/`, implementation contracts to `docs/3_設計_DES/`, and current monitoring/verification to `docs/4_運用_OPS/21_監視_MONITORING/`. Do not create a second specification hierarchy.

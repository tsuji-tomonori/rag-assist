...
test("deploys the tenant-scoped security audit reconciliation worker with bounded audit and membership read authority", () => {
  ...
  const documentShareStatements = Object.entries(resources)
    .filter(([logicalId, resource]) => logicalId.startsWith("SecurityAuditReconciliationFunctionServiceRoleDefaultPolicy") && (resource as any).Type === "AWS::IAM::Policy")
    .flatMap(([, resource]) => (resource as any).Properties.PolicyDocument.Statement as any[])
    .filter((statement) => JSON.stringify(statement.Resource).includes("documents/share-grants"))
  assert.equal(documentShareStatements.length, 1)
  assert.equal(documentShareStatements[0].Action, "s3:GetObject")
  assert.match(JSON.stringify(documentShareStatements[0]), /AWS::AccountId/)
  assert.match(JSON.stringify(documentShareStatements[0]), /documents\/share-grants\.json/)
  assert.doesNotMatch(JSON.stringify(documentShareStatements[0]), /s3:(?:ListBucket|PutObject|DeleteObject)/)

  const folderMoveStatements = Object.entries(resources)
    .filter(([logicalId, resource]) => logicalId.startsWith("SecurityAuditReconciliationFunctionServiceRoleDefaultPolicy") && (resource as any).Type === "AWS::IAM::Policy")
    .flatMap(([, resource]) => (resource as any).Properties.PolicyDocument.Statement as any[])
    .filter((statement) => JSON.stringify(statement.Resource).includes("folder-mutations/move"))
  assert.equal(folderMoveStatements.length, 1)
  assert.equal(folderMoveStatements[0].Action, "s3:GetObject")
  assert.match(JSON.stringify(folderMoveStatements[0]), /tenant-artifacts.*folder-mutations\/move/)
  assert.doesNotMatch(JSON.stringify(folderMoveStatements[0]), /s3:(?:ListBucket|PutObject|DeleteObject)/)

  const documentMoveStatements = Object.entries(resources)
    .filter(([logicalId, resource]) => logicalId.startsWith("SecurityAuditReconciliationFunctionServiceRoleDefaultPolicy") && (resource as any).Type === "AWS::IAM::Policy")
    .flatMap(([, resource]) => (resource as any).Properties.PolicyDocument.Statement as any[])
    .filter((statement) => JSON.stringify(statement.Resource).includes("document-mutations/move"))
  assert.equal(documentMoveStatements.length, 1)
  assert.equal(documentMoveStatements[0].Action, "s3:GetObject")
  assert.match(JSON.stringify(documentMoveStatements[0]), /AWS::AccountId/)
  ...
  assert.doesNotMatch(JSON.stringify(listStatements), /documents\/share-grants|folder-mutations\/move|document-mutations\/move|tenant-artifacts.*manifests/)

...
Options:
  ...
  --role ROLE                   Cognito group or Japanese role name to assign.
                                ...
                                BENCHMARK_RUNNER, ASYNC_AGENT_USER, SKILL_PROFILE_ADMIN, ASYNC_AGENT_ADMIN,
                                USER_ADMIN, ACCESS_ADMIN, COST_AUDITOR, SYSTEM_ADMIN.
                                Japanese role names: 一般利用者, 回答担当者,
                                RAGグループ管理者, 性能テスト担当者, ベンチマークAPI実行サービス,
                                ベンチマーク実行者, 非同期エージェント利用者, スキル・プロファイル管理者,
                                非同期エージェント管理者, ユーザー管理者, アクセス管理者, コスト監査者, システム管理者.
...
normalize_role() {
  case "$1" in
    ...
    ASYNC_AGENT_USER | 非同期エージェント利用者) echo "ASYNC_AGENT_USER" ;;
    SKILL_PROFILE_ADMIN | スキル・プロファイル管理者) echo "SKILL_PROFILE_ADMIN" ;;
    ASYNC_AGENT_ADMIN | 非同期エージェント管理者) echo "ASYNC_AGENT_ADMIN" ;;

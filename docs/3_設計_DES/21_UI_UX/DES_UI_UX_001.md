...
- `E2E-UI-LAYOUT-STRESS-001` は320×720 CSS px、`prefers-reduced-motion: reduce`で長文回答の先頭／末尾、長い引用、担当者対応32件／4 laneの長いtitle・質問・部署名とDOM末尾カードのviewport到達、ドキュメント30件／6 file typeの長い先頭／末尾ファイル名・50件表示時の全件表示・末尾viewport到達、履歴35件の長い先頭／末尾title・末尾viewport到達、お気に入り24件／8 target typeの長いlabel・target ID／アクセス不可cue／末尾到達と再取得後の確認済み0件を表示し、chat / assignee / documents / history / favoritesのdocument rootと対象regionの水平containmentをChromium／Firefox／WebKitのPR required gateで検証する。
- test-only認証、chat startRun／SSE、assignee、documents、history、favorites fixtureはPlaywright routeに限定する。JSON attachmentはbrowser project名、viewport、fixture量、担当者lane数、文書表示件数、担当者／文書／履歴／お気に入りの末尾矩形、URL／dimensions、automation境界を持ち、browser別に一意なattachment名とする。

...
| `documents` | `/documents`; `/documents/groups/:id`; `/documents/:id`; `/documents/reindex-migrations/:id`; `?view=documents&...` accepted and normalized | `canReadDocuments` | `operator`, `system-admin` | `JOB-UI-DOCUMENTS`: find/select/upload/share/manage/ask within effective permission | `FR-001`, `FR-002`, `FR-038`, `FR-064`, `FR-094`〜`FR-098`, `SQ-016`; `AC-FR001-001`, `AC-FR001-008`, `AC-FR094-002`, `AC-FR097-001`〜`005`, `AC-FR098-001`〜`005`, `AC-SQ016-003` | `E2E-VIEW-DOCUMENTS-001`, `E2E-UI-NAV-002`, `E2E-UI-ROUTE-001`, `E2E-UI-ROUTE-002`, `E2E-UI-STATE-001`, `E2E-UI-RISK-001`, `E2E-UI-DOCUMENTS-001`, `E2E-UI-SR-SEMANTICS-001` | implemented: authorized current context、page restoration、safe normalization、progressive disclosure、many/long/320〜1280px、required catalog/reindex loading/partial/permission/retry/false-zero、Chromium AX name / role / value / selected state evidence exist; documents keyboard journey、manual screen-reader/zoom/real-device evidence remain |
...
## Automated UI quality gate（2026-08-16更新）
...
### Documents semantic contract

- `E2E-UI-SR-SEMANTICS-001`は文書workspace / breadcrumb / folder tree / file list / current context、folder / filename search、type / status / folder / sort / page-size value、named file table、文書固有detail action、selected row、detail dialog /主要actionをCDP `Accessibility.getFullAXTree`で検証し、JSONをevidenceとして添付する。
- test-only documents / document-groups / reindex-migrations fixtureはPlaywright routeへ限定し、production component、API、effective permission、文書mutation契約を変更しない。Chromium AX tree自動証跡はdocuments keyboard journey、representative screen reader、実browser 200% / 400% zoom、touch / real device、Firefox / WebKit accessibility treeの実測を代替しない。

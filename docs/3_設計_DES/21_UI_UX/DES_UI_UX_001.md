...
- `E2E-UI-STATE-001` is part of the required Chromium UI quality gate and exercises history / favorites initial loading/500/confirmed-empty recovery、actual HTTP 403、benchmark run/suite part loading/500/confirmed-empty recovery and HTTP 403、admin partial recovery、source/as-of stale recovery. Primitive/controller/component tests cover all state variants and false-zero rules; the other four AppViews remain blocked in the cross-screen content-state matrix until feature evidence exists.

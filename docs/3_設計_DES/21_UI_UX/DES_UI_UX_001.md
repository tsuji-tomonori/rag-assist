...
| `assignee` | `/?view=assignee` | `canAnswerQuestions` | `answer-editor`, `system-admin` | `JOB-UI-ASSIGNEE`: filter/select/answer/resolve with assignment and next-action context | `FR-031`〜`FR-033`, `FR-094`〜`FR-098`, `SQ-016`; `AC-FR031-001`, `AC-FR032-004`, `AC-FR094-003`, `AC-SQ016-002`, `AC-SQ016-003`, `AC-SQ016-004`, `AC-SQ016-007` | `E2E-VIEW-ASSIGNEE-001`, `E2E-UI-NAV-002`, `E2E-UI-ROUTE-002`, `E2E-UI-STATE-001`, `E2E-UI-QUESTION-001`, `E2E-UI-KEYBOARD-NAV-001`, `E2E-UI-SR-SEMANTICS-001`, `E2E-UI-CROSS-BROWSER-SEMANTICS-003`, `E2E-UI-CONTRAST-002` | implemented automated journey: five ticket states、assignment、answer handoff、input retention、locked terminal state、required loading/error/permission/retry/false-zero、Chromium／Firefox／WebKit keyboard-only filter / search / selection / answer draft journey、Chromium AX name / role / value / pressed / checked / live state、Firefox／WebKit ARIA snapshot・filter value・selected pressed・notify checked・visible polite draft status、Chromium 320 / 1280 CSS px text・3px focus indicator contrastとpermission state複数cue evidence exist; manual screen-reader/zoom/real-device evidence remains |
...
## Automated UI quality gate（2026-09-02更新）

- PR required: Chromium で representative full-page axe serious / critical、320 / 375px mobile navigation、deterministic visual regression、keyboard navigation、Chromium accessibility tree contractを実行する。加えてFirefox／WebKitでloginと主要画面のkeyboard journey、login / chat / profile / assignee / documents / admin / history / favorites / benchmarkのARIA snapshot・dynamic ARIA state contract、履歴／文書のloading／errorまたはpartial／permission／retry state contract、chat / documents / assignee / admin / profileの640 / 320 CSS px reflow proxy、およびchat / documents / history / favoritesの320 CSS px content-extreme fixtureを実行する。
...
- required cross-browser scopeはkeyboard 4件、semantic 16件、history state 4件、documents state 4件、reflow 4件、content-extreme 4件の合計36件に限定する。より広いFirefox／WebKit visual scopeはscheduled jobに分離し、重複実行しない。

...
- ファイル: `docs/3_設計_DES/11_詳細設計_DLD/DES_DLD_005.md`
- 種別: `DES_DLD`
- 作成日: 2026-05-04
- 状態: Draft
...
資料内の金額閾値条件は rule-based parser で自然文を解釈しない。retrieval / rerank 後に `extract_policy_computations` が LLM を構造化抽出器として使い、質問対象値、資料条件、条件全体表現、比較表現、効果、対象、根拠 quote、`matchesQuestion`、`confidence` を JSON で抽出する。deterministic code は JSON schema validation、質問金額表記の質問内存在確認、quote 存在確認、条件全体表現・閾値金額表記・比較表現・対象表現・効果表現の quote 内存在確認、条件全体表現内での閾値金額表記と比較表現の近接確認、active answer policy の `policyComputation` による比較表現と comparator enum の対応確認、効果表現と effect enum の対応確認、JPY 金額正規化、comparator 評価、computed fact 採番だけを担当する。
...
10. deterministic code は extraction JSON を検証し、`matchesQuestion=true`、`confidence >= ragRuntimePolicy.confidence.computedFact`、`questionTarget.amountText` が質問内に存在、quote が selected chunk 内に存在、`condition.conditionText`、`condition.thresholdText`、`condition.comparatorText`、`consequence.targetText`、`consequence.effectText` が quote 内に存在、`condition.conditionText` が `thresholdText` と `comparatorText` を同じ条件表現として含む、active answer policy の `policyComputation` 上で `condition.comparatorText` が comparator enum と対応する、`consequence.effectText` が effect enum と対応する、JPY 金額と comparator が正規化可能な候補だけを `threshold_comparison` に変換する。
...
| 資料内の金額閾値条件への該当可否 | `extract_policy_computations` で LLM が質問対象値、資料条件、条件全体表現、比較表現、効果、対象、根拠 quote を JSON 抽出する。deterministic code は schema / 質問金額表記の質問内存在 / quote / 条件全体表現・閾値金額表記・比較表現・対象表現・効果表現の quote 内存在 / 条件全体表現内での閾値金額表記と比較表現の近接 / active answer policy の `policyComputation.comparatorTextMappings` と comparator enum の対応 / `policyComputation.effectTextMappings` と effect enum の対応 / JPY / comparator / confidence / `matchesQuestion` を検証し、通過した候補だけを `threshold_comparison` として比較する |
| policy extraction が曖昧・低 confidence・quote 不一致・質問金額表記不一致・条件全体表現不一致・閾値金額表記不一致・比較表現不一致・比較ペア不一致・対象表現不一致・効果表現不一致・効果 enum 不一致・質問要件不一致 | computed fact を生成せず、通常の RAG gate / support verification に委ねる |

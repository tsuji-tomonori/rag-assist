...
- `@aws-sdk/client-sfn`: chat / ingest / benchmark execution start（benchmark cancellation の stop command mapping は `benchmark-execution-stopper.ts` へ移管）
...
| Dependencies | 31 key、broad private readonly field、Phase 4f 後の 24 direct read key と `benchmarkRunStore` の facade occurrence 7 が一致する |

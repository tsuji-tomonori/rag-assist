...
- ファイル: `docs/3_設計_DES/11_詳細設計_DLD/DES_DLD_001.md`
- 種別: `DES_DLD`
- 作成日: 2026-05-01
- 状態: Draft
...
| `clarification_gate` | question、memory cards、retrieved evidence、clarification context | needsClarification、grounded options、missingSlots、reason、confidence |
...
6. answer policy は dataset／corpus 固有 anchor、invalid answer pattern、search clue を保持せず、document metadata や質問語句で domain policy を切り替えない。未知の `RAG_DOMAIN_POLICY_ID` は起動時の policy 解決で拒否する。
...
1. `clarification_gate` は対象が未指定または曖昧な質問に対して、memory card、検索候補、会話文脈から候補 option を抽出する。
2. option は最大 `RAG_CLARIFICATION_OPTION_LIMIT` 件に制限し、各 option は文書、memory、evidence、aspect、history のいずれかの grounding を持つ場合だけ公開応答へ含める。
3. grounded option が複数あり、曖昧性 score が `ragRuntimePolicy.confidence.clarificationMinAmbiguityScore` 以上の場合は `responseType=clarification` へ分岐する。
4. 対象が明示されている質問、または grounded option を作れない質問は確認質問にせず、通常の検索評価、回答、または回答不能へ進める。
5. `includeDebug=true` の場合、`clarification_gate` の ambiguityScore、reason、groundedOptionCount、rejectedOptions を debug trace に保存する。
...
6. fallback の `RequiredFact` は低情報量 token を除いた signal phrase を primary fact として持つ。質問語彙から amount / procedure / person などの typed fact を固定 rule で作らず、typed claim conflict は検索・評価後に得られた根拠側の signal で扱う。
...
12. simple query で新規 evidence があり、top score が high-confidence threshold 以上、かつ primary fact conflict がない場合は、false missing fact による追加検索を避けて rerank 以降の gate へ進める。
13. 選択した action は `RetrievalEvaluation.nextAction` として debug trace に保存する。

### 会話 follow-up の query 制御

1. `build_conversation_state` は assistant の汎用 refusal 文と、根拠付き回答の定型前置き（例: `資料では次のように記載されています。`）を topic / entity 抽出から除外する。
2. short follow-up かつ `turnDependency=coreference` の場合、retrieval query は standalone question、元質問、必要最小限の document anchor に絞り、previous citation や entity 由来の query を過剰展開しない。
3. 通常の standalone / multi-hop / comparison 質問では、従来どおり document / entity / citation anchor を含む最大 8 件の retrieval query を許容する。
...
5. `rerank_chunks` は最終回答 context に渡す前に、rerank 後も `minScore` 未満の chunk を `selectedChunks` から除外する。
6. `rerank_chunks` は質問から動的に抽出した語彙と汎用構造 signal で evidence relevance を評価し、最低 relevance を満たす候補だけを回答 context へ渡す。該当候補がない場合は全候補への fallback を行わず、`selectedChunks=[]` として回答不能へ流す。
7. 目次らしい候補は、同じ汎用 relevance 条件を満たす本文候補がある場合に除外する。特定文書名、章名、dataset の正解語句は判定に使わない。
8. 各 action の hit count、新規 evidence 件数、top score、retrieval diagnostics は `actionHistory` と debug trace に保存する。
...
6. corpus metadata や dataset 固有の invalid answer pattern で citation 成否を上書きしない。citation ID の整合後も `verify_answer_support` で各文の支持を検証し、不支持文は `unsupported_answer` として拒否する。

### Policy computation 抽出

1. `extract_policy_computations` は selected chunk があり、かつ tool intent、または質問と根拠内の比較可能な値 signal から policy computation が必要と判断できる場合だけ実行する。
2. 通常の事実検索質問では `extract_policy_computations` を skip し、answerability / sufficient context / citation validation / support verification で根拠性を確認する。
...
- 曖昧質問で grounded option が複数ある場合だけ `responseType=clarification` を返すこと。
- 明確な質問または grounded option がない質問では確認質問を返さないこと。

...
- ファイル: `docs/3_設計_DES/11_詳細設計_DLD/DES_DLD_004.md`
- 種別: `DES_DLD`
- 作成日: 2026-05-02
- 状態: Draft
...
8. post-confirmation trigger は対象 userPoolId / username を使い、対象ユーザーを固定値 `CHAT_USER` group に追加する。event metadata や環境変数から付与 role を選択しない。
...
| post-confirmation trigger が一時失敗する | `AdminAddUserToGroup` を最大 3 回まで短い backoff で retry し、成功後だけ event を返す |
| post-confirmation trigger が継続失敗する | 成功を返さず例外を伝播し、`CHAT_USER` 未付与のまま fail closed にする。CloudWatch Logs の構造化 event を確認し、原因解消後に管理者が group を再付与する |
...
| duplicate invocation と一時/継続失敗が安全に収束する | `cognito-post-confirmation.test.ts` |
| sign-up から sign-in、認証付き API まで接続する | `authClient.test.ts` integration scenario |
| group 数が `BENCHMARK_OPERATOR` と `BENCHMARK_RUNNER` を含む現行 role 定義と一致する | CDK stack assertion |

## 監査と権限

- trigger は成功/失敗、試行回数、固定 group、User Pool、Cognito subject を構造化 log として記録し、メールアドレスや token は記録しない。
- Lambda IAM は `cognito-idp:AdminAddUserToGroup` のみに限定する。User Pool と Lambda role の CloudFormation 循環依存を避けるため resource は同一 account/region の `userpool/*` とし、付与可能 group は handler の固定値 `CHAT_USER` で狭める。
- Cognito confirmation code が不正な場合は post-confirmation trigger が起動せず、Web は session を作成しない。

...
- ファイル: `docs/3_設計_DES/41_API_API/DES_API_001.md`
- 種別: `DES_API`
- 作成日: 2026-05-01
- 状態: Draft
...
| `POST /document-groups` | 資料グループ作成 | `FR-041`, `NFR-011` |
| `POST /document-groups/{groupId}/share` | 資料グループ共有設定更新 | `FR-041`, `NFR-011` |
...
```json
{
  ...
  "searchScope": {
    "mode": "groups",
    "groupIds": ["docgrp_company_policy"],
    "includeTemporary": true,
    "temporaryScopeId": "conversation-20260508-001"
  }
}
```
...
すべての実レスポンスには CORS header を含める。認可エラーや未検出エラーでもブラウザ側が status/body を読めるよう、API の 400/401/403/404/500 応答は `{ "error": "..." }` の JSON contract に統一し、同じ CORS header を付与する。API Gateway authorizer 由来の 4xx/5xx も GatewayResponse で CORS header を返す。
...
### `POST /document-groups/{groupId}/share` Request
...
`POST /search` と同じ hybrid search engine を使う search benchmark runner 専用 API。`suiteId` を必須とし、通常利用者向けの `rag:doc:read` ではなく `benchmark:query` と dedicated `BENCHMARK_RUNNER` role を要求する。

server は suite registry から nonprivileged simulated subject と `BENCHMARK_EVALUATION_TENANT_ID`、固定 corpus suite を解決し、`tenantId`、`source=benchmark-runner`、`docType=benchmark-corpus`、`benchmarkSuiteId` filter を search/chat resource context に強制する。request の `user`、`groups`、`tenantId`、`filters`、`scope`、旧 `benchmarkSuiteId` override は validation で拒否する。simulated subject は middleware の user context を置換せず service 呼び出しだけに使い、通常 read/share/move/delete/admin permission を持たない。
...
管理画面で選択できる非同期 benchmark suite を返す。`MMRAG-DocQA` は suite ID `mmrag-docqa-v1`、agent mode、dataset key `hf://datasets/yubo2333/MMLongBench-Doc` として返す。CodeBuild runner はこの suite で Hugging Face `MMLongBench-Doc` train split 全 1,091 rows を prepare してから実行する。`mtrag-v1` と `chatrag-bench-v1` は agent mode の conversation benchmark suite として返し、CodeBuild runner は `conversation-run.ts` で `/benchmark/query` に `conversationHistory` を渡しながら turn 順に実行する。
...
- `AUTH_ENABLED=false` の protected route は `LOCAL_AUTH_USER_ID`、`LOCAL_AUTH_ACCOUNT_STATUS`、`LOCAL_AUTH_TENANT_ID` の明示設定を必須とし、subject、account status、tenant ID の暗黙 default を持たない。production では `AUTH_ENABLED=true` と `AUTH_TENANT_ID` を必須とし、設定不足を起動時または認証時に拒否する。production の `CORS_ALLOWED_ORIGINS` はCloudFront public HTTPS origin 1件をexact originとして必須にし、unset、blank、wildcard、HTTP、malformed、複数origin、localhost/loopbackを起動前に拒否する。local/testでは暗黙wildcardを持たず、明示したexact origin（複数可）または単独wildcardだけを扱う。deployed stackの唯一設定源と移行順は `DES_HLD_002` を正とする。
- production の API middleware は署名・issuer・audience 検証済み ID token の `sub` と Cognito `AdminGetUser` の `sub` を照合し、各 request で現在の `Enabled` を再確認する。tenant ID は request/token claim から採用せず server-managed `AUTH_TENANT_ID` から構築し、Cognito user 不在、停止、照合不一致、依存障害を fail closed で拒否する。
- API middleware は public allowlist 方式で認証を適用する。public endpoint は `GET /health` と `GET /openapi.json` を基本とし、それ以外の REST / oRPC endpoint は route-level permission の有無に関係なく認証 middleware を通す。
...
| 文書 summary 一覧 | `GET /documents` | `rag:doc:read`。benchmark seed 確認に限り `benchmark:seed_corpus` | `CHAT_USER`, `RAG_GROUP_MANAGER`, `SYSTEM_ADMIN`。隔離 seed 確認のみ `BENCHMARK_RUNNER` |
| 小さなテキスト互換用の同期文書登録 | `POST /documents` | `rag:doc:write:group`。benchmark seed payload に限り `benchmark:seed_corpus` | `RAG_GROUP_MANAGER`, `SYSTEM_ADMIN`。隔離 seed のみ `BENCHMARK_RUNNER` |
| 文書 upload session 作成 | `POST /documents/uploads` | `rag:doc:write:group`。`purpose=benchmarkSeed` に限り `benchmark:seed_corpus` | `RAG_GROUP_MANAGER`, `SYSTEM_ADMIN`。隔離 seed のみ `BENCHMARK_RUNNER` |
| 非同期文書 ingest run 作成 | `POST /document-ingest-runs` | `rag:doc:write:group`。`purpose=benchmarkSeed` upload session と隔離 seed metadata に限り `benchmark:seed_corpus` | `RAG_GROUP_MANAGER`, `SYSTEM_ADMIN`。隔離 seed のみ `BENCHMARK_RUNNER` |
| 資料グループ一覧 | `GET /document-groups` | `rag:doc:read` | `CHAT_USER`, `RAG_GROUP_MANAGER`, `SYSTEM_ADMIN` |
| 資料グループ作成 | `POST /document-groups` | `rag:group:create` | `RAG_GROUP_MANAGER`, `SYSTEM_ADMIN` |
| 資料グループ共有更新 | `POST /document-groups/{groupId}/share` | `rag:group:assign_manager` + group manager | `RAG_GROUP_MANAGER`, `SYSTEM_ADMIN` |
| 非同期文書 ingest run 取得 | `GET /document-ingest-runs/{runId}` | `chat:read:own` + run owner または `chat:admin:read_all`。同一 runner が作成した `purpose=benchmarkSeed` run に限り `benchmark:seed_corpus` | `CHAT_USER`, `SYSTEM_ADMIN`。隔離 seed run 確認のみ `BENCHMARK_RUNNER` |
| 非同期文書 ingest event 購読 | `GET /document-ingest-runs/{runId}/events` | `chat:read:own` + run owner または `chat:admin:read_all`。同一 runner が作成した `purpose=benchmarkSeed` run に限り `benchmark:seed_corpus` | `CHAT_USER`, `SYSTEM_ADMIN`。隔離 seed run 確認のみ `BENCHMARK_RUNNER` |
...
| 検索改善候補作成 | `POST /questions/{questionId}/search-improvement-candidates` | `rag:alias:write:group` | `RAG_GROUP_MANAGER`, `SYSTEM_ADMIN` |
...
| benchmark query | `POST /benchmark/query` | `benchmark:query` + server suite allowlist | `BENCHMARK_RUNNER` |
| benchmark search | `POST /benchmark/search` | `benchmark:query` + server suite allowlist | `BENCHMARK_RUNNER` |
| benchmark suite 一覧 | `GET /benchmark-suites` | `benchmark:read` | `BENCHMARK_OPERATOR`, `RAG_GROUP_MANAGER`, `SYSTEM_ADMIN` |
| benchmark run 起動 | `POST /benchmark-runs` | `benchmark:run` | `BENCHMARK_OPERATOR`, `RAG_GROUP_MANAGER`, `SYSTEM_ADMIN` |
| benchmark run 一覧/詳細 | `GET /benchmark-runs`, `GET /benchmark-runs/{runId}` | `benchmark:read` | `BENCHMARK_OPERATOR`, `RAG_GROUP_MANAGER`, `SYSTEM_ADMIN` |

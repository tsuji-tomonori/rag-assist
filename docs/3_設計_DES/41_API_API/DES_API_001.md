...
- `AUTH_ENABLED=false` の protected route は `LOCAL_AUTH_USER_ID`、`LOCAL_AUTH_ACCOUNT_STATUS`、`LOCAL_AUTH_TENANT_ID` の明示設定を必須とし、subject、account status、tenant ID の暗黙 default を持たない。production では `AUTH_ENABLED=true` と `AUTH_TENANT_ID` を必須とし、設定不足を起動時または認証時に拒否する。独自ドメイン未確定期間は一時的に `CORS_ALLOWED_ORIGINS=*` を許容し、ドメイン確定後は具体 origin の allowlist に戻す。

...
| `temporaryScopeIds` | 同一チャット内で継続参照できる一時添付 scope ID の集合 |

`SearchScope` は `/chat`、`/chat-runs`、`/search` の検索境界として扱う。`mode=groups` でも `includeTemporary=true` と `temporaryScopeId` または `temporaryScopeIds` が指定された場合は、指定グループの資料と同一チャットの一時添付だけを検索対象に含める。

### SessionDocumentContext

`SessionDocumentContext` は ChatSession 内だけで参照できる一時添付と前回 citation anchor を、通常文書・通常フォルダとは分けて保持する実行時 context である。RAG 実行時は `search_scope_normalize` がユーザー指定 `SearchScope`、session default scope、`SessionDocumentContext.activeTemporaryScopeIds`、削除済み scope、TTL を合成し、`normalizedSearchScope` を作る。

一時添付は `sessionId` と `temporaryScopeId` が一致する場合だけ検索対象になる。削除済みまたは TTL 期限切れの temporary scope は `normalizedSearchScope.temporaryScopeIds` から除外する。前回回答の citation は assistant の回答本文ではなく、`documentId` / `chunkId` / page range の anchor として保持し、次ターンの retrieval query と rerank prior に使う。

...
FR-022 / AC-FR022-004に従いversion欠落はv1、明示v1/v2/v3はそのまま読み、未知versionを拒否する。read時は永続データを更新しない。明示saveのみv3へ昇格し、要約・引用・task/tool・一時文書の状態を上限内で保持する。Web更新は既存の拡張状態を継承する。session IDと一時文書期限の検証は継続する。一括migrationは不要。

...
| 項目 | 内容 |
| --- | --- |
| `schemaVersion` | 会話履歴 item のスキーマバージョン。legacy は `1`、current write は `2` |
| `id` | 会話識別子 |
...
version 未指定の保存済み item は v1 導入前の legacy data として v1 に正規化する。explicit v1 と v2 は read 可能とし、未知 version は推測変換せず拒否する。一覧 read は保存先を書き換えない。新規保存と v1 item の更新保存は current v2 を永続化し、v1 の message と既存 optional data を保持したまま write-time migration する。v2 は `decontextualizedQuery`、`rollingSummary`、`queryFocusedSummary`、`citationMemory`、`taskState`、`toolInvocations` の optional multi-turn state を保持できる。
...
- conversation history は userId 単位で分離し、schemaVersion により item の解釈を分岐する。missing persisted version は legacy v1、new/update write は v2 とし、unknown version は fail closed にする。

...
管理操作履歴 page は `total`、`nextCursor`、`truncated`、`source`、`asOf` を返す。ただし現行 `AdminLedger` と `AdminAuditLogEntry` は tenant field と共通 security mutation outbox projection をまだ持たない。この残余 security/integrity gap は `tasks/todo/20260714-1011-admin-access-audit-state.md` を owner とし、tenant 完全性や denied/conflict/failed の網羅を現行 read model から推定しない。

...
| ADR | `2_アーキテクチャ_ARC/21_重要決定_ADR/ARC_ADR_005.md` | CloudFront単一入口構成を本番公開経路として採用する判断 |
...
| `ASR-SEC-001` | 5, 6, 7, 8 | `FR-021`, `FR-022`, `FR-024`, `FR-027`, `NFR-010`, `NFR-011`, `NFR-012`, `TC-003` | 認可、権限境界、情報漏えい防止を route-level permission と userId 境界で強制する。 |
| `ASR-SEC-002` | 8 | `FR-025`, `NFR-011` | 通常利用者の self sign-up は `CHAT_USER` のみに限定し、上位権限付与は管理者操作へ分離する。 |
| `ASR-OPER-001` | 1, 7, 8 | `FR-010`, `FR-011`, `FR-019`, `FR-024`, `FR-027`, `TC-003` | trace、監視、評価、運用調査に必要な情報を追跡可能にする。 |

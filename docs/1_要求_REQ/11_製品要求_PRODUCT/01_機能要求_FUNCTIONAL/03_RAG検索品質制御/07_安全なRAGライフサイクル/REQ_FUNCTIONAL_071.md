...
| 実装適合 | OK（confirmed） | `untrusted-content-policy.ts` の deterministic detector/hash-only finding と context/prompt data-only isolation を実装し、direct/encoded/tool/secret attack corpus が exclusion を検証する |

## トレース

- 後方: `GAP-RD-014`, `docs/spec-recovery/09_gap_analysis.md` の prompt injection gap。

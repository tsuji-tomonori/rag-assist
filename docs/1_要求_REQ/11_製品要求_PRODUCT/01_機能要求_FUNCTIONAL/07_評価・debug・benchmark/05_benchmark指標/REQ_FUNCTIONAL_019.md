...
- AC-FR019-011: summary / Markdown report は answerable case に対する `falseRefusalRate` / `false_refusal_rate` を出力し、分母0は未評価として保持すること。
- AC-FR019-012: summary / case artifact / Markdown report は、answer support で評価した文の支持率を `faithfulness`、期待 file/document がある行の raw retrieved item 適合率を `contextRelevance` / `context_relevance` として出力し、evidence 不足または分母0は未評価として保持すること。
- AC-FR019-013: benchmark run が `timed_out`、または required artifact integrity が `complete` でない場合、summary 内の自己申告 metric が存在しても production 品質 evidence は unavailable とし、artifact failure count と timeout は diagnostic measurement として分離すること。
- AC-FR019-014: chat case の model first-token は versioned clock/boundary evidence が `measured` の行だけから p50/p95/p99 と sample count を再導出し、欠損、non-answer、invalid、clock mismatch を total API latency で補完しないこと。
...
| `context_relevance` | 期待ファイルまたは期待 document が指定された行で、raw `retrieved` item のうち期待識別子に一致した割合。retrieval recall と分母を分ける |
...
| `faithfulness` | answer support が評価した全回答文のうち、根拠で支持された文の割合。support evidence がない場合は未評価 |
| `refusal_precision` | 拒否した行のうち、dataset 上も回答不能だった割合 |
| `refusal_recall` | 回答不能な行のうち、実際に拒否できた割合 |
| `false_refusal_rate` | 回答可能な行のうち、実際には refusal を返した割合。answerable 行がない場合は未評価で、低いほどよい |
...
| `first_token_p95_ms` | chat orchestration ingress から最終回答生成 model の最初の非空 content delta までの 95 パーセンタイル。client-visible latency ではなく、authoritative evidence がない行は分母に含めない |
...
| 受け入れ基準 | `AC-FR019-001` から `AC-FR019-014` |
| 優先度 | S |
| 安定性 | Medium |
| 変更履歴 | 2026-05-01 初版 / 2026-05-02 LLM judge 指標を追加 / 2026-05-05 Markdown report の日本語説明列を追加 / 2026-07-17 false refusal、faithfulness/context relevance、run/artifact completeness、model first-token clock lineage の evidence 境界を追加 |

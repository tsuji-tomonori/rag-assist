...
| 変更履歴 | 2026-07-11 初版。2026-07-17 document share grant helperの暗黙default tenant補完廃止を追記 |
...
| 実装適合 | OK（confirmed） | document/RAG artifact/run/event/group/policy/membership の local+Dynamo tenant composite key/index と same-ID 並存、caller tenant 非拡張を two-tenant store/lifecycle/route tests で検証済み。document share grant helperはmanifest metadata/admissionのcanonicalかつ一致するtenantだけをpartitionとし、欠損・invalid・競合をstore/audit/principal lookup前に拒否する |
| 合意 | pending | single/multi tenant と cross-tenant 例外方針が未確定 |

## Document share grant helper の実装境界

- `confirmed`: manifestの`metadata.tenantId`と`admission.tenantId`が両方canonicalなら一致を必須にし、片方だけcanonicalな場合はその値をresource tenantとして使う。
- `confirmed`: tenant evidenceの欠損、空白を含む非canonical値、metadata/admissionの不一致では、`default`へ補完せずper-document/legacy grant read、audit prepare、principal lookup、grant writeより前にfail closedにする。
- `confirmed`: actor tenantはresource tenantの代替にせず、既存same-tenant authorizationで別途照合する。
- `open_question`: tenant-less legacy manifestのmigration/backfillと廃止期限、他serviceに残るlegacy default fallbackの段階撤去は本unit外である。

...
| 変更履歴 | 2026-07-11 初版。2026-07-17 document administrative principal downgrade rejectionを追記 |
...
| 実装適合 | OK（confirmed） | folder/document share の versioned replace が expected-version CAS、complete next state、administrative-principal invariant を不可分に検証する。documentはowner`readOnly`/`deny`を拒否し、owner`full`とnon-owner grantを許可するdirect matrix、stale/concurrent writer testsを持つ |

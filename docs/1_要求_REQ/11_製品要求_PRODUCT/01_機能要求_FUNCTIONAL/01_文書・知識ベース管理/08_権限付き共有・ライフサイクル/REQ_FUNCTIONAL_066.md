...
| 変更履歴 | 2026-07-11 初版。2026-07-17 empty folder archive の pre-CAS repair、post-CAS ledger、worker recovery を追記 |
...
| 実装適合 | OK（confirmed） | authoritative deny 後に11 scope の tenant-scoped cleanup manifest を登録し、source/account/role/group/archive/delete/failed-ingest trigger、retry/residual/superseded-deny safety を coordinator と direct tests が検証する。empty folder archive はCAS前repair、archive後ledger、exact archived-state verifierによりcache/queued-run/evaluationの収束を追跡する |
| 合意 | pending | purge/retention/restore と伝播 SLO は未確定 |

## Empty folder archive の実装境界

- `confirmed`: folder archive は全validation後かつarchive CAS前に、audit intentと相関する `folder-archive:<auditIntentId>` repairを永続化する。CAS失敗ではrepairをabandonedへ遷移し、archive成功後はexact `folder:<archived.updatedAt>` deny versionでledgerを登録する。
- `confirmed`: post-CAS ledger登録失敗ではarchived stateとdurable repairを保持し、APIはsuccessを返さずauditをpendingに保つ。production cleanup workerはexact tenant/folder/status/version/timestampを再確認してrepairからledgerを再生する。
- `confirmed`: cleanup対象はtenant cache、logical session fence、folder scopeのactive queued run、retained evaluation artifactであり、archive前のempty-folder guardを越えてsource/chunk/indexを破壊しない。
- `open_question`: actual AWS上のS3 conditional write、DynamoDB read、EventBridge/Lambda retryによるworker convergenceは未検証であり、local/CI evidenceを代替しない。

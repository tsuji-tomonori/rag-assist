...
| 変更履歴（続き） | 2026-08-18 文書画面の検索・filter value・file table・detail dialog・disclosure expandedをChromium AX tree契約へ追加し、選択行のDOM `aria-selected`も検証。2026-08-19 文書画面の検索・絞り込み・文書選択・detail dialog focus trap／Escape／trigger復帰をChromium／Firefox／WebKitのkeyboard-only required gateへ追加 |

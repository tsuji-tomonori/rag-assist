...
- `E2E-UI-STATE-001`: history / favorites / assignee の loading→500→retry→confirmed empty と HTTP 403、benchmark の loading→500 partial→retry→confirmed empty と HTTP 403、admin の partial→retry→recovered と source/as-of 付き stale→retry→recovered を required Chromium UI quality gate で検証する。証拠のない他3画面の `AC-SQ016-007` automated status は `blocked` を維持する。

...
- `E2E-UI-LAYOUT-STRESS-001`: 320 CSS pxとreduced motionで長文回答の先頭／末尾、長い引用・ファイル名、履歴35件の長い先頭／末尾title・末尾viewport到達、お気に入り24件／8 target typeの長いlabel・target ID／アクセス不可cue／末尾到達と再取得後の確認済み0件をchat / documents / history / favoritesで表示し、document rootと対象regionの水平overflowがないことをChromium／Firefox／WebKitのPR required gateで検証する。JSON evidenceにはbrowser project名、viewport、fixture量、末尾要素の矩形、URL／dimensions、実browser zoom・実支援技術・実機ではないboundaryを付ける。

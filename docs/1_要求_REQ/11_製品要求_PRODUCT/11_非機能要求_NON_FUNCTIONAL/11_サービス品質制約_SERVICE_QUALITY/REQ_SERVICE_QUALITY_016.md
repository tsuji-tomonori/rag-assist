...
- Firefox／WebKitの自動証跡はlogin / chatのsemantic contract、主要keyboard journey、5 viewのCSS viewport reflow proxyに限定する。representative screen reader、browser UIを操作する実200%/400% zoom、text-only zoom、OS scaling、touch／real device、Firefox／WebKit native accessibility treeのengine固有debug出力、非対象画面のbrowser evidenceを代替しない。

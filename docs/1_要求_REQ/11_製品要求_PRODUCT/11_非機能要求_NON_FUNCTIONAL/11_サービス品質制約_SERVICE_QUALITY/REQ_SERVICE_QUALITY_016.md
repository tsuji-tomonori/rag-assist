...
| 変更履歴（続き） | 2026-08-18 文書画面の検索・filter value・file table・detail dialog・disclosure expandedをChromium AX tree契約へ追加し、選択行のDOM `aria-selected`も検証。2026-08-19 文書画面の検索・絞り込み・文書選択・detail dialog focus trap／Escape／trigger復帰をChromium／Firefox／WebKitのkeyboard-only required gateへ追加。2026-08-20 管理者設定のoverview／section遷移とユーザー検索・絞り込みをChromium／Firefox／WebKitのkeyboard-only required gateへ追加。2026-08-21 管理者設定のworkspace／section／ユーザー管理のname・role・valueをChromium AX tree required gateへ追加。2026-08-22 チャットのtext contrast、3px focus indicator contrast、permission stateの非色依存cueをChromium required gateへ追加。2026-08-23 担当者対応のtext contrast、3px focus indicator contrast、permission stateの非色依存cueをChromium required gateへ追加。2026-08-26 個人設定のname・role・valueと変更後のvisible polite statusをFirefox／WebKitのARIA snapshot／DOM state required gateへ追加。2026-08-28 文書画面のworkspace／breadcrumb／folder tree／search・filter value／table／detail dialogとselected／expanded stateをFirefox／WebKit required gateへ追加。2026-08-29 管理者設定のworkspace／section current state／ユーザー管理search・filter value／作成form／table／polite取得statusをFirefox／WebKit required gateへ追加。2026-08-30 履歴のworkspace／検索／並び順／お気に入りfilter／主要actionとquery／sort／checked stateをFirefox／WebKit required gateへ追加。2026-08-31 お気に入りのworkspace／項目group／item label／access cue／back actionをFirefox／WebKit required gateへ追加。2026-09-01 性能テストのworkspace／job panel／suite・dataset・model・concurrency value／実行履歴scroll region・tableをFirefox／WebKit required gateへ追加。2026-09-02 文書画面、2026-09-03 担当者対応、2026-09-04 お気に入り、2026-09-05 チャット、2026-09-06 管理画面、2026-09-07 性能テストのresource stateをFirefox／WebKit required gateへ追加 |
...
- `E2E-UI-KEYBOARD-NAV-001`: チャットの質問textboxへTabで到達し、composerの3px focus indicator、既定Enter送信、処理中から回答への復帰を検証する。あわせて履歴の検索・並び順・お気に入り絞り込み・会話選択、お気に入りへの到達・チャット復帰、文書画面のfolder／filename search・type／status／folder／sort／page-size・detail trigger・dialog focus trap／Escape／trigger復帰、担当者対応のステータス絞り込み・検索・問い合わせ選択・回答入力・通知切替・一時保持、管理者設定のoverview card／section tab／ユーザー検索・状態・並び順・検索確定、個人設定の送信キー変更とチャット復帰を3px focus indicator付きで検証する。PR required gateではChromium、Firefox、WebKitで実行する。
- `E2E-UI-SR-SEMANTICS-001`: チャットのregion / form / textbox /送信buttonとidle→回答処理中→完了に伴うbusy / polite live state、履歴のregion / heading / searchbox / combobox value / checkbox checked state /主要button、お気に入りのregion / heading / target type見出し /戻るbutton、担当者対応のworkspace /一覧 / lane /選択中詳細 /回答form landmark・filter value・選択state・通知checked state・polite status、benchmarkのregion / heading / suite・dataset・model・concurrency control value /実行履歴scroll region・table、管理者設定のworkspace / heading / section navigation・ユーザー管理region / search landmark / filter value /作成form /ユーザー一覧table、文書画面のworkspace / breadcrumb / folder tree / current context・folder / filename search・filter value・file table・detail dialog /主要action / disclosure expandedのAX契約と選択行のDOM `aria-selected`、ならびに個人設定のregion / heading / combobox value /主要buttonを検証し、Chromium AX JSON evidenceをPlaywright reportへ添付する。
- `E2E-UI-CONTRAST-001`: チャットを320 / 1280 CSS pxで表示してaxe `color-contrast` violation 0を要求し、質問textbox focus時のcomposer outlineを実computed styleから3px以上かつ背景比3:1以上と検証する。`chat:create`不足時は可視案内、`role=alert`、disabled送信controlの複数cueと同じcontrast検査を要求する。Chromium自動証跡であり、manual perception、representative screen reader、実browser zoom、OS scaling、real deviceを代替しない。
- `E2E-UI-CONTRAST-002`: 担当者対応を320 / 1280 CSS pxで表示してaxe `color-contrast` violation 0を要求し、検索入力focus時のoutlineを実computed styleから3px以上かつ背景比3:1以上と検証する。問い合わせAPIのHTTP 403時は可視案内、`role=alert`、private kanban suppressionの複数cueと同じcontrast検査を要求する。Chromium自動証跡であり、manual perception、representative screen reader、実browser zoom、OS scaling、real deviceを代替しない。
- `E2E-UI-CONTRAST-003`: 文書画面を320 / 1280 CSS pxで表示してaxe `color-contrast` violation 0を要求し、フォルダ検索入力focus時のoutlineを実computed styleから3px以上かつ背景比3:1以上と検証する。文書・フォルダ・再インデックスAPIのHTTP 403時は可視案内、`role=alert`、private document panel suppressionの複数cueと同じcontrast検査を要求する。Chromium自動証跡であり、manual perception、representative screen reader、実browser zoom、OS scaling、real deviceを代替しない。
...
- `E2E-UI-CROSS-BROWSER-SEMANTICS-004`: 文書管理regionのheading、breadcrumb、folder tree、folder／filename search、filter value、文書table、detail dialogと主要actionをPlaywright ARIA snapshotで検証する。文書選択後のrow `aria-selected`とtechnical disclosureのcollapsed→expanded stateを同じFirefox／WebKit projectで検証し、browser project名とrepresentative screen reader／native AX treeではないevidence boundaryを付ける。
- `E2E-UI-CROSS-BROWSER-SEMANTICS-005`: 管理者設定regionのheading、section navigationとcurrent state、ユーザー管理region／search／filter value／作成form／table／polite取得statusをFirefox／WebKitで検証する。Playwright ARIA snapshotとstate JSONにはbrowser project名とrepresentative screen reader／native AX treeではないevidence boundaryを付ける。
- `E2E-UI-CROSS-BROWSER-SEMANTICS-006`: 履歴regionのheading、会話一覧、検索searchbox、並び順combobox、お気に入りcheckbox、favorite／delete／back actionをFirefox／WebKitで検証する。query入力、sort変更、お気に入りfilter切替後のvalue／checked stateを同じ実走で確認し、Playwright ARIA snapshotとstate JSONにはbrowser project名とrepresentative screen reader／native AX treeではないevidence boundaryを付ける。
- `E2E-UI-CROSS-BROWSER-SEMANTICS-007`: お気に入りregionのheading、項目一覧、会話／文書group、item label／target ID／アクセス不可cue、back actionをFirefox／WebKitで検証する。Playwright ARIA snapshotとstate JSONにはbrowser project名とrepresentative screen reader／native AX treeではないevidence boundaryを付け、未実装のfavorite resume／deleteを完了扱いしない。
- `E2E-UI-CROSS-BROWSER-SEMANTICS-008`: 性能テストregionのheading、job panel、suite／dataset／model／concurrency controlのname・role・value、実行／更新／戻るaction、実行履歴scroll region／tableをFirefox／WebKitで検証する。Playwright ARIA snapshotとstate JSONにはbrowser project名とrepresentative screen reader／native AX treeではないevidence boundaryを付け、実AWS上のstart／cancel／download journeyを完了扱いしない。
...
- `E2E-UI-CROSS-BROWSER-STATE-002`: 文書画面でカタログ／フォルダ／再インデックスのloading、文書取得500によるpartial state、retry、confirmed empty、全resource 403をFirefox／WebKitのPR required gateで検証する。取得済み／未更新を区別し、回復確認前のカタログ・0件・empty state・private detailが露出しないことを、browser project、state sequence、resource read count、test-only fixture境界付きartifactで確認する。
- `E2E-UI-CROSS-BROWSER-STATE-003`: 担当者対応のloading→HTTP 500→retry→confirmed emptyとHTTP 403をFirefox／WebKitのPR required gateで検証する。回復確認前の0件・empty state・カンバンとprivate detailが露出しないことを、browser project、state sequence、request count、test-only fixture境界付きartifactで確認する。
- `E2E-UI-CROSS-BROWSER-STATE-004`: お気に入りのloading→HTTP 500→retry→confirmed emptyとHTTP 403をFirefox／WebKitのPR required gateで検証する。回復確認前の0件・empty stateとprivate detailが露出しないことを、browser project、state sequence、request count、test-only fixture境界付きartifactで確認する。
- `E2E-UI-CROSS-BROWSER-STATE-005`: チャットのinitial→processing→SSE timeout→`Last-Event-ID`付きretry→recovered answer、HTTP 500の安全なerror、`chat:create`不足時のpermissionと送信抑止をFirefox／WebKitのPR required gateで検証する。processing／retry中のbusy・disabledと回復後の入力再有効化、private detail／未許可request非公開を、browser project、state sequence、request count／retry header、test-only fixture境界付きartifactで確認する。
- `E2E-UI-CROSS-BROWSER-STATE-006`: 管理画面のloading→監査取得500によるpartial→retry→recovery、ユーザー更新失敗時のsource・as-of付きstale data保持→recovery、管理権限不足deep linkのpermissionとprotected request抑止をFirefox／WebKitのPR required gateで検証する。成功part／確認済みユーザーを保持し、false zero、private detail、未許可管理requestを公開しないことを、browser project、state sequence、resource read count、test-only fixture境界付きartifactで確認する。
- `E2E-UI-CROSS-BROWSER-STATE-007`: 性能テストのloading→実行履歴取得500によるpartial→retry→confirmed emptyと、run／suite全resourceのHTTP 403をFirefox／WebKitのPR required gateで検証する。取得済みsuiteを保持し、回復確認前の実行履歴件数／history region、empty state、private detailを公開しないことを、browser project、state sequence、resource read count、test-only fixture境界付きartifactで確認する。
...
- `E2E-UI-LAYOUT-STRESS-001`: 320 CSS pxとreduced motionで長文回答の先頭／末尾、長い引用、担当者対応32件／4 laneの長いtitle・質問・部署名とDOM末尾カードのviewport到達、管理監査36件の長い対象・実行者・理由・policy・監査IDとDOM末尾項目のviewport到達、ドキュメント30件／6 file typeの長い先頭／末尾ファイル名・50件表示時の全件表示・末尾viewport到達、履歴35件の長い先頭／末尾title・末尾viewport到達、お気に入り24件／8 target typeの長いlabel・target ID／アクセス不可cue／末尾到達と再取得後の確認済み0件をchat / assignee / admin / documents / history / favoritesで表示し、document rootと対象regionの水平overflowがないことをChromium／Firefox／WebKitのPR required gateで検証する。JSON evidenceにはbrowser project名、viewport、fixture量、lane数／表示件数、末尾要素の矩形、URL／dimensions、実browser zoom・実支援技術・実機ではないboundaryを付ける。
- Firefox／WebKitの自動証跡はlogin / chat / profile / assignee / documents / admin / history / favorites / benchmarkのsemantic contract、履歴／文書／担当者対応／お気に入り／性能テストの代表resource state、チャットのSSE retry／error／permission state、管理画面のpartial／stale／permission state、主要keyboard journey、5 viewのCSS viewport reflow proxy、6 viewの320 CSS px content-extreme fixtureに限定する。representative screen reader、browser UIを操作する実200%/400% zoom、text-only zoom、OS scaling、touch／real device、Firefox／WebKit native accessibility treeのengine固有debug出力、非対象画面のbrowser evidenceを代替しない。

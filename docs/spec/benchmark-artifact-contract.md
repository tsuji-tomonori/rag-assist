...
| `BenchmarkTargetConfig` | `MODEL_ID`, `EMBEDDING_MODEL_ID`, `TOP_K`, `MEMORY_TOP_K`, `MIN_SCORE`, API base URL, evaluator profile、runner 種別に加え、model/index/prompt/pipeline/parser/chunker、runtime/workload/price、corpus/ACL/document-size/dependency-latency profile と concurrency を保存する。 |
| `BenchmarkDatasetPrepareRun` | dataset source、corpus seed 結果、抽出不能・未実行 skip、変換 version を保存する。 |
| `BenchmarkWorkloadEvidence` | 承認者・承認時刻・dataset/runtime version と eligibility probe、recovery、backlog の実測時刻を保存する。 |
| `BenchmarkPriceCatalog` | 承認者・承認時刻・region/version と model/embedding/storage/worker/egress の単価を保存する。 |
...
`caseResults` は case-level の `status`, `failureReasons`, `retrieval`, `citation`, `answerability`, `task`, `generation`, `latency`, `cost` を持つ。品質判定に使う versioned case は `questionType`、tenant-role、OCR mode、language、multi-evidence、answerability、severity の全 slice identity を持たなければならない。claim は severity、citation 要否、support span と locator を、citation は対応 claim、関連性、support、locator validity を記録する。task runner は actor、goal、success criteria、許容 handoff、severity を、latency は chat/search/ingest の endpoint/stage、backlog、outcome、retry exhaustion を記録する。不完全な case/slice は該当 metric を unavailable とし、任意 aggregate field のコピーだけで合格根拠にしない。

cost は provider usage が取得できる場合だけ入れる。model input/output token、embedding token、storage byte-hour、worker millisecond、egress byte と unit kind を列挙し、全 component を観測した case だけ `usageComplete=true` にする。`usageComplete` が false、price catalog が未承認、model rate が無い、または catalog version が candidate と一致しない場合は component/total unit cost を生成しない。

workload evidence は authoritative commit/failure/create timestamp と deny/recovery/start timestamp の差から eligibility p50/p95/p99/max、MTTR、recovery p95、backlog p99 を算出する。eligibility は share/account/role/group/classification/usage/quality/expiry/archive/delete の10 trigger と active/staged/old-index/cache/session/memory/queued-worker の7 path の全70組、recovery は vector/LLM/OCR/queue、endpoint-stage は chat/search/ingest を必須とする。unreflected resource ID、timeout/error、retry exhaustion、duplicate/lost artifact を保存する。いずれかの組、workload dimension、承認情報、dataset/runtime/workload version が欠けるか candidate と一致しない場合は workload metric 全体を生成しない。
...
- CodeBuild post-build は `rag-release-audit-v1` artifact を生成し、product runtime の dataset expected-field/identity branch と summary/prepare/seed/case manifest 整合性を記録する。
- promotion/deploy は承認済み versioned policy と完全な observations の論理積を `rag:promotion:check` で評価し、missing/unapproved/unavailable/profile mismatch/release-audit missing を pass にしない。

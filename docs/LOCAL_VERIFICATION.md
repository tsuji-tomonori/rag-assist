...
`task benchmark:sample` は上記の標準 corpus 指定を含む。`task benchmark:search:sample` も `BENCHMARK_SUITE_ID=search-standard-v1` と同じ標準 corpus seed を指定する。`smoke-agent-v1`、`standard-agent-v1`、`clarification-smoke-v1`、`search-standard-v1` では、同じ `BENCHMARK_CORPUS_SUITE_ID` の benchmark seed 文書を削除してから `handbook.md` を `/documents` に再 seed し、active chunk が作成されてから評価 query / search を実行する。PDF corpus は upload session の転送後に `POST /document-ingest-runs` を開始し、`GET /document-ingest-runs/{runId}` を polling して active chunk 付き summary を待つ。runner は `/benchmark/query` と corpus 付き `/benchmark/search` に `BENCHMARK_CORPUS_SUITE_ID` を渡し、検索前 filter で `source=benchmark-runner`、`docType=benchmark-corpus`、`benchmarkSuiteId` を強制する。削除または再 seed に失敗した場合、古い corpus で測定を継続せず runner は失敗する。抽出可能なテキストがない PDF seed と、async ingest run が Textract OCR timeout を返した PDF seed は `skipped_unextractable` として summary / report に記録され、agent benchmark ではその file を期待する row を `skippedRows` として評価対象から除外する。その他の worker failure と run polling 失敗は runner fatal とする。polling 間隔は `BENCHMARK_INGEST_RUN_POLL_INTERVAL_MS`、最大待機時間は `BENCHMARK_INGEST_RUN_TIMEOUT_MS` で調整できる。`mmrag-docqa-v1` をローカルで全量確認する場合は `MMRAG_DOCQA_DATASET_OUTPUT=.local-data/mmrag-docqa-v1/dataset.jsonl`、`MMRAG_DOCQA_CORPUS_DIR=.local-data/mmrag-docqa-v1/corpus` で `npm run prepare:mmrag-docqa -w @memorag-mvp/benchmark` を実行し、その後 `DATASET=.local-data/mmrag-docqa-v1/dataset.jsonl`、`BENCHMARK_SUITE_ID=mmrag-docqa-v1`、`BENCHMARK_CORPUS_DIR=.local-data/mmrag-docqa-v1/corpus`、`BENCHMARK_CORPUS_SUITE_ID=mmrag-docqa-v1` を指定して benchmark runner を実行する。memory grounding の ablation は converter 実行時に `MMRAG_DOCQA_USE_MEMORY=1` を付けて `useMemory=true` の dataset を生成し、既定の `useMemory=false` dataset と同じ corpus / evaluator profile で page hit と citation support を比較する。`mtrag-v1` と `chatrag-bench-v1` は `conversation-run.ts` を使い、1行1会話の JSONL を turn 順に処理して前ターンを `conversationHistory` として `/benchmark/query` に渡す。既定 prepare は `yubo2333/MMLongBench-Doc` train split の 1,091 rows と参照 PDF を対象にし、`MMRAG_DOCQA_DOWNLOAD_DOCUMENTS=0` は converter の単体確認用に限る。seed 文書は `aclGroups: ["BENCHMARK_RUNNER"]` と `docType: "benchmark-corpus"` で隔離し、通常利用者の文書一覧にも表示しない。corpus seed の削除・アップロード時間は runner setup であり、summary の latency 指標は `/benchmark/query` または `/benchmark/search` の初回 API call を対象にする。同じ corpus を複数 suite で共有する場合は `BENCHMARK_CORPUS_SUITE_ID` で seed 判定用の corpus identity を固定する。`<file>.metadata.json` の `searchAliases` は seed metadata に含まれ、search benchmark の alias case で使う。

FR-084 対応後の query/search request は評価 suite の `suiteId` だけを渡す。`BENCHMARK_CORPUS_SUITE_ID` は seed setup 用であり API request へは渡さず、API server の allowlist が corpus suite、`BENCHMARK_EVALUATION_TENANT_ID`、simulated subject を解決する。local verification では `BENCHMARK_EVALUATION_ENABLED=true` と、local user tenant と異なる `BENCHMARK_EVALUATION_TENANT_ID` を明示する。
...
この task は Hugging Face の `allganize/RAG-Evaluation-Dataset-JA` から `rag_evaluation_result.csv` と `documents.csv` を取得し、`.local-data/allganize-rag-evaluation-ja/dataset.jsonl` と `.local-data/allganize-rag-evaluation-ja/corpus/` を作成してから agent benchmark を実行する。source PDF URL が移動または削除されている場合は NDL WARP の最新アーカイブも試行する。download できた PDF に抽出可能なテキストがない場合、該当 corpus とそれを期待資料にする row は skip artifact として残し、残りの row を評価する。`ALLGANIZE_RAG_DOMAIN=finance` などで domain を絞り込み、`ALLGANIZE_RAG_EXPECTED_MODE=strict-contains` で `target_answer` の完全包含判定を有効化できる。既定では `target_answer` は `referenceAnswer` として results に保持し、既存 runner の file/page/citation/retrieval 指標を中心に確認する。

MTRAG / ChatRAG Bench:

```bash
MTRAG_INPUT=/path/to/mtrag.json \
task benchmark:mtrag

CHATRAG_INPUT_DIR=/path/to/chatrag-bench \
task benchmark:chatrag-bench
```

MTRAG adapter は raw JSON の conversation / evidence を `.local-data/mtrag/dataset.jsonl` と `.local-data/mtrag/corpus/` へ変換する。ChatRAG Bench adapter は input directory 内の `.json` / `.jsonl` を subset 単位で読み、`.local-data/chatrag-bench/dataset.jsonl` と `.local-data/chatrag-bench/corpus/` へ変換する。

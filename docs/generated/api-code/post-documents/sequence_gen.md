...
| 20 | `MemoRagService.ingest` | Service | service の usage rollout mode 処理を呼び出す。 | `this.usageRolloutMode()` | `apps/api/src/rag/memorag-service.ts:510 (MemoRagService.ingest)` |
| 21 | `MemoRagService.ingest` | Service | service の create memory cards 処理を呼び出す。 | `this.createMemoryCards(memoryInput, deps.textModel)` | `apps/api/src/rag/memorag-service.ts:521 (MemoRagService.ingest)` |
...
| 48 | `MemoRagService.registerSourceGovernance` | Service | service の source governance approval service 処理を呼び出す。 | `this.sourceGovernanceApprovalService()` | `apps/api/src/rag/memorag-service.ts:570 (MemoRagService.registerSourceGovernance)` |
...
| 55 | `MemoRagService.stageApprovedSourceGovernancePublication` | Store | `this.deps.objectStore` に対して get text を実行する。 | `this.deps.objectStore.getText(input.source.sourceObjectKey)` | `apps/api/src/rag/memorag-service.ts:3886 (MemoRagService.stageApprovedSourceGovernancePublication)` |
...
| 98 | `MemoRagService.registerSourceGovernance` | Service | service の ensure initial record 処理を呼び出す。 | `this.sourceGovernanceApprovalService().ensureInitialRecord(manifest)` | `apps/api/src/rag/memorag-service.ts:570 (MemoRagService.registerSourceGovernance)` |
...
| B048 | `MemoRagService.createCurrentDocumentIngestAuthorization` | `input.purpose` が `"benchmarkSeed"` と等しい | `apps/api/src/rag/memorag-service.ts:530 (MemoRagService.createCurrentDocumentIngestAuthorization)` |
| B049 | `MemoRagService.createCurrentDocumentIngestAuthorization` | `input.purpose` が `"chatAttachment"` と等しい | `apps/api/src/rag/memorag-service.ts:532 (MemoRagService.createCurrentDocumentIngestAuthorization)` |
| B050 | `MemoRagService.ingest` | `tenantId` が存在し、真である、かつ `this.deps.usageEventStore` が存在し、真である、かつ `this.usageRolloutMode()` が `"disabled"` と異なる | `apps/api/src/rag/memorag-service.ts:510 (MemoRagService.ingest)` |
| B051 | `MemoRagService.discardUncommittedIngest` | some の判定結果が真である | `apps/api/src/rag/memorag-service.ts:564 (MemoRagService.discardUncommittedIngest)` |

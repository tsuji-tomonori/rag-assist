...
| B002 | `MemoRagService.listDocumentGroups` | if | `detail.permission` が `"none"` と異なる | `detail.permission !== "none"` | `apps/api/src/rag/memorag-service.ts:874 (MemoRagService.listDocumentGroups)` |
| B003 | `MemoRagService.listDocumentGroups` | catch | 例外が発生した場合に catch 処理へ移る | `error` | `apps/api/src/rag/memorag-service.ts:877 (MemoRagService.listDocumentGroups)` |
| B004 | `MemoRagService.listDocumentGroups` | if | `error` が `ResourceOperationAuthorizationError` の instance である | `error instanceof ResourceOperationAuthorizationError` | `apps/api/src/rag/memorag-service.ts:878 (MemoRagService.listDocumentGroups)` |
...
| 1 | `MemoRagService.listDocumentGroups` | list document groups の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:869 (MemoRagService.listDocumentGroups)` |

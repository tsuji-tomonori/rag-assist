...
| 項目 | 型 | 必須 | 説明 | 制約 |
| --- | --- | --- | --- | --- |
| `expectedUpdatedAt` | `string` | yes | `data.expectedUpdatedAt` の値。項目名は expected updated at を表します。 | minLength=1 |
...
| 項目 | 内容 |
| --- | --- |
| 認可モード | `benchmarkSeedDeleteOrPermission` |
| 必須 permission | `rag:doc:delete:group` |
...
補足:
- BENCHMARK_RUNNER は benchmark seed 文書だけ削除できます。
...
| `403` | 必要 permission (rag:doc:delete:group) または条件付き permission を満たさない場合。 | `{"error":"Forbidden"}` |

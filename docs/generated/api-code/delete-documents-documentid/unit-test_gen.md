...
| F016 | `MemoRagService.getBenchmarkDocumentManifest` | if | `tenantId` が存在しない、または偽である | `apps/api/src/rag/memorag-service.ts:1052 (MemoRagService.getBenchmarkDocumentManifest)` |
...
| TC029 | F016: 条件成立 | `tenantId` が存在しない、または偽である 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:1052 (MemoRagService.getBenchmarkDocumentManifest)` |
| TC030 | F016: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:1052 (MemoRagService.getBenchmarkDocumentManifest)` |

...
| F003 | `MemoRagService.getDebugRun` | if | `key` が存在しない、または偽である | `apps/api/src/rag/memorag-service.ts:2366 (MemoRagService.getDebugRun)` |
...
| TC006 | F003: 条件成立 | `key` が存在しない、または偽である 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:2366 (MemoRagService.getDebugRun)` |
| TC007 | F003: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:2366 (MemoRagService.getDebugRun)` |

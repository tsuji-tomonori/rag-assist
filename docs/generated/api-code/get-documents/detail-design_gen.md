...
| Route 定義 | `apps/api/src/routes/document-routes.ts:821 (GET /documents)` |
| Handler | `apps/api/src/routes/document-routes.ts:836 (GET /documents handler)` |

## 2. 主処理

| # | 深さ | 種別 | 自然言語での処理 | コード根拠 | 実装位置 |
| ---: | ---: | --- | --- | --- | --- |
| 1 | 0 | 認証・認可 | 認証済み利用者を request context から取得する。 | `user = c.get("user")` | `apps/api/src/routes/document-routes.ts:837` |
| 2 | 0 | 分岐 | 分岐: 利用者が "rag:doc:read" permission を持たない、かつ 利用者が "benchmark:seed_corpus" permission を持たない。 | `!hasPermission(user, "rag:doc:read") && !hasPermission(user, "benchmark:seed_corpus")` | `apps/api/src/routes/document-routes.ts:838` |
| 3 | 1 | 例外 | HTTPException (403) を送出して通常処理を中断する。 | `throw new HTTPException(403, { message: "Forbidden" })` | `apps/api/src/routes/document-routes.ts:839` |
| 4 | 0 | 呼び出し | valid query を実行する。 | `query = validQuery<CollectionListQuery>(c)` | `apps/api/src/routes/document-routes.ts:841` |
| 5 | 0 | 状態 | candidates を計算して後続処理へ保持する。 | `candidates = hasPermission(user, "rag:doc:read") ? await service.listDocuments(user) : (await service.listBenchmarkDocumentManifests()) .filter(isBenchmarkSeedDocumentManifest) .map((manifest) => ({ ...manifest, current…` | `apps/api/src/routes/document-routes.ts:842` |
| 6 | 0 | 認証・認可 | authorized only page により認証・認可条件を確認する。 | `page = authorizedOnlyPage({ candidates, authorized: () => true, project: documentListItemSummary, offset: decodeCollectionCursor(query.cursor), limit: query.limit })` | `apps/api/src/routes/document-routes.ts:857` |
| 7 | 0 | 応答 | 処理結果を HTTP 200 の JSON response として返す。 | `return c.json({ documents: page.items, count: page.count, nextCursor: page.nextCursor, responseProfileVersion: page.responseProfileVersion }, 200)` | `apps/api/src/routes/document-routes.ts:864` |

## 3. 条件分岐・例外

| # | 所属関数 | 種別 | 条件の意味 | 根拠式 | 実装位置 |
| ---: | --- | --- | --- | --- | --- |
| B001 | `GET /documents handler` | if | 利用者が "rag:doc:read" permission を持たない、かつ 利用者が "benchmark:seed_corpus" permission を持たない | `!hasPermission(user, "rag:doc:read") && !hasPermission(user, "benchmark:seed_corpus")` | `apps/api/src/routes/document-routes.ts:838 (GET /documents handler)` |
| B002 | `GET /documents handler` | 三項条件 | 利用者が "rag:doc:read" permission を持つ | `hasPermission(user, "rag:doc:read")` | `apps/api/src/routes/document-routes.ts:842 (GET /documents handler)` |
| B003 | `MemoRagService.listDocuments` | if | is missing object error の判定結果が真である | `isMissingObjectError(error)` | `apps/api/src/rag/memorag-service.ts:908 (MemoRagService.listDocuments)` |
| B004 | `MemoRagService.listDocuments` | 三項条件 | `user` が存在し、真である | `user` | `apps/api/src/rag/memorag-service.ts:922 (MemoRagService.listDocuments)` |
| B005 | `MemoRagService.listDocuments` | 三項条件 | `user` が存在し、真である | `user` | `apps/api/src/rag/memorag-service.ts:925 (MemoRagService.listDocuments)` |
| B006 | `MemoRagService.listDocuments` | if | `user` が存在しない、または偽である、または `permissionService` が存在しない、または偽である | `!user \|\| !permissionService` | `apps/api/src/rag/memorag-service.ts:930 (MemoRagService.listDocuments)` |
| B007 | `MemoRagService.listBenchmarkDocumentManifests` | if | `config.benchmarkEvaluationEnabled` が存在しない、または偽である、または `tenantId` が存在しない、または偽である | `!config.benchmarkEvaluationEnabled \|\| !tenantId` | `apps/api/src/rag/memorag-service.ts:955 (MemoRagService.listBenchmarkDocumentManifests)` |
| B008 | `MemoRagService.listBenchmarkDocumentManifests` | if | is missing object error の判定結果が真である | `isMissingObjectError(error)` | `apps/api/src/rag/memorag-service.ts:962 (MemoRagService.listBenchmarkDocumentManifests)` |
...
| 深さ | Symbol | 責務 | 実装位置 |
| ---: | --- | --- | --- |
| 0 | `GET /documents handler` | GET /documents の request を受け、検証・認可・service 呼び出し・HTTP 応答を調整する。 | `apps/api/src/routes/document-routes.ts:836 (GET /documents handler)` |
...
| 1 | `MemoRagService.listDocuments` | list documents の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:901 (MemoRagService.listDocuments)` |
| 2 | `MemoRagService.documentAccessTenantId` | document access tenant id の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:3952 (MemoRagService.documentAccessTenantId)` |
...
| 2 | `MemoRagService.canAccessDocumentManifest` | can access document manifest の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:1228 (MemoRagService.canAccessDocumentManifest)` |
...
| 2 | `MemoRagService.sanitizeDirectSharedManifestForList` | sanitize direct shared manifest for list の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:1246 (MemoRagService.sanitizeDirectSharedManifestForList)` |
| 2 | `documentCapabilities` | document capabilities の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:6113 (documentCapabilities)` |
| 1 | `MemoRagService.listBenchmarkDocumentManifests` | list benchmark document manifests の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:953 (MemoRagService.listBenchmarkDocumentManifests)` |
...
| 参照 | Store | `this.deps.objectStore` | `listKeys` | `this.deps.objectStore` に対して list keys を実行する。 | `MemoRagService.listBenchmarkDocumentManifests` | `apps/api/src/rag/memorag-service.ts:958 (MemoRagService.listBenchmarkDocumentManifests)` |

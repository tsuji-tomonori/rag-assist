...
| M012 | ログ | `-` | Skipping missing document manifest listed by object store | is missing object error の判定結果が真である | `apps/api/src/rag/memorag-service.ts:930 (MemoRagService.listDocuments)` |
| M013 | 例外 | `-` | Benchmark evaluation tenant is not configured | `config.benchmarkEvaluationEnabled` が存在しない、または偽である、または `tenantId` が存在しない、または偽である | `apps/api/src/rag/memorag-service.ts:977 (MemoRagService.listBenchmarkDocumentManifests)` |

...
# GET /admin/users/{userId}/deletion-preflight メッセージ仕様
...
| M014 | 例外 | `-` | Authoritative account deletion preflight is not configured | `config.authEnabled` が存在し、真である | `apps/api/src/rag/memorag-service.ts:1721 (MemoRagService.getManagedUserDeletionPreflight)` |
| M015 | HTTP 実装応答 | `404` | User not found | `preflight` が存在しない、または偽である | `apps/api/src/routes/admin-routes.ts:172 (GET /admin/users/{userId}/deletion-preflight handler)` |

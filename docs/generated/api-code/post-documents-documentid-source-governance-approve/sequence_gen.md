...
| 6 | `MemoRagService.approveSourceGovernance` | Service | service の get manifest 処理を呼び出す。 | `this.getManifest(documentId, authoritativeActorTenantId(actor))` | `apps/api/src/rag/memorag-service.ts:648 (MemoRagService.approveSourceGovernance)` |
...
| 8 | `MemoRagService.approveSourceGovernance` | Service | service の source governance approval service 処理を呼び出す。 | `this.sourceGovernanceApprovalService()` | `apps/api/src/rag/memorag-service.ts:649 (MemoRagService.approveSourceGovernance)` |
...
| 21 | `MemoRagService.stageApprovedSourceGovernancePublication` | Store | `this.deps.objectStore` に対して get text を実行する。 | `this.deps.objectStore.getText(input.source.sourceObjectKey)` | `apps/api/src/rag/memorag-service.ts:3951 (MemoRagService.stageApprovedSourceGovernancePublication)` |
...
| 23 | `MemoRagService.createMemoryCards` | External | `textModel` へ generate を実行する。 | `textModel.generate( buildMemoryCardPrompt(input.fileName, input.text), llmOptions("memoryCard", input.modelId ?? config.defaultMemoryModelId) )` | `apps/api/src/rag/memorag-service.ts:4834 (MemoRagService.createMemoryCards)` |
...
| 88 | `MemoRagService.approveSourceGovernance` | Service | service の approve 処理を呼び出す。 | `this.sourceGovernanceApprovalService().approve(actor, manifest, input)` | `apps/api/src/rag/memorag-service.ts:649 (MemoRagService.approveSourceGovernance)` |

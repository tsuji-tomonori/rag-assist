...
| ---: | --- | --- | --- | --- | --- |
| 1 | `POST /documents/{documentId}/source-governance/approve handler` | Auth | 認証済み利用者を request context から取得する。 | `c.get("user")` | `apps/api/src/routes/document-routes.ts:1343 (POST /documents/{documentId}/source-governance/approve handler)` |
| 2 | `POST /documents/{documentId}/source-governance/approve handler` | Auth | "rag:source:approve" permission を必須条件として確認する。 | `requirePermission(user, "rag:source:approve")` | `apps/api/src/routes/document-routes.ts:1344 (POST /documents/{documentId}/source-governance/approve handler)` |
| 3 | `POST /documents/{documentId}/source-governance/approve handler` | Validation | schema 検証済みの path parameter を取得する。 | `validParam<{ documentId: string }>(c)` | `apps/api/src/routes/document-routes.ts:1345 (POST /documents/{documentId}/source-governance/approve handler)` |
| 4 | `POST /documents/{documentId}/source-governance/approve handler` | Validation | schema 検証済みの JSON request body を取得する。 | `validJson<ApproveSourceGovernanceInput>(c)` | `apps/api/src/routes/document-routes.ts:1346 (POST /documents/{documentId}/source-governance/approve handler)` |
| 5 | `POST /documents/{documentId}/source-governance/approve handler` | Service | service の approve source governance 処理を呼び出す。 | `service.approveSourceGovernance(user, documentId, body)` | `apps/api/src/routes/document-routes.ts:1348 (POST /documents/{documentId}/source-governance/approve handler)` |
| 6 | `MemoRagService.approveSourceGovernance` | Service | service の get manifest 処理を呼び出す。 | `this.getManifest(documentId, authoritativeActorTenantId(actor))` | `apps/api/src/rag/memorag-service.ts:649 (MemoRagService.approveSourceGovernance)` |
...
| 8 | `MemoRagService.approveSourceGovernance` | Service | service の source governance approval service 処理を呼び出す。 | `this.sourceGovernanceApprovalService()` | `apps/api/src/rag/memorag-service.ts:650 (MemoRagService.approveSourceGovernance)` |
...
| 21 | `MemoRagService.stageApprovedSourceGovernancePublication` | Store | `this.deps.objectStore` に対して get text を実行する。 | `this.deps.objectStore.getText(input.source.sourceObjectKey)` | `apps/api/src/rag/memorag-service.ts:3962 (MemoRagService.stageApprovedSourceGovernancePublication)` |
...
| 23 | `MemoRagService.createMemoryCards` | External | `textModel` へ generate を実行する。 | `textModel.generate( buildMemoryCardPrompt(input.fileName, input.text), llmOptions("memoryCard", input.modelId ?? config.defaultMemoryModelId) )` | `apps/api/src/rag/memorag-service.ts:4845 (MemoRagService.createMemoryCards)` |
| 24 | `assertRagSafetyInterlock` | Store | `input.objectStore` に対して get text を実行する。 | `input.objectStore.getText(RAG_SAFETY_STATE_KEY)` | `apps/api/src/rag/quality-control/production-rag-monitor.ts:315 (assertRagSafetyInterlock)` |
...
| 88 | `MemoRagService.approveSourceGovernance` | Service | service の approve 処理を呼び出す。 | `this.sourceGovernanceApprovalService().approve(actor, manifest, input)` | `apps/api/src/rag/memorag-service.ts:650 (MemoRagService.approveSourceGovernance)` |
...
| 91 | `SourceGovernanceApprovalService.approve` | Store | `new ObjectStoreRevocationCleanupRepairOutbox(this.deps.objectStore)` に対して assert resource fence released を実行する。 | `new ObjectStoreRevocationCleanupRepairOutbox(this.deps.objectStore).assertResourceFenceReleased( state.record.tenantId, cleanupResourceType(manifest), state.record.sourceId )` | `apps/api/src/rag/offline/pre-retrieval/admission/source-governance-approval-service.ts:306 (SourceGovernanceApprovalService.approve)` |
...
| 97 | `POST /documents/{documentId}/source-governance/approve handler` | HTTP/SSE | HTTP 200 で JSON response を返す。 | `c.json(publicSourceGovernanceRecord(await service.approveSourceGovernance(user, documentId, body)), 200)` | `apps/api/src/routes/document-routes.ts:1348 (POST /documents/{documentId}/source-governance/approve handler)` |
| 98 | `POST /documents/{documentId}/source-governance/approve handler` | HTTP/SSE | HTTP 400 で JSON response を返す。 | `c.json({ error: error.message }, 400)` | `apps/api/src/routes/document-routes.ts:1350 (POST /documents/{documentId}/source-governance/approve handler)` |
| 99 | `POST /documents/{documentId}/source-governance/approve handler` | HTTP/SSE | HTTP 409 で JSON response を返す。 | `c.json({ error: error.message }, 409)` | `apps/api/src/routes/document-routes.ts:1352 (POST /documents/{documentId}/source-governance/approve handler)` |
| 100 | `POST /documents/{documentId}/source-governance/approve handler` | HTTP/SSE | HTTP 503 で JSON response を返す。 | `c.json({ error: error.message }, 503)` | `apps/api/src/routes/document-routes.ts:1353 (POST /documents/{documentId}/source-governance/approve handler)` |

## 分岐

| ID | Function | 条件 | 実装位置 |
| --- | --- | --- | --- |
| B001 | `POST /documents/{documentId}/source-governance/approve handler` | 例外が発生した場合に catch 処理へ移る | `apps/api/src/routes/document-routes.ts:1349 (POST /documents/{documentId}/source-governance/approve handler)` |
| B002 | `POST /documents/{documentId}/source-governance/approve handler` | `error` が `SourceGovernanceValidationError` の instance である | `apps/api/src/routes/document-routes.ts:1350 (POST /documents/{documentId}/source-governance/approve handler)` |
| B003 | `POST /documents/{documentId}/source-governance/approve handler` | `error` が `SourceGovernanceDeniedError` の instance である | `apps/api/src/routes/document-routes.ts:1351 (POST /documents/{documentId}/source-governance/approve handler)` |
| B004 | `POST /documents/{documentId}/source-governance/approve handler` | `error` が `SourceGovernanceConflictError` の instance である | `apps/api/src/routes/document-routes.ts:1352 (POST /documents/{documentId}/source-governance/approve handler)` |
| B005 | `POST /documents/{documentId}/source-governance/approve handler` | `error` が `SourceGovernanceUnavailableError` の instance である | `apps/api/src/routes/document-routes.ts:1353 (POST /documents/{documentId}/source-governance/approve handler)` |
| B006 | `POST /documents/{documentId}/source-governance/approve handler` | `error` が `Error` の instance である、かつ `error.message` が "ENOENT" を含む、または `error.message` が "NoSuchKey" を含む | `apps/api/src/routes/document-routes.ts:1354 (POST /documents/{documentId}/source-governance/approve handler)` |

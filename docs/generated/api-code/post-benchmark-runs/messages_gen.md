...
| M006 | 例外 | `-` | `Unknown benchmark suite: ${input.suiteId}` | `suite` が存在しない、または偽である | `apps/api/src/rag/memorag-service.ts:4566 (MemoRagService.createBenchmarkRun)` |
| M007 | 例外 | `-` | `Suite ${suite.suiteId} does not support mode ${input.mode}` | `(input.mode ?? suite.mode)` が `suite.mode` と異なる | `apps/api/src/rag/memorag-service.ts:4567 (MemoRagService.createBenchmarkRun)` |
| M008 | 例外 | `-` | Only codebuild runner is supported in this version | `(input.runner ?? "codebuild")` が `"codebuild"` と異なる | `apps/api/src/rag/memorag-service.ts:4568 (MemoRagService.createBenchmarkRun)` |

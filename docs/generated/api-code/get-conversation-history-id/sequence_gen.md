...
| 5 | `MemoRagService.getConversationHistory` | Store | `this.deps.conversationHistoryStore` に対して get を実行する。 | `this.deps.conversationHistoryStore.get(tenantPartitionedOwnerKey(subject, tenantId), id)` | `apps/api/src/rag/memorag-service.ts:5181 (MemoRagService.getConversationHistory)` |
...
| B003 | `MemoRagService.getConversationHistory` | `item` が存在し、真である | `apps/api/src/rag/memorag-service.ts:5182 (MemoRagService.getConversationHistory)` |

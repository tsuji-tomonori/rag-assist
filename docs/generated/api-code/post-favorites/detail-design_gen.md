...
| B002 | `MemoRagService.saveFavorite` | if | favorite target resolver implemented の判定結果が真ではない | `!favoriteTargetResolverImplemented(input.targetType)` | `apps/api/src/rag/memorag-service.ts:3815 (MemoRagService.saveFavorite)` |
...
| 1 | `MemoRagService.saveFavorite` | save favorite の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:3814 (MemoRagService.saveFavorite)` |
| 2 | `favoriteTargetResolverImplemented` | favorite target resolver implemented の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5895 (favoriteTargetResolverImplemented)` |
| 2 | `tenantPartitionedOwnerKey` | tenant partitioned owner key の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5513 (tenantPartitionedOwnerKey)` |
| 2 | `MemoRagService.resolveFavoriteVisibility` | resolve favorite visibility の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:3850 (MemoRagService.resolveFavoriteVisibility)` |

## 5. データ・外部境界

| 種別 | 境界 | Target | Operation | 目的 | Caller | 実装位置 |
| --- | --- | --- | --- | --- | --- | --- |
| 作成・追記 | Store | `this.deps.favoriteStore` | `save` | `this.deps.favoriteStore` に対して save を実行する。 | `MemoRagService.saveFavorite` | `apps/api/src/rag/memorag-service.ts:3818 (MemoRagService.saveFavorite)` |
| 参照 | Store | `this.deps.conversationHistoryStore` | `list` | `this.deps.conversationHistoryStore` に対して list を実行する。 | `MemoRagService.resolveFavoriteVisibility` | `apps/api/src/rag/memorag-service.ts:3852 (MemoRagService.resolveFavoriteVisibility)` |
| 参照 | Store | `this.deps.objectStore` | `listKeys` | `this.deps.objectStore` に対して list keys を実行する。 | `MemoRagService.listDocuments` | `apps/api/src/rag/memorag-service.ts:802 (MemoRagService.listDocuments)` |

...
| 1 | `MemoRagService.saveConversationHistory` | save conversation history の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:4150 (MemoRagService.saveConversationHistory)` |
| 2 | `ConversationHistoryService.save` | save の実装処理を担当する。 | `apps/api/src/conversations/conversation-history-service.ts:24 (ConversationHistoryService.save)` |

## 5. データ・外部境界

| 種別 | 境界 | Target | Operation | 目的 | Caller | 実装位置 |
| --- | --- | --- | --- | --- | --- | --- |
| 作成・追記 | Store | `this.deps.conversationHistoryStore` | `save` | `this.deps.conversationHistoryStore` に対して save を実行する。 | `ConversationHistoryService.save` | `apps/api/src/conversations/conversation-history-service.ts:31 (ConversationHistoryService.save)` |

...
| F003 | `MemoRagService.getBenchmarkCodeBuildLogText` | if | `run` が存在しない、または偽である | `apps/api/src/rag/memorag-service.ts:4737 (MemoRagService.getBenchmarkCodeBuildLogText)` |
| F004 | `MemoRagService.getBenchmarkCodeBuildLogText` | if | `text` が `undefined` と等しい | `apps/api/src/rag/memorag-service.ts:4744 (MemoRagService.getBenchmarkCodeBuildLogText)` |
...
| TC006 | F003: 条件成立 | `run` が存在しない、または偽である 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:4737 (MemoRagService.getBenchmarkCodeBuildLogText)` |
| TC007 | F003: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:4737 (MemoRagService.getBenchmarkCodeBuildLogText)` |
| TC008 | F004: 条件成立 | `text` が `undefined` と等しい 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:4744 (MemoRagService.getBenchmarkCodeBuildLogText)` |
| TC009 | F004: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:4744 (MemoRagService.getBenchmarkCodeBuildLogText)` |

...
| ---: | --- | --- | --- | --- | --- |
| 1 | `GET /documents/{documentId}/share handler` | Auth | 認証済み利用者を request context から取得する。 | `c.get("user")` | `apps/api/src/routes/document-routes.ts:883 (GET /documents/{documentId}/share handler)` |
| 2 | `GET /documents/{documentId}/share handler` | Auth | "rag:doc:share" permission を必須条件として確認する。 | `requirePermission(user, "rag:doc:share")` | `apps/api/src/routes/document-routes.ts:884 (GET /documents/{documentId}/share handler)` |
| 3 | `GET /documents/{documentId}/share handler` | Validation | schema 検証済みの path parameter を取得する。 | `validParam<{ documentId: string }>(c)` | `apps/api/src/routes/document-routes.ts:885 (GET /documents/{documentId}/share handler)` |
| 4 | `GET /documents/{documentId}/share handler` | Service | service の get document share info 処理を呼び出す。 | `service.getDocumentShareInfo(user, documentId)` | `apps/api/src/routes/document-routes.ts:887 (GET /documents/{documentId}/share handler)` |
...
| 17 | `GET /documents/{documentId}/share handler` | HTTP/SSE | HTTP 200 で JSON response を返す。 | `c.json(await service.getDocumentShareInfo(user, documentId), 200)` | `apps/api/src/routes/document-routes.ts:887 (GET /documents/{documentId}/share handler)` |

## 分岐

| ID | Function | 条件 | 実装位置 |
| --- | --- | --- | --- |
| B001 | `GET /documents/{documentId}/share handler` | 例外が発生した場合に catch 処理へ移る | `apps/api/src/routes/document-routes.ts:888 (GET /documents/{documentId}/share handler)` |
| B002 | `GET /documents/{documentId}/share handler` | is forbidden error の判定結果が真である | `apps/api/src/routes/document-routes.ts:889 (GET /documents/{documentId}/share handler)` |
| B003 | `GET /documents/{documentId}/share handler` | `err` が `Error` の instance である、かつ `err.message` が "ENOENT" を含む、または `err.message` が "NoSuchKey" を含む | `apps/api/src/routes/document-routes.ts:890 (GET /documents/{documentId}/share handler)` |

...
| 5 | `MemoRagService.cancelBenchmarkRun` | Service | service の cancel 処理を呼び出す。 | `this.benchmarkRunCancellationService.cancel(actor, runId)` | `apps/api/src/rag/memorag-service.ts:4553 (MemoRagService.cancelBenchmarkRun)` |
...
| 7 | `BenchmarkRunCancellationService.cancel` | Store | `this.ports.benchmarkRunStore` に対して update を実行する。 | `this.ports.benchmarkRunStore.update(tenantId, runId, { status: "cancelled", completedAt: this.ports.now() })` | `apps/api/src/benchmark/benchmark-run-cancellation-service.ts:35 (BenchmarkRunCancellationService.cancel)` |

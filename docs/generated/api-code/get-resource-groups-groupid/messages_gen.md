...
# GET /resource-groups/{groupId} メッセージ仕様
...
| M010 | 例外 | `-` | Forbidden | `group` が存在しない、または偽である、または `group.status` が `"active"` と異なる、または canonical の判定結果が真ではない | `apps/api/src/security/resource-group-lifecycle-service.ts:562 (ResourceGroupLifecycleService.loadActiveTarget)` |
| M011 | 例外 | `-` | Forbidden | canonical の判定結果が真ではない | `apps/api/src/security/resource-group-lifecycle-service.ts:855 (authoritativeGroupTenant)` |
| M012 | 例外 | `-` | Forbidden | `minimum` が `"full"` と等しい、かつ `permission.permission` が `"full"` と異なる | `apps/api/src/security/resource-group-lifecycle-service.ts:473 (ResourceGroupLifecycleService.authorizeTarget)` |
| M013 | HTTP 実装応答 | `503` | Resource group lifecycle unavailable | 例外を捕捉した場合 | `apps/api/src/routes/resource-group-routes.ts:134 (GET /resource-groups/{groupId} handler)` |

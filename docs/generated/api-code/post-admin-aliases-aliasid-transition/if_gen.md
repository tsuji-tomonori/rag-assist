...
- 実装 route: `apps/api/src/routes/admin-routes.ts:480 (POST /admin/aliases/{aliasId}/transition)`
...
| 項目 | 型 | 必須 | 説明 | 制約 |
| --- | --- | --- | --- | --- |
| `targetStatus` | `enum(draft)` | yes | `data.targetStatus` の値。項目名は target status を表します。 | enum=draft |
...
| `503` | alias 状態遷移の永続化を完了できません | `application/json` | 2 field(s) |

...
| 5 | `MemoRagService.getParsedDocumentPreview` | Service | service の get manifest 処理を呼び出す。 | `this.getManifest(documentId, authoritativeActorTenantId(user))` | `apps/api/src/rag/memorag-service.ts:1003 (MemoRagService.getParsedDocumentPreview)` |
...
| 7 | `MemoRagService.getParsedDocumentPreview` | Service | service の catch 処理を呼び出す。 | `this.getManifest(documentId, authoritativeActorTenantId(user)).catch((error: unknown) => { if (isMissingObjectError(error)) return undefined throw error })` | `apps/api/src/rag/memorag-service.ts:1003 (MemoRagService.getParsedDocumentPreview)` |
| 8 | `MemoRagService.getParsedDocumentPreview` | Service | service の can access document manifest 処理を呼び出す。 | `this.canAccessDocumentManifest(user, manifest)` | `apps/api/src/rag/memorag-service.ts:1008 (MemoRagService.getParsedDocumentPreview)` |
...
| B005 | `MemoRagService.getParsedDocumentPreview` | is missing object error の判定結果が真である | `apps/api/src/rag/memorag-service.ts:1004 (MemoRagService.getParsedDocumentPreview)` |
| B006 | `MemoRagService.getParsedDocumentPreview` | `manifest` が存在しない、または偽である | `apps/api/src/rag/memorag-service.ts:1007 (MemoRagService.getParsedDocumentPreview)` |
| B007 | `MemoRagService.getParsedDocumentPreview` | 条件式 `await this.canAccessDocumentManifest(user, manifest)` が成立しない | `apps/api/src/rag/memorag-service.ts:1008 (MemoRagService.getParsedDocumentPreview)` |

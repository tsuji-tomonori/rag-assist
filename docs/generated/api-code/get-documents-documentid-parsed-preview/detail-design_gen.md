...
| B005 | `MemoRagService.getParsedDocumentPreview` | if | is missing object error の判定結果が真である | `isMissingObjectError(error)` | `apps/api/src/rag/memorag-service.ts:834 (MemoRagService.getParsedDocumentPreview)` |
| B006 | `MemoRagService.getParsedDocumentPreview` | if | `manifest` が存在しない、または偽である | `!manifest` | `apps/api/src/rag/memorag-service.ts:837 (MemoRagService.getParsedDocumentPreview)` |
| B007 | `MemoRagService.getParsedDocumentPreview` | if | 条件式 `await this.canAccessDocumentManifest(user, manifest)` が成立しない | `!(await this.canAccessDocumentManifest(user, manifest))` | `apps/api/src/rag/memorag-service.ts:838 (MemoRagService.getParsedDocumentPreview)` |
...
| 1 | `MemoRagService.getParsedDocumentPreview` | get parsed document preview の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:832 (MemoRagService.getParsedDocumentPreview)` |
...
| 2 | `buildParsedDocumentPreview` | build parsed document preview の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:4692 (buildParsedDocumentPreview)` |

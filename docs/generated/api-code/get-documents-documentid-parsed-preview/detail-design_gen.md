...
| B005 | `MemoRagService.getParsedDocumentPreview` | if | is missing object error の判定結果が真である | `isMissingObjectError(error)` | `apps/api/src/rag/memorag-service.ts:1071 (MemoRagService.getParsedDocumentPreview)` |
| B006 | `MemoRagService.getParsedDocumentPreview` | if | `manifest` が存在しない、または偽である | `!manifest` | `apps/api/src/rag/memorag-service.ts:1074 (MemoRagService.getParsedDocumentPreview)` |
| B007 | `MemoRagService.getParsedDocumentPreview` | if | 条件式 `await this.canAccessDocumentManifest(user, manifest)` が成立しない | `!(await this.canAccessDocumentManifest(user, manifest))` | `apps/api/src/rag/memorag-service.ts:1075 (MemoRagService.getParsedDocumentPreview)` |
...
| 1 | `MemoRagService.getParsedDocumentPreview` | get parsed document preview の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:1069 (MemoRagService.getParsedDocumentPreview)` |
...
| 2 | `MemoRagService.canAccessDocumentManifest` | can access document manifest の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:1316 (MemoRagService.canAccessDocumentManifest)` |
| 2 | `buildParsedDocumentPreview` | build parsed document preview の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5134 (buildParsedDocumentPreview)` |

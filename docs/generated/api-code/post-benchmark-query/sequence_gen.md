...
| ---: | --- | --- | --- | --- | --- |
| 1 | `POST /benchmark/query handler` | Auth | 認証済み利用者を request context から取得する。 | `c.get("user")` | `apps/api/src/routes/benchmark-routes.ts:42 (POST /benchmark/query handler)` |
| 2 | `POST /benchmark/query handler` | Auth | "benchmark:query" permission を必須条件として確認する。 | `requirePermission(c.get("user"), "benchmark:query")` | `apps/api/src/routes/benchmark-routes.ts:42 (POST /benchmark/query handler)` |
| 3 | `POST /benchmark/query handler` | Validation | schema 検証済みの JSON request body を取得する。 | `validJson<z.infer<typeof BenchmarkQueryRequestSchema>>(c)` | `apps/api/src/routes/benchmark-routes.ts:43 (POST /benchmark/query handler)` |
| 4 | `POST /benchmark/query handler` | Auth | 認証済み利用者を request context から取得する。 | `c.get("user")` | `apps/api/src/routes/benchmark-routes.ts:52 (POST /benchmark/query handler)` |
| 5 | `POST /benchmark/query handler` | Service | service の chat 処理を呼び出す。 | `service.chat({ ...body, includeDebug: body.includeDebug ?? true, searchFilters: { source: "benchmark-runner", docType: "benchmark-corpus", benchmarkSuiteId: body.benchmarkSuiteId } }, c.get("user"))` | `apps/api/src/routes/benchmark-routes.ts:44 (POST /benchmark/query handler)` |
...
| 54 | `POST /benchmark/query handler` | HTTP/SSE | HTTP 200 で JSON response を返す。 | `c.json({ id: body.id, ...result }, 200)` | `apps/api/src/routes/benchmark-routes.ts:53 (POST /benchmark/query handler)` |
...
| B002 | `MemoRagService.chat` | `user` が存在し、真である | `apps/api/src/rag/memorag-service.ts:1089 (MemoRagService.chat)` |

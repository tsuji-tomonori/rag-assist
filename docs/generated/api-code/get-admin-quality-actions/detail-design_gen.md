...
| Route 定義 | `apps/api/src/routes/admin-routes.ts:469 (GET /admin/quality-actions)` |
| Handler | `apps/api/src/routes/admin-routes.ts:485 (GET /admin/quality-actions handler)` |

## 2. 主処理

| # | 深さ | 種別 | 自然言語での処理 | コード根拠 | 実装位置 |
| ---: | ---: | --- | --- | --- | --- |
| 1 | 0 | 認証・認可 | 認証済み利用者を request context から取得する。 | `user = c.get("user")` | `apps/api/src/routes/admin-routes.ts:486` |
| 2 | 0 | 認証・認可 | "rag:doc:read" permission を必須条件として確認する。 | `requirePermission(user, "rag:doc:read")` | `apps/api/src/routes/admin-routes.ts:487` |
| 3 | 0 | 応答 | 処理結果を HTTP 200 の JSON response として返す。 | `return c.json({ actions: await service.listQualityActionCards(user) }, 200)` | `apps/api/src/routes/admin-routes.ts:488` |
...
| 深さ | Symbol | 責務 | 実装位置 |
| ---: | --- | --- | --- |
| 0 | `GET /admin/quality-actions handler` | GET /admin/quality-actions の request を受け、検証・認可・service 呼び出し・HTTP 応答を調整する。 | `apps/api/src/routes/admin-routes.ts:485 (GET /admin/quality-actions handler)` |
...
| 1 | `MemoRagService.listQualityActionCards` | list quality action cards の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:1740 (MemoRagService.listQualityActionCards)` |
...
| 2 | `qualityActionCardsForManifest` | quality action cards for manifest の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:4737 (qualityActionCardsForManifest)` |

...
| ID | Function | 条件 | 実装位置 |
| --- | --- | --- | --- |
| B001 | `getPermissionsForGroups` | `groups` が存在し、真である | `apps/api/src/authorization.ts:214 (getPermissionsForGroups)` |
| B002 | `getPermissionsForGroups` | `rolePermissions[group as Role]` が `[]` の条件を満たす | `apps/api/src/authorization.ts:215 (getPermissionsForGroups)` |

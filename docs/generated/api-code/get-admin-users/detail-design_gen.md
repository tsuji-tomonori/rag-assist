...
| B004 | `MemoRagService.listManagedUsersPage` | 三項条件 | `this.deps.verifiedIdentityProvider` が存在し、真である | `this.deps.verifiedIdentityProvider` | `apps/api/src/rag/memorag-service.ts:1705 (MemoRagService.listManagedUsersPage)` |
| B005 | `MemoRagService.listManagedUsersPage` | if | `actor.userId` が `user.userId` と等しい | `actor.userId === user.userId` | `apps/api/src/rag/memorag-service.ts:1712 (MemoRagService.listManagedUsersPage)` |
| B006 | `MemoRagService.listManagedUsersPage` | if | `user.status` が `"active"` と異なる | `user.status !== "active"` | `apps/api/src/rag/memorag-service.ts:1713 (MemoRagService.listManagedUsersPage)` |
| B007 | `MemoRagService.listManagedUsersPage` | if | `user.groups` が "SYSTEM_ADMIN" を含む、かつ `activeRecoveryPrincipals.length` が `1` 以下である | `user.groups.includes("SYSTEM_ADMIN") && activeRecoveryPrincipals.length <= 1` | `apps/api/src/rag/memorag-service.ts:1714 (MemoRagService.listManagedUsersPage)` |
| B008 | `MemoRagService.listManagedUsersPage` | 三項条件 | `query.sort` が `"updatedDesc"` と等しい | `query.sort === "updatedDesc"` | `apps/api/src/rag/memorag-service.ts:1728 (MemoRagService.listManagedUsersPage)` |
| B009 | `MemoRagService.listManagedUsersPage` | 三項条件 | `sort` が `"updatedDesc"` と等しい | `sort === "updatedDesc"` | `apps/api/src/rag/memorag-service.ts:1737 (MemoRagService.listManagedUsersPage)` |
| B010 | `MemoRagService.listManagedUsersPage` | 三項条件 | `sort` が `"updatedDesc"` と等しい | `sort === "updatedDesc"` | `apps/api/src/rag/memorag-service.ts:1738 (MemoRagService.listManagedUsersPage)` |
...
| 1 | `MemoRagService.listManagedUsersPage` | list managed users page の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:1697 (MemoRagService.listManagedUsersPage)` |
...
| --- | --- | --- | --- | --- | --- | --- |
| 参照 | Store | `this` | `loadAdminLedger` | `this` に対して load admin ledger を実行する。 | `MemoRagService.listManagedUsersPage` | `apps/api/src/rag/memorag-service.ts:1698 (MemoRagService.listManagedUsersPage)` |

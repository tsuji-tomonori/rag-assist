...
| 4 | `MemoRagService.listManagedUsersPage` | Store | `this` に対して load admin ledger を実行する。 | `this.loadAdminLedger(actor, { syncUserDirectory: true })` | `apps/api/src/rag/memorag-service.ts:1782 (MemoRagService.listManagedUsersPage)` |
...
| B004 | `MemoRagService.listManagedUsersPage` | `this.deps.verifiedIdentityProvider` が存在し、真である | `apps/api/src/rag/memorag-service.ts:1789 (MemoRagService.listManagedUsersPage)` |
| B005 | `MemoRagService.listManagedUsersPage` | `actor.userId` が `user.userId` と等しい | `apps/api/src/rag/memorag-service.ts:1796 (MemoRagService.listManagedUsersPage)` |
| B006 | `MemoRagService.listManagedUsersPage` | `user.status` が `"active"` と異なる | `apps/api/src/rag/memorag-service.ts:1797 (MemoRagService.listManagedUsersPage)` |
| B007 | `MemoRagService.listManagedUsersPage` | `user.groups` が "SYSTEM_ADMIN" を含む、かつ `activeRecoveryPrincipals.length` が `1` 以下である | `apps/api/src/rag/memorag-service.ts:1798 (MemoRagService.listManagedUsersPage)` |
| B008 | `MemoRagService.listManagedUsersPage` | `query.sort` が `"updatedDesc"` と等しい | `apps/api/src/rag/memorag-service.ts:1812 (MemoRagService.listManagedUsersPage)` |
| B009 | `MemoRagService.listManagedUsersPage` | `sort` が `"updatedDesc"` と等しい | `apps/api/src/rag/memorag-service.ts:1821 (MemoRagService.listManagedUsersPage)` |
| B010 | `MemoRagService.listManagedUsersPage` | `sort` が `"updatedDesc"` と等しい | `apps/api/src/rag/memorag-service.ts:1822 (MemoRagService.listManagedUsersPage)` |

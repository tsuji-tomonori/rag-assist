...
| B002 | `MemoRagService.listFavorites` | if | `favorite.targetType` が `"chatSession"` と等しい | `favorite.targetType === "chatSession"` | `apps/api/src/rag/memorag-service.ts:3835 (MemoRagService.listFavorites)` |
| B003 | `MemoRagService.listFavorites` | if | `favorite.targetType` が `"document"` と等しい | `favorite.targetType === "document"` | `apps/api/src/rag/memorag-service.ts:3838 (MemoRagService.listFavorites)` |
| B004 | `MemoRagService.listFavorites` | if | `favorite.targetType` が `"folder"` と等しい | `favorite.targetType === "folder"` | `apps/api/src/rag/memorag-service.ts:3842 (MemoRagService.listFavorites)` |
...
| 1 | `MemoRagService.listFavorites` | list favorites の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:3826 (MemoRagService.listFavorites)` |
...
| 2 | `MemoRagService.listDocuments` | list documents の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:800 (MemoRagService.listDocuments)` |
| 2 | `MemoRagService.listDocumentGroups` | list document groups の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:917 (MemoRagService.listDocumentGroups)` |
| 2 | `favoriteListItem` | favorite list item の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5875 (favoriteListItem)` |

## 5. データ・外部境界

| 種別 | 境界 | Target | Operation | 目的 | Caller | 実装位置 |
| --- | --- | --- | --- | --- | --- | --- |
| 参照 | Store | `this.deps.favoriteStore` | `list` | `this.deps.favoriteStore` に対して list を実行する。 | `MemoRagService.listFavorites` | `apps/api/src/rag/memorag-service.ts:3828 (MemoRagService.listFavorites)` |
| 参照 | Store | `this.deps.conversationHistoryStore` | `list` | `this.deps.conversationHistoryStore` に対して list を実行する。 | `MemoRagService.listFavorites` | `apps/api/src/rag/memorag-service.ts:3829 (MemoRagService.listFavorites)` |

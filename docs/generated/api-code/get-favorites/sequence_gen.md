...
| 4 | `MemoRagService.listFavorites` | Store | `this.deps.favoriteStore` に対して list を実行する。 | `this.deps.favoriteStore.list(tenantPartitionedOwnerKey(user))` | `apps/api/src/rag/memorag-service.ts:3500 (MemoRagService.listFavorites)` |
| 5 | `MemoRagService.listFavorites` | Store | `this.deps.conversationHistoryStore` に対して list を実行する。 | `this.deps.conversationHistoryStore.list(tenantPartitionedOwnerKey(user))` | `apps/api/src/rag/memorag-service.ts:3501 (MemoRagService.listFavorites)` |
| 6 | `MemoRagService.listFavorites` | Service | service の list documents 処理を呼び出す。 | `this.listDocuments(user)` | `apps/api/src/rag/memorag-service.ts:3504 (MemoRagService.listFavorites)` |
...
| 20 | `MemoRagService.listFavorites` | Service | service の list document groups 処理を呼び出す。 | `this.listDocumentGroups(user)` | `apps/api/src/rag/memorag-service.ts:3505 (MemoRagService.listFavorites)` |
| 21 | `MemoRagService.listDocumentGroups` | Store | `this.deps.documentGroupStore` に対して list を実行する。 | `this.deps.documentGroupStore.list(authoritativeActorTenantId(user))` | `apps/api/src/rag/memorag-service.ts:870 (MemoRagService.listDocumentGroups)` |
...
| B002 | `MemoRagService.listFavorites` | `favorite.targetType` が `"chatSession"` と等しい | `apps/api/src/rag/memorag-service.ts:3507 (MemoRagService.listFavorites)` |
| B003 | `MemoRagService.listFavorites` | `favorite.targetType` が `"document"` と等しい | `apps/api/src/rag/memorag-service.ts:3510 (MemoRagService.listFavorites)` |
| B004 | `MemoRagService.listFavorites` | `favorite.targetType` が `"folder"` と等しい | `apps/api/src/rag/memorag-service.ts:3514 (MemoRagService.listFavorites)` |

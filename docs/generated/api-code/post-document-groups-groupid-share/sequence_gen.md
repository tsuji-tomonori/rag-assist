...
| 6 | `MemoRagService.updateDocumentGroupSharing` | Store | `this.deps.documentGroupStore` に対して list を実行する。 | `this.deps.documentGroupStore.list(authoritativeActorTenantId(actor))` | `apps/api/src/rag/memorag-service.ts:1221 (MemoRagService.updateDocumentGroupSharing)` |
...
| 13 | `MemoRagService.updateDocumentGroupSharing` | Store | `this.deps.documentGroupStore` に対して update with path locks を実行する。 | `this.deps.documentGroupStore.updateWithPathLocks(group.tenantId, pathUpdates)` | `apps/api/src/rag/memorag-service.ts:1251 (MemoRagService.updateDocumentGroupSharing)` |
| 14 | `MemoRagService.updateDocumentGroupSharing` | Store | `this.deps.documentGroupStore` に対して update with path locks を実行する。 | `this.deps.documentGroupStore.updateWithPathLocks(group.tenantId, [{ current: group, next: { ...group, ...update, updatedAt: now } }])` | `apps/api/src/rag/memorag-service.ts:1254 (MemoRagService.updateDocumentGroupSharing)` |
...
| B006 | `MemoRagService.updateDocumentGroupSharing` | `group` が存在しない、または偽である | `apps/api/src/rag/memorag-service.ts:1223 (MemoRagService.updateDocumentGroupSharing)` |
| B007 | `MemoRagService.updateDocumentGroupSharing` | `(await folderPermissions.resolveEffectiveFolderPermission(actor, group.groupId))` が `"full"` と異なる | `apps/api/src/rag/memorag-service.ts:1225 (MemoRagService.updateDocumentGroupSharing)` |
| B008 | `MemoRagService.updateDocumentGroupSharing` | `input.name` が `undefined` と異なる | `apps/api/src/rag/memorag-service.ts:1231 (MemoRagService.updateDocumentGroupSharing)` |
| B009 | `MemoRagService.updateDocumentGroupSharing` | `input.description` が `undefined` と異なる | `apps/api/src/rag/memorag-service.ts:1236 (MemoRagService.updateDocumentGroupSharing)` |
| B010 | `MemoRagService.updateDocumentGroupSharing` | `group.parentGroupId` が存在し、真である | `apps/api/src/rag/memorag-service.ts:1237 (MemoRagService.updateDocumentGroupSharing)` |
| B011 | `MemoRagService.updateDocumentGroupSharing` | `input.name` が `undefined` と異なる | `apps/api/src/rag/memorag-service.ts:1239 (MemoRagService.updateDocumentGroupSharing)` |
| B012 | `MemoRagService.updateDocumentGroupSharing` | `pathUpdates.length` が `maxDocumentGroupPathTransactionItems` より大きい | `apps/api/src/rag/memorag-service.ts:1241 (MemoRagService.updateDocumentGroupSharing)` |
| B013 | `MemoRagService.updateDocumentGroupSharing` | map の判定結果が真である | `apps/api/src/rag/memorag-service.ts:1242 (MemoRagService.updateDocumentGroupSharing)` |
| B014 | `MemoRagService.updateDocumentGroupSharing` | `conflict` が存在し、真である | `apps/api/src/rag/memorag-service.ts:1249 (MemoRagService.updateDocumentGroupSharing)` |

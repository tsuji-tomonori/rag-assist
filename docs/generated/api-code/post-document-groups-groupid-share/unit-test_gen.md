...
| F009 | `MemoRagService.updateDocumentGroupSharing` | if | `input.description` が `undefined` と異なる | `apps/api/src/rag/memorag-service.ts:1128 (MemoRagService.updateDocumentGroupSharing)` |
| F010 | `MemoRagService.updateDocumentGroupSharing` | 三項条件 | `group.parentGroupId` が存在し、真である | `apps/api/src/rag/memorag-service.ts:1129 (MemoRagService.updateDocumentGroupSharing)` |
| F011 | `MemoRagService.updateDocumentGroupSharing` | if | `input.name` が `undefined` と異なる | `apps/api/src/rag/memorag-service.ts:1131 (MemoRagService.updateDocumentGroupSharing)` |
| F012 | `MemoRagService.updateDocumentGroupSharing` | if | `pathUpdates.length` が `maxDocumentGroupPathTransactionItems` より大きい | `apps/api/src/rag/memorag-service.ts:1133 (MemoRagService.updateDocumentGroupSharing)` |
| F013 | `MemoRagService.updateDocumentGroupSharing` | loop | map の判定結果が真である | `apps/api/src/rag/memorag-service.ts:1134 (MemoRagService.updateDocumentGroupSharing)` |
| F014 | `MemoRagService.updateDocumentGroupSharing` | if | `conflict` が存在し、真である | `apps/api/src/rag/memorag-service.ts:1141 (MemoRagService.updateDocumentGroupSharing)` |
...
| TC014 | F007: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:1117 (MemoRagService.updateDocumentGroupSharing)` |
| TC015 | F008: 条件成立 | `input.name` が `undefined` と異なる 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:1123 (MemoRagService.updateDocumentGroupSharing)` |
| TC016 | F008: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:1123 (MemoRagService.updateDocumentGroupSharing)` |
| TC017 | F009: 条件成立 | `input.description` が `undefined` と異なる 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:1128 (MemoRagService.updateDocumentGroupSharing)` |
| TC018 | F009: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:1128 (MemoRagService.updateDocumentGroupSharing)` |
| TC019 | F010: 条件成立 | `group.parentGroupId` が存在し、真である 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:1129 (MemoRagService.updateDocumentGroupSharing)` |
| TC020 | F010: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:1129 (MemoRagService.updateDocumentGroupSharing)` |
| TC021 | F011: 条件成立 | `input.name` が `undefined` と異なる 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:1131 (MemoRagService.updateDocumentGroupSharing)` |
| TC022 | F011: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:1131 (MemoRagService.updateDocumentGroupSharing)` |
| TC023 | F012: 条件成立 | `pathUpdates.length` が `maxDocumentGroupPathTransactionItems` より大きい 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:1133 (MemoRagService.updateDocumentGroupSharing)` |
| TC024 | F012: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:1133 (MemoRagService.updateDocumentGroupSharing)` |
| TC025 | F013: 0件 | 反復対象が空でも不正な副作用や例外を生じない。 | `apps/api/src/rag/memorag-service.ts:1134 (MemoRagService.updateDocumentGroupSharing)` |
| TC026 | F013: 複数件 | 各要素を順に処理し、順序・終了条件を守る。 | `apps/api/src/rag/memorag-service.ts:1134 (MemoRagService.updateDocumentGroupSharing)` |
| TC027 | F014: 条件成立 | `conflict` が存在し、真である 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:1141 (MemoRagService.updateDocumentGroupSharing)` |
| TC028 | F014: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:1141 (MemoRagService.updateDocumentGroupSharing)` |

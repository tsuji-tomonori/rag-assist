...
| 5 | `MemoRagService.rollbackReindexMigration` | Store | `this` に対して load reindex migration ledger を実行する。 | `this.loadReindexMigrationLedger()` | `apps/api/src/rag/memorag-service.ts:670 (MemoRagService.rollbackReindexMigration)` |
...
| 7 | `MemoRagService.rollbackReindexMigration` | Store | `ledger` に対して find を実行する。 | `ledger.find((candidate) => candidate.migrationId === migrationId)` | `apps/api/src/rag/memorag-service.ts:671 (MemoRagService.rollbackReindexMigration)` |
| 8 | `MemoRagService.rollbackReindexMigration` | Store | `compensationStore` に対して get を実行する。 | `compensationStore.get(actorTenantId, migrationId, "rollback")` | `apps/api/src/rag/memorag-service.ts:676 (MemoRagService.rollbackReindexMigration)` |
...
| 11 | `MemoRagService.rollbackReindexMigration` | Store | `this.deps.objectStore` に対して get text を実行する。 | `this.deps.objectStore.getText(migration.previousManifestObjectKey)` | `apps/api/src/rag/memorag-service.ts:681 (MemoRagService.rollbackReindexMigration)` |
| 12 | `MemoRagService.rollbackReindexMigration` | Service | service の get manifest 処理を呼び出す。 | `this.getManifest(migration.stagedDocumentId, actorTenantId)` | `apps/api/src/rag/memorag-service.ts:685 (MemoRagService.rollbackReindexMigration)` |
...
| 14 | `MemoRagService.rollbackReindexMigration` | Service | service の assert document manifest writable 処理を呼び出す。 | `this.assertDocumentManifestWritable(actor, staged)` | `apps/api/src/rag/memorag-service.ts:686 (MemoRagService.rollbackReindexMigration)` |
...
| 25 | `MemoRagService.rollbackReindexMigration` | Service | service の assert document manifest writable 処理を呼び出す。 | `this.assertDocumentManifestWritable(currentActor, staged)` | `apps/api/src/rag/memorag-service.ts:696 (MemoRagService.rollbackReindexMigration)` |
...
| 27 | `MemoRagService.rollbackReindexMigration` | Service | service の reconcile revoked rollback 処理を呼び出す。 | `this.reconcileRevokedRollback( migration, compensation, compensationStore )` | `apps/api/src/rag/memorag-service.ts:704 (MemoRagService.rollbackReindexMigration)` |
...
| 71 | `MemoRagService.reconcileRevokedRollback` | Store | `store` に対して mark compensated を実行する。 | `store.markCompensated( repair, reindexCompensationResult(rolledBack), new Date().toISOString() )` | `apps/api/src/rag/memorag-service.ts:3378 (MemoRagService.reconcileRevokedRollback)` |
...
| 74 | `MemoRagService.reconcileRevokedRollback` | Store | `store` に対して mark failed を実行する。 | `store.markFailed(repair, error, new Date().toISOString())` | `apps/api/src/rag/memorag-service.ts:3384 (MemoRagService.reconcileRevokedRollback)` |
| 75 | `MemoRagService.reconcileRevokedRollback` | Store | `this` に対して complete reindex compensation ledger を実行する。 | `this.completeReindexCompensationLedger(migration, repair)` | `apps/api/src/rag/memorag-service.ts:3388 (MemoRagService.reconcileRevokedRollback)` |
...
| 79 | `MemoRagService.reconcileRevokedRollback` | Store | `store` に対して mark completed を実行する。 | `store.markCompleted(repair, new Date().toISOString())` | `apps/api/src/rag/memorag-service.ts:3389 (MemoRagService.reconcileRevokedRollback)` |
| 80 | `MemoRagService.rollbackReindexMigration` | Store | `compensationStore` に対して prepare を実行する。 | `compensationStore.prepare({ action: "rollback", tenantId: actorTenantId, migrationId, publicationRunId: migration.publicationRunId, expectedMigrationStatus: "cutover", preparedAt: new Date().toISOString() })` | `apps/api/src/rag/memorag-service.ts:727 (MemoRagService.rollbackReindexMigration)` |
...
| 82 | `MemoRagService.rollbackReindexMigration` | Store | `compensationStore` に対して mark compensated を実行する。 | `compensationStore.markCompensated( compensation, reindexCompensationResult(rolledBack), new Date().toISOString() )` | `apps/api/src/rag/memorag-service.ts:735 (MemoRagService.rollbackReindexMigration)` |
| 83 | `MemoRagService.rollbackReindexMigration` | Store | `this` に対して save reindex migration ledger を実行する。 | `this.saveReindexMigrationLedger([migration])` | `apps/api/src/rag/memorag-service.ts:742 (MemoRagService.rollbackReindexMigration)` |
| 84 | `MemoRagService.rollbackReindexMigration` | Service | service の assert document manifest writable 処理を呼び出す。 | `this.assertDocumentManifestWritable(currentActor, staged)` | `apps/api/src/rag/memorag-service.ts:755 (MemoRagService.rollbackReindexMigration)` |
| 85 | `MemoRagService.rollbackReindexMigration` | Store | `this.deps.objectStore` に対して get text を実行する。 | `this.deps.objectStore.getText(previous.sourceObjectKey)` | `apps/api/src/rag/memorag-service.ts:761 (MemoRagService.rollbackReindexMigration)` |
| 86 | `MemoRagService.rollbackReindexMigration` | Service | service の load structured blocks 処理を呼び出す。 | `this.loadStructuredBlocks(previous)` | `apps/api/src/rag/memorag-service.ts:762 (MemoRagService.rollbackReindexMigration)` |
...
| 88 | `MemoRagService.rollbackReindexMigration` | Store | `this.deps.evidenceVectorStore` に対して delete を実行する。 | `this.deps.evidenceVectorStore.delete(staged.evidenceVectorKeys ?? staged.vectorKeys)` | `apps/api/src/rag/memorag-service.ts:764 (MemoRagService.rollbackReindexMigration)` |
| 89 | `MemoRagService.rollbackReindexMigration` | Store | `this.deps.memoryVectorStore` に対して delete を実行する。 | `this.deps.memoryVectorStore.delete(staged.memoryVectorKeys ?? staged.vectorKeys)` | `apps/api/src/rag/memorag-service.ts:765 (MemoRagService.rollbackReindexMigration)` |
| 90 | `MemoRagService.rollbackReindexMigration` | Service | service の mark manifest lifecycle 処理を呼び出す。 | `this.markManifestLifecycle(staged, "superseded")` | `apps/api/src/rag/memorag-service.ts:766 (MemoRagService.rollbackReindexMigration)` |
| 91 | `MemoRagService.markManifestLifecycle` | Store | `this.deps.objectStore` に対して put text を実行する。 | `this.deps.objectStore.putText(next.manifestObjectKey, JSON.stringify(next, null, 2), "application/json")` | `apps/api/src/rag/memorag-service.ts:3759 (MemoRagService.markManifestLifecycle)` |
| 92 | `MemoRagService.rollbackReindexMigration` | Store | `restoredAdmissionContext` に対して restored admission context を実行する。 | `restoredAdmissionContext(previous, migrationId)` | `apps/api/src/rag/memorag-service.ts:767 (MemoRagService.rollbackReindexMigration)` |
| 93 | `MemoRagService.rollbackReindexMigration` | Service | service の ingest 処理を呼び出す。 | `this.ingest({ fileName: previous.fileName, text, structuredBlocks, sourceExtractorVersion: previous.sourceExtractorVersion, mimeType: previous.mimeType, metadata: { ...(previous.metadata ?? {}), lifecycleStatus: "active…` | `apps/api/src/rag/memorag-service.ts:768 (MemoRagService.rollbackReindexMigration)` |
| 94 | `MemoRagService.createMemoryCards` | External | `this.deps.textModel` へ generate を実行する。 | `this.deps.textModel.generate( buildMemoryCardPrompt(input.fileName, input.text), llmOptions("memoryCard", input.modelId ?? config.defaultMemoryModelId) )` | `apps/api/src/rag/memorag-service.ts:4798 (MemoRagService.createMemoryCards)` |
...
| 118 | `MemoRagService.rollbackReindexMigration` | Store | `this` に対して save reindex migration ledger を実行する。 | `this.saveReindexMigrationLedger([migration])` | `apps/api/src/rag/memorag-service.ts:792 (MemoRagService.rollbackReindexMigration)` |
...
| B005 | `MemoRagService.rollbackReindexMigration` | `migration` が存在しない、または偽である | `apps/api/src/rag/memorag-service.ts:672 (MemoRagService.rollbackReindexMigration)` |
| B006 | `MemoRagService.rollbackReindexMigration` | `migration.publicationRunId` が存在し、真である | `apps/api/src/rag/memorag-service.ts:675 (MemoRagService.rollbackReindexMigration)` |
| B007 | `MemoRagService.rollbackReindexMigration` | `migration.status` が `"cutover"` と異なる、かつ 「`compensation` が存在し、真である、かつ `migration.status` が `"rolled_back"` と等しい」ではない | `apps/api/src/rag/memorag-service.ts:678 (MemoRagService.rollbackReindexMigration)` |
| B008 | `MemoRagService.rollbackReindexMigration` | `migration.publicationRunId` が存在し、真である | `apps/api/src/rag/memorag-service.ts:702 (MemoRagService.rollbackReindexMigration)` |
| B009 | `MemoRagService.rollbackReindexMigration` | `compensation` が存在し、真である | `apps/api/src/rag/memorag-service.ts:703 (MemoRagService.rollbackReindexMigration)` |
| B010 | `MemoRagService.rollbackReindexMigration` | 例外が発生した場合に catch 処理へ移る | `apps/api/src/rag/memorag-service.ts:726 (MemoRagService.rollbackReindexMigration)` |

...
| B004 | `MemoRagService.listAdminAuditLog` | 三項条件 | `intent.status` が `"completed"` と等しい | `intent.status === "completed"` | `apps/api/src/rag/memorag-service.ts:2063 (MemoRagService.listAdminAuditLog)` |
...
| 1 | `MemoRagService.listAdminAuditLog` | list admin audit log の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:2051 (MemoRagService.listAdminAuditLog)` |
...
| 2 | `jsonRecord` | json record の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5851 (jsonRecord)` |
| 2 | `adminAuditActionForOperation` | admin audit action for operation の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5826 (adminAuditActionForOperation)` |
| 2 | `managedUserStatusValue` | managed user status value の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5861 (managedUserStatusValue)` |
| 2 | `stringArrayValue` | string array value の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5857 (stringArrayValue)` |
| 2 | `normalizeLegacyAdminAuditEntry` | normalize legacy admin audit entry の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5835 (normalizeLegacyAdminAuditEntry)` |

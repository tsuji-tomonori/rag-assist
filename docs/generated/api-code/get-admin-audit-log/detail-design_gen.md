...
| Route 定義 | `apps/api/src/routes/admin-routes.ts:246 (GET /admin/audit-log)` |
| Handler | `apps/api/src/routes/admin-routes.ts:257 (GET /admin/audit-log handler)` |

## 2. 主処理

| # | 深さ | 種別 | 自然言語での処理 | コード根拠 | 実装位置 |
| ---: | ---: | --- | --- | --- | --- |
| 1 | 0 | 認証・認可 | 認証済み利用者を request context から取得する。 | `user = c.get("user")` | `apps/api/src/routes/admin-routes.ts:258` |
| 2 | 0 | 認証・認可 | "access:policy:read" permission を必須条件として確認する。 | `requirePermission(user, "access:policy:read")` | `apps/api/src/routes/admin-routes.ts:259` |
| 3 | 0 | 呼び出し | valid query を実行する。 | `query = validQuery<z.infer<typeof AdminAuditLogQuerySchema>>(c)` | `apps/api/src/routes/admin-routes.ts:260` |
| 4 | 0 | 状態 | 例外を監視しながら主要処理を実行する。 | `try` | `apps/api/src/routes/admin-routes.ts:261` |
| 5 | 1 | 応答 | service の list admin audit log 処理を呼び出す。 その結果を HTTP 200 の JSON response として返す。 | `return c.json(await service.listAdminAuditLog(user, query), 200)` | `apps/api/src/routes/admin-routes.ts:262` |
| 6 | 0 | 例外 | 例外を捕捉し、種類と内容に応じて HTTP 応答へ変換する。 | `catch (error) { if (error instanceof InvalidPageCursorError) return c.json({ error: error.message }, 400) throw error }` | `apps/api/src/routes/admin-routes.ts:263` |
| 7 | 1 | 分岐 | 分岐: `error` が `InvalidPageCursorError` の instance である。 | `error instanceof InvalidPageCursorError` | `apps/api/src/routes/admin-routes.ts:264` |
| 8 | 2 | 応答 | 処理結果を HTTP 400 の JSON response として返す。 | `return c.json({ error: error.message }, 400)` | `apps/api/src/routes/admin-routes.ts:264` |
| 9 | 1 | 例外 | error を例外として送出する。 | `throw error` | `apps/api/src/routes/admin-routes.ts:265` |

## 3. 条件分岐・例外

| # | 所属関数 | 種別 | 条件の意味 | 根拠式 | 実装位置 |
| ---: | --- | --- | --- | --- | --- |
| B001 | `GET /admin/audit-log handler` | catch | 例外が発生した場合に catch 処理へ移る | `error` | `apps/api/src/routes/admin-routes.ts:263 (GET /admin/audit-log handler)` |
| B002 | `GET /admin/audit-log handler` | if | `error` が `InvalidPageCursorError` の instance である | `error instanceof InvalidPageCursorError` | `apps/api/src/routes/admin-routes.ts:264 (GET /admin/audit-log handler)` |
...
| 深さ | Symbol | 責務 | 実装位置 |
| ---: | --- | --- | --- |
| 0 | `GET /admin/audit-log handler` | GET /admin/audit-log の request を受け、検証・認可・service 呼び出し・HTTP 応答を調整する。 | `apps/api/src/routes/admin-routes.ts:257 (GET /admin/audit-log handler)` |
...
| 2 | `ObjectStoreSecurityMutationAuditOutbox.listAll` | list all の実装処理を担当する。 | `apps/api/src/security/security-mutation-audit-outbox.ts:360 (ObjectStoreSecurityMutationAuditOutbox.listAll)` |
| 2 | `jsonRecord` | json record の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5904 (jsonRecord)` |
| 2 | `adminAuditActionForOperation` | admin audit action for operation の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5879 (adminAuditActionForOperation)` |
| 2 | `managedUserStatusValue` | managed user status value の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5914 (managedUserStatusValue)` |
| 2 | `stringArrayValue` | string array value の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5910 (stringArrayValue)` |
| 2 | `normalizeLegacyAdminAuditEntry` | normalize legacy admin audit entry の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:5888 (normalizeLegacyAdminAuditEntry)` |

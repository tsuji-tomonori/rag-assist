...
| F004 | `MemoRagService.listAdminAuditLog` | 三項条件 | `intent.status` が `"completed"` と等しい | `apps/api/src/rag/memorag-service.ts:2124 (MemoRagService.listAdminAuditLog)` |
...
| TC007 | F004: 条件成立 | `intent.status` が `"completed"` と等しい 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:2124 (MemoRagService.listAdminAuditLog)` |
| TC008 | F004: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:2124 (MemoRagService.listAdminAuditLog)` |

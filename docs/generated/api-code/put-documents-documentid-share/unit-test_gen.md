...
| F006 | `MemoRagService.updateDocumentShare` | if | can share document の判定結果が真ではない | `apps/api/src/rag/memorag-service.ts:1285 (MemoRagService.updateDocumentShare)` |
...
| TC011 | F006: 条件成立 | can share document の判定結果が真ではない 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:1285 (MemoRagService.updateDocumentShare)` |
| TC012 | F006: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:1285 (MemoRagService.updateDocumentShare)` |

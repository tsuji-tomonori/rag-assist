...
sequenceDiagram
  ...
  Note over API: 分岐 benchmarkFilterSuiteId が存在し、真である
...
| ---: | --- | --- | --- | --- | --- |
| 1 | `POST /benchmark/search handler` | Auth | 認証済み利用者を request context から取得する。 | `c.get("user")` | `apps/api/src/routes/benchmark-routes.ts:75 (POST /benchmark/search handler)` |
| 2 | `POST /benchmark/search handler` | Auth | "benchmark:query" permission を必須条件として確認する。 | `requirePermission(user, "benchmark:query")` | `apps/api/src/routes/benchmark-routes.ts:76 (POST /benchmark/search handler)` |
| 3 | `POST /benchmark/search handler` | Validation | schema 検証済みの JSON request body を取得する。 | `validJson<z.infer<typeof BenchmarkSearchRequestSchema>>(c)` | `apps/api/src/routes/benchmark-routes.ts:77 (POST /benchmark/search handler)` |
| 4 | `POST /benchmark/search handler` | Service | service の search 処理を呼び出す。 | `service.search(searchInput, benchmarkSearchUser(user, requestUser))` | `apps/api/src/routes/benchmark-routes.ts:88 (POST /benchmark/search handler)` |
...
| 32 | `POST /benchmark/search handler` | HTTP/SSE | HTTP 200 で JSON response を返す。 | `c.json(await service.search(searchInput, benchmarkSearchUser(user, requestUser)), 200)` | `apps/api/src/routes/benchmark-routes.ts:88 (POST /benchmark/search handler)` |

## 分岐

| ID | Function | 条件 | 実装位置 |
| --- | --- | --- | --- |
| B001 | `POST /benchmark/search handler` | `benchmarkFilterSuiteId` が存在し、真である | `apps/api/src/routes/benchmark-routes.ts:80 (POST /benchmark/search handler)` |
| B002 | `requirePermission` | 利用者が 指定された permission を持たない | `apps/api/src/authorization.ts:267 (requirePermission)` |
| B003 | `benchmarkSearchUser` | `requestUser` が存在しない、または偽である | `apps/api/src/routes/benchmark-routes.ts:21 (benchmarkSearchUser)` |

...
# POST /benchmark/search Unit Test仕様
...
| F004 | `MemoRagService.search` | 三項条件 | `input.conversationId` が存在し、真である | `apps/api/src/rag/memorag-service.ts:3223 (MemoRagService.search)` |
...
| TC007 | F004: 条件成立 | `input.conversationId` が存在し、真である 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:3223 (MemoRagService.search)` |
| TC008 | F004: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:3223 (MemoRagService.search)` |

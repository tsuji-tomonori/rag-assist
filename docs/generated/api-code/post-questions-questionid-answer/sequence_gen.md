...
| 12 | `MemoRagService.answerQuestion` | Store | `this.deps.questionStore` に対して answer を実行する。 | `this.deps.questionStore.answer(questionId, { ...input, responderName: input.responderName?.trim() \|\| userDisplayName(user) })` | `apps/api/src/rag/memorag-service.ts:2886 (MemoRagService.answerQuestion)` |

...
| 4 | `MemoRagService.listUsageSummaries` | Service | service の usage rollout mode 処理を呼び出す。 | `this.usageRolloutMode()` | `apps/api/src/rag/memorag-service.ts:2276 (MemoRagService.listUsageSummaries)` |
| 5 | `MemoRagService.listUsageSummaries` | Store | `this.deps.usageEventStore` に対して query を実行する。 | `this.deps.usageEventStore.query(tenantId, normalized)` | `apps/api/src/rag/memorag-service.ts:2278 (MemoRagService.listUsageSummaries)` |
...
| B004 | `MemoRagService.listUsageSummaries` | `rolloutMode` が `"active"` と等しい、かつ `this.deps.usageEventStore` が存在し、真である | `apps/api/src/rag/memorag-service.ts:2277 (MemoRagService.listUsageSummaries)` |

...
| 1 | `MemoRagService.listReindexMigrations` | list reindex migrations の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:908 (MemoRagService.listReindexMigrations)` |
...
| --- | --- | --- | --- | --- | --- | --- |
| 参照 | Store | `this` | `loadReindexMigrationLedger` | `this` に対して load reindex migration ledger を実行する。 | `MemoRagService.listReindexMigrations` | `apps/api/src/rag/memorag-service.ts:909 (MemoRagService.listReindexMigrations)` |
...
| 実行 | Store | `(await this.loadReindexMigrationLedger())` | `sort` | `(await this.loadReindexMigrationLedger())` に対して sort を実行する。 | `MemoRagService.listReindexMigrations` | `apps/api/src/rag/memorag-service.ts:909 (MemoRagService.listReindexMigrations)` |

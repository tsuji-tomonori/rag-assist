...
- 実装 route: `apps/api/src/routes/admin-routes.ts:442 (POST /admin/aliases/{aliasId}/review)`
...
| --- | --- |
| 認可モード | `required` |
| 必須 permission | `rag:alias:review:group` |
| 条件付き permission | - |
...
| `403` | 必要 permission (rag:alias:review:group) または条件付き permission を満たさない場合。 | `{"error":"Forbidden"}` |

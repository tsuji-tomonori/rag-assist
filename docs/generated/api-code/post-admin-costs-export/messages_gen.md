...
| M008 | HTTP 実装応答 | `503` | Export storage is not configured | `err` が `Error` の instance である、かつ `err.message` が "DEBUG_DOWNLOAD_BUCKET_NAME" を含む | `apps/api/src/routes/admin-routes.ts:547 (POST /admin/costs/export handler)` |

...
| Route 定義 | `apps/api/src/routes/admin-routes.ts:369 (POST /admin/aliases)` |
| Handler | `apps/api/src/routes/admin-routes.ts:384 (POST /admin/aliases handler)` |

## 2. 主処理

| # | 深さ | 種別 | 自然言語での処理 | コード根拠 | 実装位置 |
| ---: | ---: | --- | --- | --- | --- |
| 1 | 0 | 認証・認可 | 認証済み利用者を request context から取得する。 | `user = c.get("user")` | `apps/api/src/routes/admin-routes.ts:385` |
| 2 | 0 | 認証・認可 | "rag:alias:write:group" permission を必須条件として確認する。 | `requirePermission(user, "rag:alias:write:group")` | `apps/api/src/routes/admin-routes.ts:386` |
| 3 | 0 | 入力検証 | schema 検証済みの JSON request body を取得する。 | `body = validJson<z.infer<typeof CreateAliasRequestSchema>>(c)` | `apps/api/src/routes/admin-routes.ts:387` |
| 4 | 0 | 応答 | service の create alias 処理を呼び出す。 その結果を HTTP 200 の JSON response として返す。 | `return c.json(await service.createAlias(user, body), 200)` | `apps/api/src/routes/admin-routes.ts:388` |
...
| B002 | `MemoRagService.createAlias` | if | `result` が存在しない、または偽である | `!result` | `apps/api/src/rag/memorag-service.ts:1305 (MemoRagService.createAlias)` |
...
| 深さ | Symbol | 責務 | 実装位置 |
| ---: | --- | --- | --- |
| 0 | `POST /admin/aliases handler` | POST /admin/aliases の request を受け、検証・認可・service 呼び出し・HTTP 応答を調整する。 | `apps/api/src/routes/admin-routes.ts:384 (POST /admin/aliases handler)` |
...
| 1 | `MemoRagService.createAlias` | create alias の実装処理を担当する。 | `apps/api/src/rag/memorag-service.ts:1277 (MemoRagService.createAlias)` |
...
| --- | --- | --- | --- | --- | --- | --- |
| 実行 | Store | `ledger.aliases` | `push` | `ledger.aliases` に対して push を実行する。 | `MemoRagService.createAlias` | `apps/api/src/rag/memorag-service.ts:1292 (MemoRagService.createAlias)` |
...
| 実行 | Store | `this` | `mutateAliasLedger` | `this` に対して mutate alias ledger を実行する。 | `MemoRagService.createAlias` | `apps/api/src/rag/memorag-service.ts:1279 (MemoRagService.createAlias)` |

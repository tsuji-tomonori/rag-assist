...
| F002 | `MemoRagService.createAlias` | if | `result` が存在しない、または偽である | `apps/api/src/rag/memorag-service.ts:1305 (MemoRagService.createAlias)` |

## 3. コード由来テストケース

| Case | シナリオ | 期待観点 | 根拠 |
| --- | --- | --- | --- |
| TC001 | 正常系 | 検索 alias draft を作成する が成功 response を返す。 | `apps/api/src/routes/admin-routes.ts:384 (POST /admin/aliases handler)` |
...
| TC004 | F002: 条件成立 | `result` が存在しない、または偽である 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:1305 (MemoRagService.createAlias)` |
| TC005 | F002: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:1305 (MemoRagService.createAlias)` |

...
| F002 | `MemoRagService.getCostAuditSummary` | 三項条件 | `totalBenchmarkCases` が `0` より大きい | `apps/api/src/rag/memorag-service.ts:958 (MemoRagService.getCostAuditSummary)` |

## 3. コード由来テストケース

| Case | シナリオ | 期待観点 | 根拠 |
| --- | --- | --- | --- |
| TC001 | 正常系 | 概算コストを取得する が成功 response を返す。 | `apps/api/src/routes/admin-routes.ts:425 (GET /admin/costs handler)` |
...
| TC004 | F002: 条件成立 | `totalBenchmarkCases` が `0` より大きい 場合の response / side effect が実装どおりである。 | `apps/api/src/rag/memorag-service.ts:958 (MemoRagService.getCostAuditSummary)` |
| TC005 | F002: 条件不成立 | 反対側または後続処理へ進み、成立側の副作用を行わない。 | `apps/api/src/rag/memorag-service.ts:958 (MemoRagService.getCostAuditSummary)` |

...
| assignee | AC-SQ016-001 | pass | blocked | blocked | 4 / 2 / 1 column remediation後、required E2E-UI-ZOOM-REFLOW-001の640/320 CSS px到達・root overflow 0に加え、Chromium／Firefox／WebKit required E2E-UI-LAYOUT-STRESS-001で320pxの問い合わせ32件／4 lane、長い先頭／末尾title・質問・部署名、DOM末尾カードのviewport到達とregion overflow 0を検証。実browser zoomは未検証 |
...
| assignee | AC-SQ016-006 | pass | blocked | blocked | 4 viewportのcomputed auditとChromium／Firefox／WebKit required E2E-UI-LAYOUT-STRESS-001でreduced motion下の問い合わせ32件／4 lane・末尾viewport到達を検証。orientation / safe-area / virtual-keyboard実機は未検証 |
| assignee | AC-SQ016-007 | pass | blocked | blocked | required E2E-UI-STATE-001に加え、Chromium／Firefox／WebKit required E2E-UI-CROSS-BROWSER-STATE-003でloading→500→retry→confirmed emptyとHTTP 403を区別し、false zero、未確認kanban、private detail露出を防ぐ。E2E-UI-LAYOUT-STRESS-001で4 statusに分散した問い合わせ32件、長いcontent、DOM末尾カードのviewport到達も検証。manual evidenceは未実施 |
| assignee | AC-SQ016-008 | not_applicable | blocked | blocked | manual required scopeは未実施 |
| history | AC-SQ016-001 | pass | blocked | blocked | 4 viewport auditのroot/unresolved overflow 0に加え、Chromium／Firefox／WebKit required E2E-UI-LAYOUT-STRESS-001で320pxの履歴35件・長い先頭／末尾title・末尾viewport到達とregion overflow 0を検証。実browser zoomは未検証 |
...
| history | AC-SQ016-006 | pass | blocked | blocked | 4 viewportのcomputed auditとChromium／Firefox／WebKit required E2E-UI-LAYOUT-STRESS-001でreduced motion下の履歴35件末尾viewport到達を検証。orientation / safe-area / virtual-keyboard実機は未検証 |
| history | AC-SQ016-007 | pass | blocked | blocked | required E2E-UI-STATE-001に加え、Chromium／Firefox／WebKit required E2E-UI-CROSS-BROWSER-STATE-001でloading→500→retry→confirmed empty／403、false zero、private detail非表示を検証し、E2E-UI-LAYOUT-STRESS-001で履歴35件の長い先頭／末尾title・末尾viewport到達も検証。manual evidenceは未実施 |
| history | AC-SQ016-008 | not_applicable | blocked | blocked | manual required scopeは未実施 |
| favorites | AC-SQ016-001 | pass | blocked | blocked | 4 viewport auditのroot/unresolved overflow 0に加え、Chromium／Firefox／WebKit required E2E-UI-LAYOUT-STRESS-001で320pxのお気に入り24件／8 target type、長いlabel・target ID、アクセス不可cue、末尾到達、再取得後の確認済み0件とregion overflow 0を検証。実browser zoomは未検証 |
...
| favorites | AC-SQ016-007 | pass | blocked | blocked | required E2E-UI-STATE-001に加え、Chromium／Firefox／WebKit required E2E-UI-CROSS-BROWSER-STATE-004でloading→500→retry→confirmed emptyとHTTP 403を区別し、false zero、empty、private detail露出を防ぐ。E2E-UI-LAYOUT-STRESS-001でpopulated 24件から再取得後の確認済み0件への遷移も検証。manual evidenceは未実施 |
...
| benchmark | AC-SQ016-007 | pass | blocked | blocked | Chromium required E2E-UI-STATE-001とFirefox／WebKit required E2E-UI-CROSS-BROWSER-STATE-007でruns/suitesのloading、実行履歴500によるpartial、retry→confirmed empty、全resource HTTP 403を区別し、取得済みsuiteを保持してfalse zero、未確認history、private detail露出を防ぐ。manual evidenceは未実施 |
| benchmark | AC-SQ016-008 | not_applicable | blocked | blocked | manual required scopeは未実施 |
| admin | AC-SQ016-001 | pass | blocked | blocked | required E2E-UI-ZOOM-REFLOW-001をChromium／Firefox／WebKitの640/320 CSS pxで実行し、到達・root overflow 0を検証。required E2E-UI-LAYOUT-STRESS-001で320pxの管理監査36件、長い対象・実行者・理由・policy・監査ID、DOM末尾項目のviewport到達とregion overflow 0を検証。CSS viewport proxyであり実browser zoomは未検証 |
...
| admin | AC-SQ016-006 | pass | blocked | blocked | 4 viewportのcomputed auditとChromium／Firefox／WebKit required E2E-UI-LAYOUT-STRESS-001でreduced motion下の管理監査36件末尾viewport到達を検証。orientation / safe-area / virtual-keyboard実機は未検証 |
| admin | AC-SQ016-007 | pass | blocked | blocked | required E2E-UI-STATE-001に加え、Chromium／Firefox／WebKit required E2E-UI-CROSS-BROWSER-STATE-006でloading→監査取得500によるpartial→retry→recovery、ユーザー更新失敗時のsource・as-of付きstale data保持、権限不足deep linkのpermissionとprotected request抑止を検証し、成功part／確認済みdataを保持してfalse zeroとprivate detail露出を防ぐ。E2E-UI-LAYOUT-STRESS-001で管理監査36件の全件表示、長いcontent、DOM末尾項目のviewport到達も検証。manual evidenceは未実施 |
| admin | AC-SQ016-008 | not_applicable | blocked | blocked | manual required scopeは未実施 |
| documents | AC-SQ016-001 | pass | blocked | blocked | required E2E-UI-ZOOM-REFLOW-001のChromium／Firefox／WebKit 640/320 CSS px到達・root overflow 0に加え、required E2E-UI-LAYOUT-STRESS-001で320pxの文書30件／6 file type、50件表示時の全件表示、長い先頭／末尾ファイル名、末尾viewport到達とregion overflow 0を検証。desktop row 6件はfocus可能なtable ownerとkeyboard代替操作付きsupported_scroll。実browser zoomは未検証 |
...
| documents | AC-SQ016-006 | pass | blocked | blocked | 4 viewportのcomputed auditとChromium／Firefox／WebKit required E2E-UI-LAYOUT-STRESS-001でreduced motion下の文書30件末尾viewport到達を検証。orientation / safe-area / virtual-keyboard実機は未検証 |
| documents | AC-SQ016-007 | pass | blocked | blocked | required E2E-UI-STATE-001に加え、Chromium／Firefox／WebKit required E2E-UI-CROSS-BROWSER-STATE-002でcatalog／folder／reindexのloading、文書取得500によるpartial、retry→confirmed empty、全resource 403を区別し、false zero／emptyとprivate detail露出を防ぐ。E2E-UI-LAYOUT-STRESS-001で文書30件の全件表示、長い先頭／末尾ファイル名と末尾viewport到達も検証。manual evidenceは未実施 |

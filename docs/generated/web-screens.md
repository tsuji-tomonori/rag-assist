...
| ドキュメント | documents | / (client-state) | [ドキュメント](web-features/documents.md) | DocumentWorkspace | canManageDocuments | 管理者設定へ戻る、文書詳細を閉じる、この資料に質問する、再インデックス、削除、共有フォルダ / 選択してください / 共有 Cognito group / 既存 shared group 候補 / 追加: / 削除: / 変更なし: /… ほか 72 件 | confirmed |
...
- 主要操作: チャットへ戻る、更新、管理対象ユーザー作成、メール、表示名、任意、初期ロール、role、作成、付与 ほか 15 件
...
- 主要操作: 管理者設定へ戻る、文書詳細を閉じる、この資料に質問する、再インデックス、削除、共有フォルダ / 選択してください / 共有 Cognito group / 既存 shared group 候補 / 追加: / 削除: / 変更なし: /…、共有フォルダ / 選択してください、shareGroupId || selectedGroupId、選択してください、共有 Cognito group ほか 68 件

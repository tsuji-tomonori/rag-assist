...
browser runtime設定とservice/infrastructure consumerを区別する。productionではCloudFormation `ApiUrl` / `OpenApiUrl` outputsをCloudFront `/api/`へ固定する。Lambdaのbenchmark targetはCloudFront Distributionへの逆依存を避けるためREST専用custom domainを内部利用し、SPA `config.json`へcustom domainを混入させない。non-productionだけはrollbackとlocal integrationのためinternal targetがdefault `execute-api` endpointを利用できる。
...
| `/api/*` | REST専用API Gateway Regional custom domain | `/api` prefixを削除してroot mappingへ転送する。キャッシュは無効化する。`Authorization` を転送する。 |
| `/ws/v1` | WebSocket専用API Gateway Regional custom domain | exact pathだけをWebSocket originへ振り分け、root mapping `/` へrewriteする。viewerの `Host` はoriginへ渡さない。upgrade/subprotocol headerだけを転送し、cookie/query stringは転送しない。キャッシュは無効化する。 |
...
WebSocket API behaviorもAPI Gateway origin向けであるため、viewerの `Host` はoriginへ渡さず、origin requestではAPI Gateway origin domainの `Host` を使う。WebSocket handshakeに必要な `Sec-WebSocket-Key` と `Sec-WebSocket-Version`、credential transportとprotocol negotiationに使う `Sec-WebSocket-Protocol`、extension negotiationの `Sec-WebSocket-Extensions` だけをoriginへ転送する。`Sec-WebSocket-Accept` はoriginからviewerへ返るhandshake responseとして通す。CloudFrontはWebSocketをHTTP/1.1でoriginへ中継する。

viewer-request CloudFront Functionが `/ws/v1` をWebSocket custom domainのroot mapping `/` へrewriteする。CloudFront FunctionのURI変更は選択済みbehavior/originを変えないため、`ws/v1` exact behaviorでWebSocket originを選んだ後にrewriteする。`/ws/v1/*` や任意pathをWebSocket originへ広げない。
...
WebSocketは長いJWT、短命ticket、authorization codeをquery/path/cookieに持たせない。CloudFront standard access logはorigin forwarding設定にかかわらず完全なquery stringを記録するため、ticket queryは採用しない。認証済みREST APIで短命ticketを発行し、browser WebSocket APIが `Sec-WebSocket-Protocol` に固定protocol `memorag.v1` とopaque ticket protocolを提示する。API Gateway WebSocket APIの `$connect` Lambda REQUEST authorizerはheaderをidentity sourceとしてticketをatomicに一度だけ消費し、`$connect` integrationは選択済みprotocol `memorag.v1` だけをresponse headerで返す。

ticketのraw valueは256-bit entropyを持ち、DynamoDBにはSHA-256 hashだけを保存する。ticket recordは60秒TTL、single-use state、`userId`、`tenantId`、Cognito `origin_jti` session、`jti` token、token issued/expiryへbindingする。authorizerはticket消費後にauthoritative identity、active status、tenant、application revocation registry、administrative-principal fence、session invalidation epochを再確認する。missing、malformed、expired、replayed、revoked、binding mismatch、identity/store障害はgeneric denyへfail closedにする。

Lambdaはraw event、header、ticket、JWTをログへ出さず、request/connection/ticket correlation ID、route、status、allowlisted reason codeだけを記録する。API Gateway WebSocket execution log/data traceは無効にし、access logにもquery/header/authorizer contextを含めない。CloudFront standard logがqueryを記録する特性は変えられないため、client contractとorigin request policyの両方でcredential queryを使用しない。

Lambda authorizerは `$connect` でのみ実行され、既接続socketへ後続revocationを遡及適用しない。段階5は業務message/server pushを実装せず、将来の送信経路は送信直前にconnection/sessionをauthoritativeに再認可するまで導入しない。disconnect後のreconnectは新しいticketを必要とする。

productionではRESTとWebSocketのdefault `execute-api` endpointを無効化する。CloudFrontもdefault endpointへ依存させず、同じregionのACM certificateを使う別々のRegional custom domain、root API mapping、Route 53 aliasへ同時に切り替える。WebSocket custom domainはREST/HTTP APIと共有できないAWS制約があるため、2 domainを必須とする。

CDK context `restApiOriginDomainName`、`webSocketApiOriginDomainName`、`apiGatewayOriginCertificateArn`、`apiGatewayOriginHostedZoneId`、`apiGatewayOriginHostedZoneName` をproductionの明示入力とする。domainはlowercase FQDN、互いに異なるhosted zone配下のsubdomain、certificateはstackと同じregion、hosted zone IDはRoute 53形式でなければsynth前に拒否する。disable flag、custom domain/mapping、DNS alias、CloudFront origin切替は同じstack changeとして扱い、一部だけを先行しない。

default endpoint無効化はcustom domain自体をprivate originにする機能ではない。browserと外部運用outputをCloudFrontへ固定し、default `execute-api` bypassを閉じる範囲である。custom domainの追加origin cloakingが必要な場合はWAF/custom header等を別の脅威分析と実AWS検証で扱い、段階6の自動testだけで成立済みとしない。
...
5. 短命・単回WebSocket ticketとCloudFront exact `/ws/v1` behaviorを実装する。
6. default `execute-api` endpointを無効化し、REST/WS custom domain mappingとCloudFront originを同時切替する。CORS wildcard 0、SPA direct origin 0、認証・認可・ログ相関を最終検証する。
...
- WebSocket upgrade header、subprotocol応答、idle timeout、disconnect/reconnectの実環境検証が必要になる。
- productionのdefault `execute-api` endpointは閉じられるが、Regional custom domainはCloudFront専用のprivate originではない。追加のorigin cloakingには別の防御と実環境検証が必要になる。
...
- TTLは60秒。
- 利用回数は1回。
- raw ticketは256-bit entropyを持ち、DynamoDBにはSHA-256 hashだけを保存する。
- `userId`、`tenantId`、Cognito session/token ID、token issued/expiry、ticket issued/expiryと紐付ける。
- DynamoDB conditional updateでissuedからconsumedへatomicに遷移し、同時利用・replayを拒否する。
- 発行後のidentity suspension、application deny/fence、session invalidationを `$connect` で再確認する。
- 失敗時は権限外情報を返さず401または403を返す。
- disconnect後のreconnectは新規ticketを要求する。
...
- AWS Documentation: Query string parameters and CloudFront standard logs, `https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/QueryStringParameters.html`
- AWS Documentation: Lambda authorizers for WebSocket APIs, `https://docs.aws.amazon.com/apigateway/latest/developerguide/apigateway-websocket-api-lambda-auth.html`
- AWS Documentation: Set up a `$connect` route that requires a WebSocket subprotocol, `https://docs.aws.amazon.com/apigateway/latest/developerguide/websocket-connect-route-subprotocol.html`
- AWS Documentation: WebSocket API stages, `https://docs.aws.amazon.com/apigateway/latest/developerguide/websocket-api-stages.html`
...
- CloudFrontに `default (*)`、`api` / `api/*`、exact `ws/v1` behaviorを維持する。
- `/api` prefix削除とexact `/ws/v1` → `/prod` stage rewriteをCloudFront Functionで維持する。
...
- WebSocketの業務message/server pushを実装する前に、送信直前のconnection/session再認可と既接続revocation境界を別設計する。

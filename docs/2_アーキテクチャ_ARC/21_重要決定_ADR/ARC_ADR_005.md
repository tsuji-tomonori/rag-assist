...
| `/ws/v1` | API Gateway WebSocket API | exact pathだけをWebSocket originへ振り分け、API Gateway stage path `/prod` へrewriteする。viewerの `Host` はoriginへ渡さない。upgrade/subprotocol headerだけを転送し、cookie/query stringは転送しない。キャッシュは無効化する。 |
...
viewer-request CloudFront Functionが `/ws/v1` をAPI Gateway WebSocket invoke URLのstage path `/prod` へrewriteする。CloudFront FunctionのURI変更は選択済みbehavior/originを変えないため、`ws/v1` exact behaviorでWebSocket originを選んだ後にrewriteする。`/ws/v1/*` や任意pathをWebSocket originへ広げない。

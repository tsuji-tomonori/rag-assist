...
dev 初期 policy は `config/rag-quality/dev-policy.json` の `memorag-dev-rag-quality@2026-07-16.draft-1` を正とする。2026-07-16 に `tsuji-tomonori` が、同ファイルの slice、threshold、sample/confidence、non-regression、response action を承認した。version 文字列は承認対象との同一性を保つため `draft-1` を含むが、承認欄が空の draft ではない。runtime version は product の `ragRuntimePolicy.profile.version` と一致させる。workload/price/dataset/workload dimension の version は dev 初期 evidence 集合の一致条件であり、未取得 evidence の値を表さない。
...
| sanitized debug trace | chat orchestration | evidence retention、citation locator、chat final latency、model first-token diagnostic、成功/失敗 | `quality-control/source-samples/` |
...
model first-token は `performance.chat_first_token_p50_ms` / `p95` / `p99` の diagnostic measurement で確認する。これは orchestration ingress から最終回答生成 Bedrock stream の最初の非空 content delta までで、`clientVisible=false` の model-side 境界である。summary/run metrics の `firstTokenSampleCount` が case artifact の valid `measured` evidence 数と一致することを確認する。値が unavailable の場合は trace/case の status と reason を確認し、`performance.chat_*` の final response latency、worker latency、wall-clock 差分で補完しない。現行 API の client-visible TTFT と承認済み first-token SLO threshold は未取得のため、promotion pass として扱わない。
...
`Deploy MemoRAG MVP` は AWS credential 設定後、既存 `MemoRagMvpStack` の CloudFormation output `DocumentsBucketName` から docs bucket を解決する。bucket 名、account、region は repository や GitHub variable に固定しない。`quality-control/observations/` の個別 JSON から、承認済み policy の profile/version、workload/runtime/price version、10 provenance dimension が完全一致する最新の signal/slice 一件ずつを選び、JSON array bundle を作る。

workflow は解決済み policy を `quality-control/policies/active.json` と `quality-control/promotion-candidates/<policy-version>/<commit>/<run>/policy.json` に、bundle と preparation result を同じ candidate prefix に upload する。その場で `RAG_QUALITY_POLICY_S3_URI` と `RAG_QUALITY_OBSERVATIONS_S3_URI` を `GITHUB_ENV` へ設定するため、GitHub Environment variables や手動 URI input は使用しない。CDK bootstrap/synth/deploy には同じ policy の runtime/workload/price/index/prompt/pipeline/parser/chunker version context を渡す。

必須 signal/slice が不足する初回は空値、0、推定値を作らず、policy を active 化して deploy を保留する。preparation result と bundle は workflow artifact と S3 に残す。全組が揃った場合だけ `npm run rag:promotion:check` を build/synth/deploy より前に実行する。policy 未承認、source provenance 不足、release audit 不在、unavailable、sample/confidence/baseline 不足、または threshold/non-regression/zero-tolerance 違反では workflow を失敗させ、deploy を開始しない。

dev 初期 policy の version context を runtime へ初めて反映しないと同 version の observation を生成できない場合に限り、`workflow_dispatch` の `policy-bootstrap=true` を使える。authorization は dispatch 対象の main commit と policy identity を連結した `<main SHA>:<profileId>@<version>` に完全一致させ、environment は `dev` に固定する。workflow は policy の承認状態と model 一致を通常どおり検査し、policy/bundle/preparation を自動 upload するが、不足 observation や合格値は生成しない。この経路は初期 version 反映であり promotion pass ではない。deploy 成功後は `quality-control/bootstrap-authorizations/<profileId>/<version>.json` に完了 marker を保存し、同じ policy の bootstrap 再実行を拒否する。deploy 失敗時は marker を作らず、原因修正後の再試行を許可する。通常の main push、authorization 不一致、dev 以外では bootstrap を許可せず、完全な evidence が無ければ build/synth/deploy を保留する。
...
alert は `quality-control/alerts/`、実行 action は `quality-control/actions/`、現在の interlock 状態は `quality-control/runtime/safety-state.json` に保存する。allowed action は少なくとも promotion freeze、candidate quarantine、limited/refuse answer を含み、last-known-safe runtime を設定する場合は rollback も含める。リストが空、重複、または必須 action 欠損なら policy invalid とし、code-owned の availability-reducing action を用いて安全 action 全体の無効化を防ぐ。promotion freeze、candidate/document quarantine、last-known-safe rollback、limited answer、refuse answer の範囲を超えない。

`RAG_MONITORING_REQUIRED=1` では、安全状態が無い・壊れている・期限切れ、または要求 runtime と active monitored runtime が一致しない場合、ingest を含む全 RAG operation を拒否する。active runtime が candidate quarantine に含まれる場合、chat、search、publication、promotion は拒否する。一方、ingest は `documentQuarantineRequired=true` を強制して非公開 staging artifact としてのみ継続し、通常 RAG へ vector / memory card を公開しない。この限定経路は ingest manifest / worker outcome の observation を追加して原因調査と収束確認を可能にするためのものであり、quarantine、promotion freeze、limited/refuse state を解除しない。
...
5. active runtime が quarantine 済みで ingest が必要な場合は、ingest 後の manifest が `admission.status=quarantined`、`lifecycleStatus=staging`、`publicationEligible=false` であることと、対応する source sample が追加されたことを確認する。通常 RAG での検索・回答や publication を回復確認に使わない。
6. `RagQualityMonitorLambdaErrorAlarm` では active policy の存在・JSON contract・docs bucket access を先に確認する。
...
- AC-OPS-MON-008: 通常 deploy が承認済み policy と完全な observations の promotion pass より前に build、synth、deploy を開始せず、初回 bootstrap は dev の手動 dispatch と main SHA/policy identity の完全一致に限定されること。
...
- AC-OPS-MON-014: active runtime の quarantine 中は、chat、search、publication、promotion を拒否したまま、ingest を強制文書隔離で継続し、非公開 manifest と observation を確認できること。

...
- `docs/`: 要件、設計、運用、生成 docs、仕様復元成果物
- `agents/`, `skills/`: Codex/PM 用 agent 設定と repository-local skill
- `scripts/validate_spec_recovery.py`: 仕様復元成果物の軽量検証 script
...
Hono + `@hono/zod-openapi` で既存 REST API の OpenAPI を生成します。`packages/contract` は Zod schema、oRPC contract、共有型の source of truth として追加しており、初回移行では `/rpc/*` に chat / benchmark の oRPC endpoint を併設しています。Web と benchmark runner は `@memorag-mvp/contract` 由来の `ApiClient` 型で oRPC client を作成し、既存 REST endpoint は移行期間の互換性のため残しています。移行期間中も REST route schema と oRPC contract の drift は `npm run docs:openapi:check`、API contract test、PR self-review で確認し、同じ API 概念を変更するときは両方の schema 更新要否を確認します。

- `GET /health`
- `POST /rpc/*` oRPC client 用。初回対象は `system.health`、`chat.create`、`chat.startRun`、`benchmark.query`、`benchmark.search` です。
- `GET /openapi.json`
- `GET /documents`
- `POST /documents` 小さなテキスト互換用の同期登録（ファイルアップロード用途では非推奨）
...
ファイルアップロード UI は S3 upload session へ転送したあと `POST /document-ingest-runs` で非同期 ingest run を開始し、run 状態を取得して完了 summary を受け取ります。既存 `POST /documents` と `POST /documents/uploads/{uploadId}/ingest` は後方互換の同期 API として残しますが、大容量ファイル用途では非推奨です。外部 API は full manifest、chunk metadata、vector key を返さず、ファイル実体が必要な場合は S3 署名付き URL、内部処理では object key を使います。
...
建築・AEC 図面理解向け QARAG 評価の性能テスト用正本は [`benchmark/architecture-drawing-qarag-v0.1.json`](benchmark/architecture-drawing-qarag-v0.1.json) で管理します。説明・レビュー用の補助文書は [`benchmark/architecture-drawing-qarag-v0.1.md`](benchmark/architecture-drawing-qarag-v0.1.md) です。管理画面の suite 一覧には `architecture-drawing-qarag-v0.1` として表示され、CodeBuild runner の pre_build で `npm run prepare:architecture-drawing-qarag -w @memorag-mvp/benchmark` を実行し、JSON から JSONL dataset を生成して国土交通省・自治体公開 PDF を一時 corpus に download してから `/benchmark/query` を実行します。prepare は source ごとに `<file>.metadata.json` も生成し、`drawingSourceType`、`drawingSheetMetadata`、`drawingRegionIndex`、`drawingReferenceGraph`、`drawingExtractionArtifacts` を保持します。benchmark seed upload では S3 Vectors の filterable metadata 2048 bytes 制限を避けるため、document ingest metadata へ渡す値を `drawingSourceType` と `searchAliases` などの小さな検索 filter 用 metadata に compact 化します。単一 sheet と判定できる corpus では `pageOrSheet`、`drawingNo`、`sheetTitle`、`scale` も compact scalar metadata として渡し、複数 sheet の corpus は誤 page hit を避けるため単一値に潰しません。`drawingSheetMetadata`、`drawingRegionIndex`、`drawingReferenceGraph`、`drawingExtractionArtifacts` は vector metadata へ複製しません。`drawingRegionIndex` の bbox は seed QA の evidence anchor から作る初期 region candidate であり、実測 OCR crop ではありません。`drawingExtractionArtifacts` は region 単位の `pdf_text -> ocr -> vlm_ocr` fallback 結果を表し、`sourceMethod`、`bbox`、`confidence`、`parserVersion`、`rawText`、`normalizedValues`、`failureReason` を保持します。VLM-OCR は page 全体ではなく crop region の最後の fallback とし、利用できない場合は架空値で埋めず failure reason を残します。`drawingReferenceGraph` は page / region / detail / section / callout node と contains / references edge、`detailIndex`、`calloutEdges`、source hierarchy conflict を保持し、詳細図・断面参照の source / target bbox を診断に渡します。縮尺・寸法・口径・延長・範囲条件の QA には `expectedNormalizedValues` を生成でき、runner は `normalized_answer_accuracy` で表記ゆれを吸収した数値一致を評価します。図面診断用には `expectedRegionIds`、`expectedExtractionValues`、`expectedCounts`、`expectedGraphResolutions`、`evidenceSufficiency` を任意指定でき、未指定 metric は `not_applicable` として扱います。`evidenceSufficiency` は `requireBbox`、`expectedSourceTypes`、`sourcePriority` を使い、根拠 bbox の有無、案件図面と標準図の source hierarchy、正規化値一致を通常回答の gate として評価します。graph evidence がない detail / section QA は推測回答に進めず、`graph_resolution_accuracy` と abstention 系 metric で分離して見る前提です。`ARCHITECTURE_QARAG_VISUAL_PAGE_RETRIEVAL=1` を指定すると、default path には入れない評価候補として `visual-page-index.json` と `visual-page-retrieval-report.md` を生成し、benchmark summary の `page_recall_at_k`、`answerable_accuracy`、`unsupported_answer_rate`、`p95_latency_ms`、`no_access_leak_count` で baseline と比較できます。公開元 URL の変更、外部ネットワーク制約、PDF/OCR 抽出結果によって dataset/corpus 準備または評価結果が変わるため、本番評価セット化前には source URL、令和4年改定版への差し替え、根拠 bbox / crop の確定を別途確認してください。

図面 QA の page 評価では、citation / `finalEvidence` / raw `retrieved` のいずれにも `pageStart`、`pageEnd`、`pageNumber`、`pageOrSheet` などの page metadata が観測できない場合、`expected_page_hit_rate` と `grounded_page_accuracy` は `not_applicable` として扱います。API の citation は検索 hit metadata に compact な図面根拠情報がある場合、optional field として `pageOrSheet`、`drawingNo`、`sheetTitle`、`scale`、`regionId`、`regionType`、`sourceType`、`bbox` を返します。回答内容のみを見る `answer_content_accuracy`、回答内容と期待ファイル根拠を見る `grounded_file_accuracy`、page metadata がある行だけで page grounding まで見る `grounded_page_accuracy` を分けて確認してください。

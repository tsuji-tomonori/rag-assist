...
`ManagedPolicyArns` は必須です。`AdministratorAccess` は指定せず、デプロイに必要な最小権限のポリシーのみを指定してください。

`AllowedWorkflowRef` を使うと、想定したworkflowファイル・ブランチに限定できます（既定値: `.github/workflows/memorag-deploy.yml@refs/heads/main`）。

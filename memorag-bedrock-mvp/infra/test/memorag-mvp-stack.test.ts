...
test("fails the benchmark CodeBuild runner when auth token resolution fails", () => {
  ...
  assert.ok(buildSpec.phases.pre_build.commands.includes("if [ \"$SUITE_ID\" = \"standard-agent-v1\" ] || [ \"$SUITE_ID\" = \"smoke-agent-v1\" ] || [ \"$SUITE_ID\" = \"clarification-smoke-v1\" ] || [ \"$SUITE_ID\" = \"search-standard-v1\" ] || [ \"$SUITE_ID\" = \"search-smoke-v1\" ]; then export BENCHMARK_CORPUS_DIR=benchmark/corpus/standard-agent-v1; export BENCHMARK_CORPUS_SUITE_ID=standard-agent-v1; fi"))
  ...
  assert.ok(buildSpec.phases.pre_build.commands.includes("if [ \"$SUITE_ID\" = \"allganize-rag-evaluation-ja-v1\" ]; then export ALLGANIZE_RAG_DATASET_OUTPUT=\"$DATASET\"; export ALLGANIZE_RAG_CORPUS_DIR=./benchmark/.runner-allganize-corpus; export BENCHMARK_CORPUS_DIR=\"$ALLGANIZE_RAG_CORPUS_DIR\"; npm run prepare:allganize-ja -w @memorag-mvp/benchmark; elif [ \"$SUITE_ID\" = \"mmrag-docqa-v1\" ]; then export MMRAG_DOCQA_DATASET_OUTPUT=\"$DATASET\"; export MMRAG_DOCQA_CORPUS_DIR=./benchmark/.runner-mmrag-docqa-corpus; export BENCHMARK_CORPUS_DIR=\"$MMRAG_DOCQA_CORPUS_DIR\"; export BENCHMARK_CORPUS_SUITE_ID=mmrag-docqa-v1; npm run prepare:mmrag-docqa -w @memorag-mvp/benchmark; elif [ \"$SUITE_ID\" = \"jp-public-pdf-qa-v1\" ]; then export JP_PUBLIC_PDF_QA_DATASET_OUTPUT=\"$DATASET\"; export JP_PUBLIC_PDF_QA_CORPUS_DIR=./benchmark/.runner-jp-public-pdf-qa-corpus; export BENCHMARK_CORPUS_DIR=\"$JP_PUBLIC_PDF_QA_CORPUS_DIR\"; export BENCHMARK_CORPUS_SUITE_ID=jp-public-pdf-qa-v1; npm run prepare:jp-public-pdf-qa -w @memorag-mvp/benchmark; else aws s3 cp \"$DATASET_S3_URI\" \"$DATASET\"; fi"))
  assert.equal(

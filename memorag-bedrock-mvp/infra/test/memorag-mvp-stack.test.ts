...
test("fails the benchmark CodeBuild runner when auth token resolution fails", () => {
  ...
  assert.ok(buildSpec.phases.pre_build.commands.includes("if [ \"$SUITE_ID\" = \"standard-agent-v1\" ]; then export BENCHMARK_CORPUS_DIR=benchmark/corpus/standard-agent-v1; fi"))
  assert.equal(

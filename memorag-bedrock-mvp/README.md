...
Hono + `@hono/zod-openapi` で既存 REST API の OpenAPI を生成します。`packages/contract` は Zod schema、oRPC contract、共有型の source of truth として追加しており、初回移行では `/rpc/*` に chat / benchmark の oRPC endpoint を併設しています。Web と benchmark runner は `@memorag-mvp/contract` 由来の `ApiClient` 型で oRPC client を作成し、既存 REST endpoint は移行期間の互換性のため残しています。

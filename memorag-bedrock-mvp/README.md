...
図面 QA の page 評価では、citation / `finalEvidence` / raw `retrieved` のいずれにも `pageStart`、`pageEnd`、`pageNumber`、`pageOrSheet` などの page metadata が観測できない場合、`expected_page_hit_rate` と `grounded_page_accuracy` は `not_applicable` として扱います。回答内容のみを見る `answer_content_accuracy`、回答内容と期待ファイル根拠を見る `grounded_file_accuracy`、page metadata がある行だけで page grounding まで見る `grounded_page_accuracy` を分けて確認してください。

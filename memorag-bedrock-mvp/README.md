...
`standard-agent-v1` の runner は、実行前に `benchmark/corpus/standard-agent-v1/handbook.md` を `/documents` へ seed し、active chunk が作成されたことを確認してから `/benchmark/query` を実行します。同じ source hash と ingest signature の seed 済み資料が active な場合は再アップロードを省略します。任意の corpus を使う場合は `BENCHMARK_CORPUS_DIR` を指定してください。

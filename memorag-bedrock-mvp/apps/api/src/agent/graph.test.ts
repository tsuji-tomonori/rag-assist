...
test("fixed workflow search plan trace records complexity, facts, actions, and stop criteria from input", async () => {
  ...
  assert.match(planStep?.detail ?? "", /- fact-1 priority=1 status=missing: 経費精算の申請手順と期限 期限/)
  assert.match(planStep?.detail ?? "", /- fact-2 priority=2 status=missing: 経費精算の申請手順と期限 手順/)

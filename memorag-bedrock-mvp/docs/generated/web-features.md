...
| チャット | chat | RAG 質問、回答表示、引用、追加確認、担当者エスカレーション、チャット入力を扱う領域です。 | chat | 16 | 44 | 「回答をコピー済み / 回答をコピー」を実行するボタン。<br>「この候補で質問する」を実行するボタン。<br>「自分で入力」を実行するボタン。 ほか 40 件 | [chat.md](web-features/chat.md) |
...
| ドキュメント | documents | ドキュメント upload、document group、共有、blue-green reindex 操作を扱う領域です。 | documents | 5 | 33 | 「管理者設定へ戻る」を実行するボタン。<br>「共有フォルダ / 選択してください / 共有 Cognito group / 共有更新」を入力・送信するフォーム。<br>「共有フォルダ / 選択してください」に紐づく入力ラベル。 ほか 25 件 | [documents.md](web-features/documents.md) |
...
| 共通 | shared | 複数領域で再利用される表示部品です。単独の画面ではなく、他の画面から使われます。 | - | 8 | 5 | 「キャンセル」を実行するボタン。<br>「label」を実行するボタン。 | [shared.md](web-features/shared.md) |

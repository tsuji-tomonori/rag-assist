...
| ドキュメント | documents | / (client-state) | [ドキュメント](web-features/documents.md) | DocumentWorkspace | canManageDocuments | 管理者設定へ戻る、文書詳細を閉じる、この資料に質問する、再インデックス、削除、共有フォルダ / 選択してください / 共有 Cognito group / 既存 shared group 候補 / 追加: / 削除: / 変更なし: /… ほか 61 件 | confirmed |
...
- 主要操作: チャットへ戻る、ドキュメント管理 / 件、担当者対応 / 件が対応待ち、デバッグ / 評価 / 件の実行履歴、性能テスト / 件の実行履歴、更新、管理対象ユーザー作成、メール、表示名、任意 ほか 19 件
...
- 主要操作: 管理者設定へ戻る、文書詳細を閉じる、この資料に質問する、再インデックス、削除、共有フォルダ / 選択してください / 共有 Cognito group / 既存 shared group 候補 / 追加: / 削除: / 変更なし: /…、共有フォルダ / 選択してください、shareGroupId || selectedGroupId、選択してください、共有 Cognito group ほか 57 件

...
`task benchmark:sample` は上記の標準 corpus 指定を含む。`task benchmark:search:sample` も `BENCHMARK_SUITE_ID=search-standard-v1` と同じ標準 corpus seed を指定する。`smoke-agent-v1`、`standard-agent-v1`、`clarification-smoke-v1`、`search-standard-v1` では、同じ `BENCHMARK_CORPUS_SUITE_ID` の benchmark seed 文書を削除してから `handbook.md` を `/documents` に再 seed し、active chunk が作成されてから評価 query / search を実行する。PDF corpus は upload session の転送後に `POST /document-ingest-runs` を開始し、`GET /document-ingest-runs/{runId}` を polling して active chunk 付き manifest を待つ。runner は `/benchmark/query` と corpus 付き `/benchmark/search` に `BENCHMARK_CORPUS_SUITE_ID` を渡し、検索前 filter で `source=benchmark-runner`、`docType=benchmark-corpus`、`benchmarkSuiteId` を強制する。削除または再 seed に失敗した場合、古い corpus で測定を継続せず runner は失敗する。抽出可能なテキストがない PDF seed と、async ingest run が Textract OCR timeout を返した PDF seed は `skipped_unextractable` として summary / report に記録され、agent benchmark ではその file を期待する row を `skippedRows` として評価対象から除外する。その他の worker failure と run polling 失敗は runner fatal とする。polling 間隔は `BENCHMARK_INGEST_RUN_POLL_INTERVAL_MS`、最大待機時間は `BENCHMARK_INGEST_RUN_TIMEOUT_MS` で調整できる。`mmrag-docqa-v1` をローカルで全量確認する場合は `MMRAG_DOCQA_DATASET_OUTPUT=.local-data/mmrag-docqa-v1/dataset.jsonl`、`MMRAG_DOCQA_CORPUS_DIR=.local-data/mmrag-docqa-v1/corpus` で `npm run prepare:mmrag-docqa -w @memorag-mvp/benchmark` を実行し、その後 `DATASET=.local-data/mmrag-docqa-v1/dataset.jsonl`、`BENCHMARK_SUITE_ID=mmrag-docqa-v1`、`BENCHMARK_CORPUS_DIR=.local-data/mmrag-docqa-v1/corpus`、`BENCHMARK_CORPUS_SUITE_ID=mmrag-docqa-v1` を指定して benchmark runner を実行する。`mtrag-v1` と `chatrag-bench-v1` は `conversation-run.ts` を使い、1行1会話の JSONL を turn 順に処理して前ターンを `conversationHistory` として `/benchmark/query` に渡す。既定 prepare は `yubo2333/MMLongBench-Doc` train split の 1,091 rows と参照 PDF を対象にし、`MMRAG_DOCQA_DOWNLOAD_DOCUMENTS=0` は converter の単体確認用に限る。seed 文書は `aclGroups: ["BENCHMARK_RUNNER"]` と `docType: "benchmark-corpus"` で隔離し、通常利用者の文書一覧にも表示しない。corpus seed の削除・アップロード時間は runner setup であり、summary の latency 指標は `/benchmark/query` または `/benchmark/search` の初回 API call を対象にする。同じ corpus を複数 suite で共有する場合は `BENCHMARK_CORPUS_SUITE_ID` で seed 判定用の corpus identity を固定する。`<file>.metadata.json` の `searchAliases` は seed metadata に含まれ、search benchmark の alias case で使う。

...
`.github/workflows/memorag-ci.yml` はpull requestごとに lint、typecheck、test、build に加えて、cdk-nag有効状態で `npm run synth:yaml -w @memorag-mvp/infra` を実行する。これにより `AwsSolutions-*` のerrorはdeploy前のCIで検出される。

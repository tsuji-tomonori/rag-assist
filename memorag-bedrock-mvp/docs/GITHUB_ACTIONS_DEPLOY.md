...
`AllowedWorkflowRef` を使うと、OIDCトークンの `workflow_ref` claim を特定workflow/branchに固定できる（既定値: `tsuji-tomonori/rag-assist/.github/workflows/memorag-deploy.yml@refs/heads/main`）。
...
`.github/workflows/memorag-ci.yml` はpull requestごとに lint、typecheck、test、build に加えて、cdk-nag有効状態で `npm run cdk -w @memorag-mvp/infra -- synth --context benchmarkSourceOwner=... --context benchmarkSourceRepo=... --context benchmarkSourceBranch=...` を実行する。これにより `AwsSolutions-*` のerrorはdeploy前のCIで検出される。

Benchmark runner の CodeBuild source は、context 未指定時に `tsuji-tomonori/rag-assist` の `main` を既定値として使う。GitHub Actions の deploy workflow は意図を明示するため、bootstrap、synth、deploy の各 CDK コマンドで `benchmarkSourceOwner`、`benchmarkSourceRepo`、`benchmarkSourceBranch` を渡す。
...
task cdk:test
task cdk:synth:yaml
task cdk:synth:yaml CDK_CONTEXT_ARGS='--context benchmarkSourceOwner=tsuji-tomonori --context benchmarkSourceRepo=rag-assist --context benchmarkSourceBranch=main'
task cdk:deploy:ci CDK_CONTEXT_ARGS='--context defaultModelId=amazon.nova-lite-v1:0 --context embeddingModelId=amazon.titan-embed-text-v2:0 --context embeddingDimensions=1024 --context deploymentEnvironment=dev --context benchmarkSourceOwner=tsuji-tomonori --context benchmarkSourceRepo=rag-assist --context benchmarkSourceBranch=main'
```

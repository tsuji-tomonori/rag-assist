...
- NFR-013: システムは alias の作成、更新、review、無効化の監査ログを、検索 runtime と分離した object store 上の append-only record として保持できること。

## 受け入れ条件（この要件専用）

- AC-NFR013-001: alias audit log は `aliases/audit-log/{timestamp}-{eventId}.json` に保存されること。
...
- AC-NFR013-003: alias audit log は document metadata、lexical index、index manifest とは別の object key prefix に保持されること。
...
| 受け入れ基準 | `AC-NFR013-001` から `AC-NFR013-006` |
| 優先度 | A |
| 安定性 | High |
| 変更履歴 | 2026-05-02 初版 |
...
| 十分性 | OK | 保存場所、保持項目、参照権限、通常 response 非露出を含む |
| 理解容易性 | OK | object store prefix と required fields が明確 |

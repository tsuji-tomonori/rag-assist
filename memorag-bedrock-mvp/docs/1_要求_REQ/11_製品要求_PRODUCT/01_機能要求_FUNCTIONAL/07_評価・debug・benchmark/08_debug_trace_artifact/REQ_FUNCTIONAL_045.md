...
- FR-045: 管理者は、chat run の debug trace を時系列 artifact として取得できること。

## 受け入れ条件（この要件専用）

- AC-FR045-001: `SYSTEM_ADMIN` または同等の debug 権限を持つ利用者は、trace timeline を取得できること。
- AC-FR045-002: trace timeline は主要 step、sentence assessment、finalEvidence、判定理由を時系列で確認できること。
- AC-FR045-003: JSON または Markdown の download artifact は、対象 run を識別できる情報を含むこと。
- AC-FR045-004: 通常ユーザーは debug trace artifact を取得できないこと。
...
- `memorag-bedrock-mvp/docs/1_要求_REQ/11_製品要求_PRODUCT/11_非機能要求_NON_FUNCTIONAL/REQ_NON_FUNCTIONAL_015.md`

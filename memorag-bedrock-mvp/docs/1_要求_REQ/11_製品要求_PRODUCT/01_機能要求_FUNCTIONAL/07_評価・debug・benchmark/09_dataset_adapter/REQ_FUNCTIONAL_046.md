...
- FR-046: benchmark dataset adapter は、dataset ごとの入力を共通評価形式へ正規化できること。

## 受け入れ条件（この要件専用）

- AC-FR046-001: adapter は corpus、expected answer、source context、skip reason を共通 schema へ変換できること。
- AC-FR046-002: adapter は dataset row ごとの評価可否を runner が判別できる形で返すこと。
- AC-FR046-003: adapter は dataset 固有値や期待語句の分岐を product RAG 実装へ持ち込まないこと。
- AC-FR046-004: adapter 出力から生成される metrics と report artifact は、dataset 間で比較可能な項目名を持つこと。

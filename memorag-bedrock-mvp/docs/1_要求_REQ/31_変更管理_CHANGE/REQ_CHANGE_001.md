...
| 7. 評価・debug・benchmark | `FR-010`, `FR-011`, `FR-012`, `FR-013`, `FR-019`, `FR-039`, `FR-040` | 実行履歴確認、実行結果ダウンロード、UI非依存評価、外部連携仕様、benchmark 指標、benchmark corpus seed、benchmark corpus 隔離 | `FR-024` は管理導線として関連する。`FR-027` は利用状況・コスト監査として関連する。`FR-029` は確認質問指標として関連する。`FR-039` は文書取り込みにも関連する。`FR-040` は検索品質制御と認可境界にも関連する。 |

...
| `FR-021`, `FR-031`, `FR-032`, `FR-033`, `FR-034`, `FR-035`, `FR-036`, `FR-037`, `FR-024`, `NFR-011` | `ASR-SEC-001` | `ARC_ADR_001` | `DES_HLD_001`, `DES_DLD_007`, `DES_API_001`, `DES_DATA_001` | `REQ_ACCEPTANCE_001` |
...
| --- | --- | --- | --- |
| 1. 文書・知識ベース管理 | `FR-001`, `FR-002`, `FR-007`, `FR-008`, `FR-020` | 文書登録、QA利用可能化、文書削除、多抽象度メモリ生成 | `FR-024` は管理導線として関連する。`FR-020` は検索補助メモリとして検索品質制御にも関連する。 |
...
| 7. 評価・debug・benchmark | `FR-010`, `FR-011`, `FR-012`, `FR-013`, `FR-019` | 実行履歴確認、実行結果ダウンロード、UI非依存評価、外部連携仕様、benchmark 指標 | `FR-024` は管理導線として関連する。`FR-027` は利用状況・コスト監査として関連する。`FR-029` は確認質問指標として関連する。 |

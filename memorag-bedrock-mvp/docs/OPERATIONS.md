...
非同期 document ingest worker は、PDF benchmark の corpus seed も本番と同じ ingest 経路で処理するため、`DocumentIngestRunWorkerFunction` を 4096MB memory、15分 timeout、4GiB ephemeral storage で動かす。大きい PDF の失敗調査では CloudWatch Logs の `document_ingest_stage` と `document_extract_stage` を確認する。これらのログは本文を出さず、`s3_read`、`extract`、`pdf-parse`、`pdftotext`、`textract`、`chunk`、`embedding`、`vector_put` の stage、file size、text length、chunk count、`process.memoryUsage()` の snapshot を記録する。

`architecture-drawing-qarag-v0.1` の prepare は、source PDF ごとの `<file>.metadata.json` に `drawingSourceType`、`drawingSheetMetadata`、`drawingRegionIndex` を生成する。これらは benchmark seed metadata と vector metadata に伝搬し、図面番号・図面名・縮尺・凡例・注記・詳細図の初期検索手がかりとして使う。`drawingRegionIndex` の bbox は seed QA の evidence anchor から作る初期 region candidate であり、実測 OCR crop 座標ではないため、精密な bbox / crop 確定は後続の図面領域検出 task で扱う。

...
`architecture-drawing-qarag-v0.1` の prepare は、source PDF ごとの `<file>.metadata.json` に `drawingSourceType`、`drawingSheetMetadata`、`drawingRegionIndex`、`drawingReferenceGraph` を生成する。これらは benchmark seed metadata と vector metadata に伝搬し、図面番号・図面名・縮尺・凡例・注記・詳細図の初期検索手がかりとして使う。`drawingRegionIndex` の bbox は seed QA の evidence anchor から作る初期 region candidate であり、実測 OCR crop 座標ではないため、精密な bbox / crop 確定は後続の図面領域検出 task で扱う。`drawingReferenceGraph` は page / region / detail / section / callout node、contains / references edge、`detailIndex`、`calloutEdges`、source hierarchy conflict を保持し、詳細図・断面参照 QA で source bbox と target bbox を検証できるようにする。

図面 benchmark の寸法・縮尺・口径・延長・範囲条件は、dataset row の `expectedNormalizedValues` で正規化期待値を持てる。runner は回答本文と citation/evidence text から `scale:1/<n>`、`dimension:mm:<value>`、`diameter:<phi|d|a>:<value>`、`length:m:<value>`、`range:<op>:mm:<value>` を抽出し、`normalized_answer_accuracy` と `normalized_answer_mismatch` で通常の `expectedContains` とは別に評価する。これは採点時の単位表記ゆれを吸収するための評価補助であり、根拠 bbox / crop の実測抽出を代替しない。

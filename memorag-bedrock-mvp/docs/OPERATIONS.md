...
- UI の文書アップロードは `POST /documents/uploads` で upload URL を発行し、S3 へ転送してから `POST /document-ingest-runs` で非同期 ingest run を開始する。run 状態は `GET /document-ingest-runs/{runId}`、進捗 event は `GET /document-ingest-runs/{runId}/events` で確認する。既存 `POST /documents/uploads/{uploadId}/ingest` は後方互換の同期 API として残すが、大きな PDF、OCR fallback、embedding が絡む通常運用では使わない。S3 転送で CORS エラーが出る場合は `DocumentsBucket` の PUT CORS、署名 URL の有効期限、ブラウザが送っている `Content-Type` を確認する。local file store では同じ contract の API upload URL を返す。

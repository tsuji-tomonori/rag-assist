...
非同期 document ingest worker は、PDF benchmark の corpus seed も本番と同じ ingest 経路で処理するため、`DocumentIngestRunWorkerFunction` を 4096MB memory、30分 timeout、4GiB ephemeral storage で動かす。大きい PDF の失敗調査では CloudWatch Logs の `document_ingest_stage` と `document_extract_stage` を確認する。これらのログは本文を出さず、`s3_read`、`extract`、`pdf-parse`、`pdftotext`、`textract`、`chunk`、`embedding`、`vector_put` の stage、file size、text length、chunk count、`process.memoryUsage()` の snapshot を記録する。

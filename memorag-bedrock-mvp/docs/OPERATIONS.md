...
- UI の文書アップロードは `POST /documents/uploads` で upload URL を発行し、S3 へ転送してから `POST /documents/uploads/{uploadId}/ingest` で登録する。S3 転送で CORS エラーが出る場合は `DocumentsBucket` の PUT CORS、署名 URL の有効期限、ブラウザが送っている `Content-Type` を確認する。local file store では同じ contract の API upload URL を返す。

...
`smoke-agent-v1`、`standard-agent-v1`、`clarification-smoke-v1`、`search-smoke-v1`、`search-standard-v1` は `handbook.md` を期待資料にする。CodeBuild runner は query / search 実行前に `benchmark/corpus/standard-agent-v1/handbook.md` を seed し、active chunk が存在することを確認する。同じ `benchmarkSourceHash`、per-file metadata、`benchmarkIngestSignature` の seed 済み active document がある場合は再アップロードしない。PDF seed は `/documents/uploads` で `purpose=benchmarkSeed` の upload URL を作成し、S3 に転送してから `/documents/uploads/{uploadId}/ingest` で登録する。これにより 10 MiB 前後の JSON request body 制限に base64 PDF を載せない。抽出可能テキストがない PDF と Textract OCR fallback が同期 ingestion の待機時間内に完了しない PDF は `skipped_unextractable` として corpus seed 結果に残し、その file を期待する dataset row は評価対象から除外する。CodeBuild ではこれらの suite の `BENCHMARK_CORPUS_SUITE_ID` を `standard-agent-v1` に固定し、同じ corpus を共有する。`mmrag-docqa-v1` は pre_build で `MMRAG_DOCQA_DATASET_OUTPUT=$DATASET`、`MMRAG_DOCQA_CORPUS_DIR=./benchmark/.runner-mmrag-docqa-corpus` を設定し、`npm run prepare:mmrag-docqa -w @memorag-mvp/benchmark` で全量 dataset と参照 PDF corpus を準備する。`BENCHMARK_CORPUS_SUITE_ID` は `mmrag-docqa-v1` に固定する。prepare は dataset server が返す行数が既定の 1,091 と一致しない場合に失敗し、上流更新を意図的に受け入れる場合だけ `MMRAG_DOCQA_ALLOW_ROW_COUNT_DRIFT=1` を使う。seed 文書は `metadata.source = "benchmark-runner"`、`metadata.docType = "benchmark-corpus"`、`metadata.aclGroups = ["BENCHMARK_RUNNER"]` を強制し、通常利用者の RAG 検索・回答・文書一覧から隔離する。agent mode の `/benchmark/query` は内部的に `source=benchmark-runner` と `docType=benchmark-corpus` の検索 filter を適用し、一般文書が benchmark の retrieval / citation に混入しないようにする。`<file>.metadata.json` の `searchAliases` は benchmark seed の alias 検証用に許可する。`BENCHMARK_RUNNER` はこの前処理のために `benchmark:seed_corpus` を持ち、seed 文書確認用に `/documents` の一覧取得だけを許可される。通常の `rag:doc:write:group` や `rag:doc:read` は持たないため、任意の一般文書 upload や通常 `/search` は実行できない。

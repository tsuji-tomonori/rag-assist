...
`architecture-drawing-qarag-v0.1` の prepare は、source PDF ごとの `<file>.metadata.json` に `drawingSourceType`、`drawingSheetMetadata`、`drawingRegionIndex`、`drawingReferenceGraph`、`drawingExtractionArtifacts` を生成する。これらは benchmark seed metadata と vector metadata に伝搬し、図面番号・図面名・縮尺・凡例・注記・詳細図の初期検索手がかりとして使う。`drawingRegionIndex` の bbox は seed QA の evidence anchor から作る初期 region candidate であり、実測 OCR crop 座標ではないため、精密な bbox / crop 確定は後続の図面領域検出 task で扱う。`drawingExtractionArtifacts` は region 単位で `pdf_text -> ocr -> vlm_ocr` の順に局所抽出を試した結果を保持し、`sourceMethod`、bbox、confidence、parser version、raw text、normalized value lineage、failure reason を残す。VLM-OCR は page 全体ではなく crop region の最後の fallback とし、利用できない場合は推測値を作らず `failureReason=vlm_ocr_unavailable` を残す。`drawingReferenceGraph` は page / region / detail / section / callout node、contains / references edge、`detailIndex`、`calloutEdges`、source hierarchy conflict を保持し、詳細図・断面参照 QA で source bbox と target bbox を検証できるようにする。

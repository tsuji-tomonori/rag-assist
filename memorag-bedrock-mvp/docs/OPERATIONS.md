...
`standard-agent-v1` は `handbook.md` を期待資料にする。CodeBuild runner は query 実行前に `benchmark/corpus/standard-agent-v1/handbook.md` を `/documents` へ seed し、active chunk が存在することを確認する。同じ `benchmarkSourceHash` と `benchmarkIngestSignature` の seed 済み active document がある場合は再アップロードしない。`BENCHMARK_RUNNER` はこの前処理のために `benchmark:run` に加えて `rag:doc:read` と `rag:doc:write:group` を持つ。`rag:doc:read` により `POST /search` も実行可能だが、文書削除、run 履歴参照、キャンセル、artifact download は許可しない。

...
機能要求の詳細な分類索引は `1_要求_REQ/11_製品要求_PRODUCT/01_機能要求_FUNCTIONAL/README.md` を正とする。

| L1 主カテゴリ | 主分類の FR | 主な L2 機能群 |
| --- | --- | --- |
...
| 3. RAG検索品質制御 | `FR-016`, `FR-017`, `FR-018`, `FR-023`, `FR-026` | 検索計画、検索実行、検索結果統合、検索結果評価、検索alias管理 |
| 4. 回答検証・ガードレール | `FR-014`, `FR-015` | 回答前ガード、回答後検証、strict grounded 制御 |
| 5. 会話履歴・お気に入り | `FR-022`, `FR-028` | 会話履歴保存、再表示、削除、スキーマ管理、お気に入り |
| 6. 問い合わせ・人手対応 | `FR-021` | 問い合わせ登録、属性、状態管理、担当者回答、通常利用者導線 |
| 7. 評価・debug・benchmark | `FR-010`, `FR-011`, `FR-012`, `FR-013`, `FR-019` | 実行履歴、ダウンロード、UI非依存評価、外部連携仕様、benchmark 指標 |
| 8. 認証・認可・管理・監査 | `FR-024`, `FR-025`, `FR-027` | self sign-up、最小権限、RAG運用管理、ユーザー・ロール管理、監査、コスト監査 |

各 `REQ_FUNCTIONAL_*` ファイルは `分類（L0-L3）` セクションを持ち、主分類、L2 機能群、関連カテゴリを個別に示す。

...
| Web Admin Workspace | `GET /me` permissions、documents、questions、debug runs、benchmark runs、admin users、roles、admin audit log、usage、costs | Phase 1/2 管理 view | `FR-024`, `FR-027`, `NFR-011` |
...
22. 未認証の通常利用者がアカウント作成を行う場合、Web UI は Cognito `SignUp` と `ConfirmSignUp` を使い、Cognito post-confirmation trigger が `CHAT_USER` のみを付与する。

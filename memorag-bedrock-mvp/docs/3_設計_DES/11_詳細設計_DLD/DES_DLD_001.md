...
5. 新しい RAG 判定値、件数上限、LLM 呼び出し設定を追加する場合は、node 内に直書きせず `config.ts` と `runtime-policy.ts` を先に拡張する。
...
6. 同一 required fact の high-score chunk 間で期限・金額などの値が食い違う場合は `riskSignals[type=value_mismatch]` と `conflictingFactIds` に記録する。
7. `riskSignals` がある場合だけ LLM judge に委譲し、同一 scope の矛盾か、scope 差分や誤検出で説明できるかを `llmJudge` に保存する。
8. LLM judge が高信頼で `NO_CONFLICT` と判定した場合のみ conflict 候補を解消し、それ以外は `retrievalQuality=partial` として現行条件・適用範囲の追加検索へ進める。
9. `矛盾`、`廃止`、`無効`、`取消` などの cue は初期 heuristic では拒否確定に使わず、追加検索または後段 gate の判断対象とする。
10. 選択した action は `RetrievalEvaluation.nextAction` として debug trace に保存する。

...
6. `RequiredFact` は fact type、subject、scope、expected value type、confidence、planner source を optional field として持ち、deterministic planner が question requirement detector の slot と signal を hot path で補完する。検索計画側に同じ slot 判定を重複実装しない。
...
6. 各 action の hit count、新規 evidence 件数、top score、retrieval diagnostics は `actionHistory` と debug trace に保存する。
...
1. `extract_policy_computations` は selected chunk があり、かつ tool intent、検索計画の required fact、質問と根拠内の比較可能な値 signal から policy computation が必要と判断できる場合だけ実行する。

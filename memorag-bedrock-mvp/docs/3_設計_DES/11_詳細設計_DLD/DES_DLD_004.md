...
3. workflow / 管理手順は `ANSWER_EDITOR`、`RAG_GROUP_MANAGER`、`BENCHMARK_RUNNER`、`USER_ADMIN`、`ACCESS_ADMIN`、`COST_AUDITOR`、`SYSTEM_ADMIN` を必要に応じて付与する。
...
| group 数が `BENCHMARK_RUNNER` を含む現行 role 定義と一致する | CDK stack assertion |

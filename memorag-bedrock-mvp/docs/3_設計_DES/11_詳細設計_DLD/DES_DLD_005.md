...
資料内の金額閾値条件は rule-based parser で自然文を解釈しない。retrieval / rerank 後に `extract_policy_computations` が LLM を構造化抽出器として使い、質問対象値、資料条件、効果、根拠 quote、`matchesQuestion`、`confidence` を JSON で抽出する。deterministic code は JSON schema validation、quote 存在確認、JPY 金額正規化、comparator 評価、computed fact 採番だけを担当する。
...
10. deterministic code は extraction JSON を検証し、`matchesQuestion=true`、`confidence >= 0.75`、quote が selected chunk 内に存在、JPY 金額と comparator が正規化可能な候補だけを `threshold_comparison` に変換する。
11. `execute_computation_tools` は question-only の日付・算術・未対応 index 判定に限定し、LLM 呼び出しや文書条件の自然文解釈を含めない。
12. `generate_answer` には `temporalContext` と `computedFacts` を渡し、LLM は計算を再実行せず自然文化だけを担当する。
13. `validate_citations` は document citation がなくても `usedComputedFactIds` が有効なら strict grounded として扱う。
14. `verify_answer_support` は document chunk と computed fact の両方を回答根拠として検証する。
...
| 資料内の金額閾値条件への該当可否 | `extract_policy_computations` で LLM が質問対象値、資料条件、効果、根拠 quote を JSON 抽出する。deterministic code は schema / quote / JPY / comparator / confidence / `matchesQuestion` を検証し、通過した候補だけを `threshold_comparison` として比較する |
| policy extraction が曖昧・低 confidence・quote 不一致・質問要件不一致 | computed fact を生成せず、通常の RAG gate / support verification に委ねる |
...
| quote 不一致、低 confidence、質問要件不一致、曖昧条件の破棄 | `policy-computation.test.ts` |

...
資料内の金額閾値条件は rule-based parser で自然文を解釈しない。retrieval / rerank 後に `extract_policy_computations` が LLM を構造化抽出器として使い、質問対象値、資料条件、効果、根拠 quote、`matchesQuestion`、`confidence` を JSON で抽出する。deterministic code は JSON schema validation、質問金額表記の質問内存在確認、quote 存在確認、閾値金額表記の quote 内存在確認、JPY 金額正規化、comparator 評価、computed fact 採番だけを担当する。
...
10. deterministic code は extraction JSON を検証し、`matchesQuestion=true`、`confidence >= 0.75`、`questionTarget.amountText` が質問内に存在、quote が selected chunk 内に存在、`condition.thresholdText` が quote 内に存在、JPY 金額と comparator が正規化可能な候補だけを `threshold_comparison` に変換する。
...
| 資料内の金額閾値条件への該当可否 | `extract_policy_computations` で LLM が質問対象値、資料条件、効果、根拠 quote を JSON 抽出する。deterministic code は schema / 質問金額表記の質問内存在 / quote / 閾値金額表記の quote 内存在 / JPY / comparator / confidence / `matchesQuestion` を検証し、通過した候補だけを `threshold_comparison` として比較する |
| policy extraction が曖昧・低 confidence・quote 不一致・質問金額表記不一致・閾値金額表記不一致・質問要件不一致 | computed fact を生成せず、通常の RAG gate / support verification に委ねる |

...
8. 資料内の「1万円以上なら領収書が必要」「1万円未満なら領収書は不要」のような金額閾値条件と質問中の金額を比較できる場合は、retrieval 後に `threshold_comparison` として確定する。`polarity` で必要条件と不要条件を区別し、`satisfiesCondition` だけで「必要」と断定しない。
...
| 資料内の金額閾値条件への該当可否 | RAG retrieval 後に文書条件と質問金額を `threshold_comparison` として比較する。具体的な requirement term が質問と文書で一致しない場合は生成しない |
...
| 不要条件、複数金額、無関係な対象質問の誤変換防止 | `computation.test.ts`, `graph.test.ts` |

...
資料内の金額閾値条件は rule-based parser で自然文を解釈しない。retrieval / rerank 後に `extract_policy_computations` が LLM を構造化抽出器として使い、質問対象値、資料条件、条件全体表現、比較表現、効果、対象、根拠 quote、`matchesQuestion`、`confidence` を JSON で抽出する。deterministic code は JSON schema validation、質問金額表記の質問内存在確認、quote 存在確認、条件全体表現・閾値金額表記・比較表現・対象表現・効果表現の quote 内存在確認、条件全体表現内での閾値金額表記と比較表現の近接確認、比較表現と comparator enum の固定対応確認、効果表現と effect enum の固定対応確認、JPY 金額正規化、comparator 評価、computed fact 採番だけを担当する。
...
10. deterministic code は extraction JSON を検証し、`matchesQuestion=true`、`confidence >= 0.75`、`questionTarget.amountText` が質問内に存在、quote が selected chunk 内に存在、`condition.conditionText`、`condition.thresholdText`、`condition.comparatorText`、`consequence.targetText`、`consequence.effectText` が quote 内に存在、`condition.conditionText` が `thresholdText` と `comparatorText` を同じ条件表現として含む、`condition.comparatorText` が comparator enum と固定対応する、`consequence.effectText` が effect enum と固定対応する、JPY 金額と comparator が正規化可能な候補だけを `threshold_comparison` に変換する。

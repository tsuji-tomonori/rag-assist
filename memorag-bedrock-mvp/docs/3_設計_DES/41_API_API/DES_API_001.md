...
管理画面で選択できる非同期 benchmark suite を返す。`MMRAG-DocQA` は suite ID `mmrag-docqa-v1`、agent mode、dataset key `hf://datasets/yubo2333/MMLongBench-Doc` として返す。CodeBuild runner はこの suite で Hugging Face `MMLongBench-Doc` train split 全 1,091 rows を prepare してから実行する。

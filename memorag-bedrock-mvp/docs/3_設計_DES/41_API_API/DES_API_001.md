...
| `POST /chat-runs` | 非同期質問応答 run 作成 | `FR-003`, `FR-004`, `FR-005`, `FR-026` |
| `GET /chat-runs/{runId}/events` | 質問応答 run の SSE 進捗・最終回答 streaming | `FR-003`, `FR-004`, `FR-005`, `FR-026`, `NFR-005` |
...
| 検索実行 | `POST /search` | `rag:doc:read` | `CHAT_USER`, `RAG_GROUP_MANAGER`, `BENCHMARK_RUNNER`, `SYSTEM_ADMIN` |
...
| 文書一覧 | `GET /documents` | `rag:doc:read` | `CHAT_USER`, `RAG_GROUP_MANAGER`, `BENCHMARK_RUNNER`, `SYSTEM_ADMIN` |

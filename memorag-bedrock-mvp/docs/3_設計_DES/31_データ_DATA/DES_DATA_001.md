...
| 項目 | 内容 |
| --- | --- |
| `queryCount` | agent `search_evidence` が実行した query 数 |
| `indexVersions` | agent search step 内で使った lexical index の opaque version 配列 |
| `aliasVersions` | agent search step 内で使った alias set の opaque version 配列 |
| `lexicalCount` | agent search step 全体の lexical 候補件数 |
| `semanticCount` | agent search step 全体の semantic 候補件数 |
| `fusedCount` | agent search step 全体の fused 候補件数 |
...
| `createdBy` / `updatedBy` / `reviewedBy` | 作成者、最終更新者、reviewer |
| `version` | alias 定義 version。検索 index に compile 済みの alias set version とは別に扱う |
| `createdAt` / `updatedAt` / `reviewedAt` / `disabledAt` | 作成・更新・review・無効化日時 |

### AliasAuditLogEntry

| 項目 | 内容 |
| --- | --- |
| `schemaVersion` | audit log schema version |
| `eventId` | audit event 識別子 |
| `aliasId` | 対象 alias |
| `action` | `created` / `updated` / `reviewed` / `disabled` |
| `actorUserId` / `actorEmail` | 操作者 |
| `at` | 操作日時 |
| `beforeStatus` / `afterStatus` | 操作前後の lifecycle status |
| `reason` | 操作理由 |
| `aliasVersion` | 操作後 alias 定義 version |
| `scope` | 操作時点の alias scope |
...
- 管理 API の alias 定義は `aliases/tenantId=.../source=.../docType=.../definitions/{aliasId}.json` に保存する。
- alias audit log は `NFR-013` に従い、document artifact とは別の監査ログ専用 bucket の `aliases/audit-log/{timestamp}-{eventId}.json` に保存する。
...
- `NFR-004`, `NFR-005`, `NFR-006`, `NFR-010`, `NFR-011`, `NFR-012`, `NFR-013`
